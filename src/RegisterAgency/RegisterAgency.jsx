...
const RegisterAgency = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col font-sans" style={{ background: "#f9fafb" }}>
      ...
      {/* ── Navbar ── */}
      <div className="flex justify-between items-center px-4 sm:px-6 md:px-12 py-3 bg-white"
        style={{ borderBottom: `1px solid ${G.greenBorder}`, boxShadow: "0 2px 12px rgba(110,192,48,0.08)" }}>
        <img src="/weblance.jpeg" alt="Weblance" style={{ height: 36, width: "auto" }} className="sm:h-11" />
        <button onClick={onExit}
          className="flex items-center gap-1.5 bg-transparent border-none text-xs sm:text-sm font-semibold cursor-pointer transition-colors"
          style={{ color: "#4b5563" }}
          onMouseEnter={e => e.currentTarget.style.color = G.navyDeep}
          onMouseLeave={e => e.currentTarget.style.color = "#4b5563"}>
          <span>⊞</span> <span className="hidden sm:inline">Save &amp; Exit</span><span className="sm:hidden">Exit</span>
        </button>
      </div>

      {/* ── Step counter + % Complete ── */}
      <div className="px-4 sm:px-6 md:px-12 pt-3 sm:pt-4 pb-2" style={{ background: "#f9fafb" }}>
      ...
      <div className="px-4 sm:px-6 md:px-12" style={{ background: "#f9fafb" }}>
      ...
      <div className="py-3 sm:py-4 px-4 sm:px-6 md:px-12 overflow-x-auto" style={{ background: "#f9fafb" }}>
        <div className="flex items-start w-full max-w-4xl mx-auto min-w-max sm:min-w-0">
          {STEPS.map((s, idx) => {
            ...
            return (
              <React.Fragment key={s.id}>
                ...
                >
                  <div
                    className="w-7 sm:w-8 h-7 sm:h-8 rounded-full flex items-center justify-center text-xs font-semibold transition-all duration-200"
                    style={circleStyle(isActive, isDone)}
                  >
                    {s.id}
                  </div>
                  <div className="text-[9px] sm:text-[10px] text-center whitespace-nowrap mt-1.5" style={{
      ...
      {/* ── Content ── */}
      <div className="flex-1 px-4 sm:px-6 md:px-12 pt-4 sm:pt-6 pb-8 sm:pb-10 w-full">
        {renderStep()}
      </div>
