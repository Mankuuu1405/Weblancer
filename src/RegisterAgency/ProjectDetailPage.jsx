...
export default function ProjectDetailPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {/* Sticky header */}
      <div className="bg-white border-b border-gray-200 px-4 sm:px-8 py-3 sticky top-0 z-20">
        <div className="max-w-6xl mx-auto flex items-center justify-between gap-4 flex-wrap">
          <div className="flex items-center gap-3">
            {/* Weblance logo */}
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-8 py-6 flex flex-col gap-5">
        ...
        <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
          <div className="flex border-b border-gray-100 px-4 overflow-x-auto">
            {["Overview","Task Board","Team","Finance","AI Insights"].map(t => (
              <button key={t} onClick={() => setSubTab(t)}
                className={`px-4 py-3.5 text-sm font-medium whitespace-nowrap transition-colors ${subTab===t?"text-gray-900 border-b-2 border-blue-600":"text-gray-400 hover:text-gray-700"}`}>
                {t==="AI Insights"?"🤖 ":""}{t}
              </button>
            ))}
          </div>
          ...
          <div className="p-5 sm:p-6">
            ...
            {subTab==="Overview" && (
              <div className="flex flex-col gap-6">
                ...
                <div className="flex flex-wrap gap-2 pt-1">
                  {["Open ProjectStream", `Submit ${project.milestones.find(m=>m.status==="inprogress")?.id||"Next"}`, "Message Client", "Upload Deliverable"].map((btn,i) => (
                    <button key={i} className={`text-sm px-4 py-2 rounded-lg border transition-colors font-medium ${i===1?"text-white border-blue-700":"border-gray-200 text-gray-700 hover:bg-gray-50"}`}
              ...
              <div className="flex flex-col gap-4">
                ...
                <div className="border border-gray-200 rounded-xl p-5">
                  ...
                  {project.milestones.map((m,i) => (
                    <div key={i} className="flex items-center justify-between py-3 border-b border-gray-50 last:border-0">
                      <div className="flex items-center gap-3">
                        <span className={`w-2 h-2 rounded-full inline-block ${m.status==="done"?"bg-blue-600":m.status==="inprogress"?"bg-blue-400":"bg-gray-200"}`} />
                        <span className="text-sm text-gray-700">{m.id}: {m.label}</span>
                      </div>
                      <div className="flex items-center gap-3">
                        <span className="text-sm font-semibold text-gray-900">{m.amount}</span>
