...
function MilestoneCard({ m }) {
  const s = {
    done:       { wrap:"border-blue-200 bg-blue-50",   label:"text-blue-800",  amount:"text-blue-600"  },
  ...
  return (
    <div className={`rounded-xl border ${s.wrap} p-4 flex flex-col gap-1.5`}>
      <div className="flex items-center justify-between">
        ...
        {m.status==="done" && <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><circle cx="8" cy="8" r="7" stroke="#1B72C0" strokeWidth="1.3"/><path d="M4.5 8l2.5 2.5 4.5-4.5" stroke="#1B72C0" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round"/></svg>}
        {m.status==="inprogress" && <span className="text-xs font-bold text-blue-600 bg-blue-50 border border-blue-200 px-2 py-0.5 rounded-full">{m.pct}%</span>}
      </div>
      <div className={`text-sm font-semibold ${s.label}`}>{m.label}</div>
      <div className={`text-sm font-bold ${s.amount}`}>{m.amount}</div>
      {m.status==="inprogress" && <div className="w-full bg-blue-100 rounded-full h-1.5 mt-1"><div className="h-1.5 rounded-full" style={{width:`${m.pct}%`, background:"linear-gradient(90deg,#0D2855,#1B72C0)"}}/></div>}
      {m.completedDate && <div className="text-xs text-blue-600 mt-0.5">✓ {m.completedDate}</div>}
...
function TaskColumn({ label, count, tasks, accent }) {
  ...
  const hc = { todo:"text-gray-500", inprogress:"text-blue-700", review:"text-purple-600", done:"text-blue-800" };
  return (
...
export default function ProjectDetailPage() {
  ...
  if (!project) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-4 bg-gray-50">
        ...
        <button onClick={() => navigate("/agency/dashboard")} className="text-sm text-blue-600 hover:underline">← Back to Dashboard</button>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {/* Sticky header */}
      <div className="bg-white border-b border-gray-200 px-3 sm:px-8 py-3 sticky top-0 z-20">
        <div className="max-w-6xl mx-auto flex items-center justify-between gap-3 flex-wrap">
          <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
            {/* Weblance logo */}
            <img src="/weblance.jpeg" alt="Weblance" style={{ height: 32, width: "auto" }} className="sm:h-9" />
            <span className="text-gray-200 hidden sm:inline">|</span>
            <button onClick={() => navigate("/agency/dashboard")}
              className="flex items-center gap-1.5 text-xs sm:text-sm text-gray-500 hover:text-gray-900 transition-colors font-medium border border-gray-200 px-2 sm:px-3 py-1.5 rounded-lg hover:bg-gray-50">
            ...
            <span className="text-gray-200 hidden sm:inline">|</span>
            <span className="font-bold text-gray-900 text-sm sm:text-base">{project.name}</span>
            <span className="text-gray-300 hidden md:inline">·</span>
            <span className="text-xs sm:text-sm text-gray-500 hidden md:inline">
              {project.client}
              {project.clientVerified && (
                <svg width="13" height="13" viewBox="0 0 13 13" fill="none" className="inline ml-1"><circle cx="6.5" cy="6.5" r="5.5" stroke="#1B72C0" strokeWidth="1.2"/><path d="M4 6.5l2 2 3.5-3.5" stroke="#1B72C0" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round"/></svg>
              )}
            </span>
          </div>
      ...
      <div className="max-w-6xl mx-auto px-3 sm:px-8 py-4 sm:py-6 flex flex-col gap-4 sm:gap-5">

        {/* Stats */}
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
          {[
            { label:"Total Budget", value:project.budget,        icon:"💰", color:"text-gray-900"  },
            { label:"Released",     value:project.released,      icon:"✅", color:"text-blue-700"  },
            ...
            <div key={i} className="bg-white border border-gray-200 rounded-xl p-3 sm:p-4 shadow-sm">
        ...
        {/* Progress bar */}
        <div className="bg-white border border-gray-200 rounded-xl px-4 sm:px-6 py-3 sm:py-4 shadow-sm">
          ...
          <div className="w-full bg-gray-100 rounded-full h-3 overflow-hidden">
            <div className="h-3 rounded-full" style={{width:`${project.progress}%`, background:"linear-gradient(90deg,#0D2855,#1B72C0)"}} />
          </div>
          ...
          <div className="flex border-b border-gray-100 px-2 sm:px-4 overflow-x-auto">
            {["Overview","Task Board","Team","Finance","AI Insights"].map(t => (
              <button key={t} onClick={() => setSubTab(t)}
                className={`px-3 sm:px-4 py-3 sm:py-3.5 text-xs sm:text-sm font-medium whitespace-nowrap transition-colors ${subTab===t?"text-gray-900 border-b-2 border-blue-600":"text-gray-400 hover:text-gray-700"}`}>
                {t==="AI Insights"?"🤖 ":""}{t}
              </button>
            ))}
          </div>
              ...
              <div className="flex flex-col gap-4 sm:gap-6">
                ...
                <div>
                  ...
                  <div className="flex flex-col gap-2">
                    {project.recentActivity.map((a,i) => (
                      <div key={i} className="flex items-start gap-2 sm:gap-3 bg-gray-50 rounded-lg px-3 sm:px-4 py-2 sm:py-3">
                        <span className="text-sm sm:text-base flex-shrink-0">{a.icon}</span>
                        <span className="flex-1 text-xs sm:text-sm text-gray-700">{a.text}</span>
                ...
                <div className="flex flex-wrap gap-2 pt-1">
                  {["Open ProjectStream", `Submit ${project.milestones.find(m=>m.status==="inprogress")?.id||"Next"}`, "Message Client", "Upload Deliverable"].map((btn,i) => (
                    <button key={i} className={`text-xs sm:text-sm px-3 sm:px-4 py-2 rounded-lg border transition-colors font-medium ${i===1?"text-white border-blue-700":"border-gray-200 text-gray-700 hover:bg-gray-50"}`}
                      style={i===1 ? {background:"linear-gradient(135deg,#0D2855,#1B72C0)"} : {}}>
                      {btn}
                    </button>
                  ))}
                </div>
            ...
            {subTab==="Team" && (
              <div className="flex flex-col gap-4">
                {project.team.map((m,i) => (
                  <div key={i} className="border border-gray-200 rounded-xl p-4 sm:p-5 flex flex-wrap items-center justify-between gap-3 sm:gap-4">
                    ...
                    <div className="flex flex-col gap-1.5 w-full sm:w-auto sm:min-w-[160px]">
                      ...
                      <div className="w-full bg-gray-100 rounded-full h-2">
                        <div className="h-2 rounded-full" style={{width:`${m.workload}%`, background:m.workload>80?"#ef4444":m.workload>60?"#f59e0b":"#1B72C0"}} />
                      </div>
                    </div>
                  </div>
                ))}
              ...
              <div className="flex flex-col gap-4">
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
                  {[
                    ...
                    { label:"Paid Out",        value:project.released,  sub:`${releasedPct}% of total`, color:"text-blue-700" },
                ...
                <div className="border border-gray-200 rounded-xl p-4 sm:p-5">
                  ...
                  {project.milestones.map((m,i) => (
                    <div key={i} className="flex items-center justify-between py-3 border-b border-gray-50 last:border-0 flex-wrap gap-2">
                      <div className="flex items-center gap-3">
                        <span className={`w-2 h-2 rounded-full inline-block flex-shrink-0 ${m.status==="done"?"bg-blue-600":m.status==="inprogress"?"bg-blue-400":"bg-gray-200"}`} />
                        <span className="text-xs sm:text-sm text-gray-700">{m.id}: {m.label}</span>
                      </div>
                      <div className="flex items-center gap-3">
                        <span className="text-sm font-semibold text-gray-900">{m.amount}</span>
                        <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${m.status==="done"?"bg-blue-50 text-blue-700":m.status==="inprogress"?"bg-blue-50 text-blue-600":"bg-gray-100 text-gray-400"}`}>
            ...
            {subTab==="AI Insights" && (
              <div className="flex flex-col gap-4">
                ...
                {project.aiInsights.map((ins,i) => (
                  <div key={i} className="flex items-start gap-2 sm:gap-3 bg-white border border-gray-200 rounded-xl px-4 sm:px-5 py-3 sm:py-4 shadow-sm">
                    <span className="text-sm sm:text-base flex-shrink-0">🤖</span>
                    <p className="text-xs sm:text-sm text-gray-700 leading-relaxed">{ins}</p>
                  </div>
                ))}
              </div>
            )}
