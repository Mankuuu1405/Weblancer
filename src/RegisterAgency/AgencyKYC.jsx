...
(() => {
  if (document.getElementById("wl-kyc-responsive")) return;
  const s = document.createElement("style");
  s.id = "wl-kyc-responsive";
  s.textContent = `
    @media (max-width: 768px) {
      .kyc-layout { grid-template-columns: 1fr !important; }
      .kyc-step-nav { flex-direction: row !important; overflow-x: auto; gap: 8px !important; padding-bottom: 4px; }
      .kyc-step-nav::-webkit-scrollbar { height: 3px; }
      .kyc-step-nav::-webkit-scrollbar-thumb { background: #1B72C0; border-radius: 99px; }
      .kyc-step-btn { min-width: 140px; margin-bottom: 0 !important; }
      .kyc-step-line { display: none !important; }
      .kyc-fields-grid { grid-template-columns: 1fr !important; }
      .kyc-header-row { flex-direction: column; align-items: flex-start !important; gap: 12px; }
      .kyc-status-bar { flex-direction: column; align-items: flex-start !important; gap: 10px; }
      .kyc-progress-row { width: 100%; }
      .kyc-main { padding: 16px 16px 48px !important; }
      .kyc-page-header { padding: 0 16px !important; }
      .kyc-panel-header { padding: 14px 16px !important; }
      .kyc-panel-body { padding: 16px !important; }
      .kyc-step-header { flex-direction: column; gap: 8px !important; }
      .kyc-doc-row { flex-wrap: wrap; gap: 10px !important; }
    }
    @media (max-width: 480px) {
      .kyc-step-btn { min-width: 120px; }
      .kyc-navbar { padding: 0 12px !important; }
      .kyc-navbar-mid { display: none !important; }
    }
  `;
  ...
  return (
    <div style={{ minHeight:"100vh", background:G.bg, fontFamily:FONT }}>
      ...
      <header style={{ background:G.white, borderBottom:`1px solid ${G.border}` }}>
        <div style={{ maxWidth:1100, margin:"0 auto" }} className="kyc-page-header" style={{ padding:"0 28px" }}>
          <div style={{ padding:"20px 0 0" }}>
            <h1 style={{ fontSize:"clamp(18px,4vw,22px)", fontWeight:800, color:G.navy, margin:0, letterSpacing:"-0.4px" }}>KYC Verification</h1>
          ...
          {/* Overall status bar */}
          <div className="kyc-status-bar" style={{ display:"flex", alignItems:"center", justifyContent:"space-between", paddingTop:16, paddingBottom:16, marginTop:14, borderTop:`1px solid #f3f4f6` }}>
            ...
            <div className="kyc-progress-row" style={{ display:"flex", alignItems:"center", gap:12 }}>
              <div style={{ flex:1, minWidth:120, maxWidth:200, background:"#f3f4f6", borderRadius:99, height:7, overflow:"hidden" }}>
      ...
      {/* Content */}
      <main className="kyc-main" style={{ maxWidth:1100, margin:"0 auto", padding:"24px 28px 64px" }}>
        ...
        <div className="kyc-layout" style={{ display:"grid", gridTemplateColumns:"280px 1fr", gap:20 }}>

          {/* Step navigator */}
          <div className="kyc-step-nav" style={{ display:"flex", flexDirection:"column", gap:0, position:"relative" }}>
            <div className="kyc-step-line" style={{ position:"absolute", left:19, top:28, bottom:28, width:2, background:"#f3f4f6", zIndex:0 }} />
            {steps.map(step => {
              const ss       = STEP_STATUS[step.status] || STEP_STATUS.pending;
              const isActive = activeStep === step.id;
              return (
                <button key={step.id} onClick={() => setActiveStep(step.id)}
                  className="kyc-step-btn"
...
function StepPanel({ step, onEdit, onUpload, onFieldChange, isEditing, onSave, onCancelEdit, allSteps }) {
  ...
  return (
    <div style={{ background:G.white, border:`1px solid ${G.border}`, borderRadius:14, overflow:"hidden" }}>
      ...
      {/* Header */}
      <div style={{ background:G.grad, padding:"18px 22px" }} className="kyc-panel-header">
        <div className="kyc-step-header" style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between" }}>
          <div>
            <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:6, flexWrap:"wrap" }}>
              <span style={{ fontSize:20 }}>{step.icon}</span>
              <p style={{ fontSize:"clamp(15px,3vw,18px)", fontWeight:800, color:G.white, margin:0, fontFamily:FONT }}>
          ...
          {step.status !== "under_review" && step.status !== "pending" && !isEditing && (
            <button onClick={onEdit}
              style={{ fontSize:12, fontWeight:700, color:G.white, background:"rgba(255,255,255,0.15)", border:"1px solid rgba(255,255,255,0.25)", borderRadius:8, padding:"7px 14px", cursor:"pointer", fontFamily:FONT, flexShrink:0 }}>
              ✏️ Edit
            </button>
          )}
        </div>
      ...
      <div style={{ padding:"22px" }} className="kyc-panel-body">
        ...
        {step.fields.length > 0 && (
          <div style={{ marginBottom:24 }}>
            ...
            <div className="kyc-fields-grid" style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
              {step.fields.map((f, i) => (
                <div key={i} style={{ gridColumn:f.type === "textarea" ? "1 / -1" : "auto" }}>
                  ...
                  {isEditing ? (
                    ...
                    <div style={{ background:G.bg, border:`1px solid #f3f4f6`, borderRadius:8, padding:"10px 12px", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
                      <p style={{ fontSize:13, color:G.text, fontWeight:500, wordBreak:"break-word" }}>{f.value || "—"}</p>
                      {f.masked && <span style={{ fontSize:10, color:G.muted, background:"#f3f4f6", padding:"2px 7px", borderRadius:99, flexShrink:0, marginLeft:8 }}>MASKED</span>}
                    </div>
                  )}
                </div>
              ))}
          ...
          <div>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
              {step.docs.map((doc, i) => {
                ...
                return (
                  <div key={i} className="kyc-doc-row" style={{ display:"flex", alignItems:"center", gap:14,
                    ...
                    <div style={{ flex:1, minWidth:0 }}>
                      <p style={{ fontSize:13, fontWeight:600, color:G.text, marginBottom:3, wordBreak:"break-word" }}>{doc.name}</p>
                    ...
                    <span style={{ display:"inline-flex", alignItems:"center", gap:5, fontSize:11, fontWeight:700, background:ds.bg, color:ds.text, padding:"3px 10px", borderRadius:99, flexShrink:0 }}>
                    ...
                    {(doc.status === "not_uploaded" || doc.status === "rejected") && (
                      <label style={{ fontSize:12, fontWeight:700, color:G.navy, background:G.blueBg, border:`1.5px solid ${G.blueBorder}`, borderRadius:8, padding:"6px 12px", cursor:"pointer", fontFamily:FONT, flexShrink:0 }}>
    ...
    <div style={{ background:G.white, border:`1px solid ${G.border}`, borderRadius:14, overflow:"hidden" }}>
      <div style={{ background:G.grad, padding:"18px 22px" }}>
        <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:4 }}>
          ...
          <p style={{ fontSize:"clamp(15px,3vw,18px)", fontWeight:800, color:G.white, margin:0, fontFamily:FONT }}>Step 5 — Final Review</p>
        </div>
              ...
              {steps.slice(0, 4).map(step => {
                ...
                return (
                  <div key={step.id}
                    ...
                    <span style={{ fontSize:11, fontWeight:700, color:ss.labelColor, background:ss.bg, padding:"3px 10px", borderRadius:99, flexShrink:0 }}>{ss.label}</span>
                  </div>
                );
              })}
            </div>
...
function Navbar({ page }) {
  return (
    <nav className="kyc-navbar" style={{ height:56, background:G.white, borderBottom:`1px solid ${G.border}`, display:"flex", alignItems:"center", padding:"0 28px", gap:12, position:"sticky", top:0, zIndex:1 }}>
      <img src="/weblance.jpeg" alt="Weblance" style={{ height:44, width:"auto" }} />
      <div className="kyc-navbar-mid" style={{ display:"flex", alignItems:"center", gap:12 }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:5, background:G.blueBg, border:`1px solid ${G.blueBorder}`, borderRadius:99, padding:"3px 10px" }}>
