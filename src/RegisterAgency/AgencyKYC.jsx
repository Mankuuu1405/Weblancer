...
export default function AgencyKYC() {
  ...
  const overallStatus = (() => {
    ...
    if (statuses.every(s => s === "verified"))      return { label:"Fully Verified",  color:G.greenDark, bg:G.greenBg,  border:G.greenBorder,  icon:"✅" };
          ...
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", paddingTop:16, paddingBottom:16, marginTop:14, borderTop:`1px solid #f3f4f6` }}>
            ...
            <div style={{ display:"flex", alignItems:"center", gap:12 }}>
              <div style={{ width:200, background:"#f3f4f6", borderRadius:99, height:7, overflow:"hidden" }}>
                <div style={{ width:`${(verifiedCount / steps.length) * 100}%`, height:"100%", background:`linear-gradient(90deg,${G.green},#86efac)`, borderRadius:99, transition:"width 0.3s" }} />
              </div>
            ...
            {steps.map(step => {
              ...
              return (
                <button key={step.id} onClick={() => setActiveStep(step.id)}
                  style={{ display:"flex", alignItems:"flex-start", gap:14, padding:"14px 16px", borderRadius:12, border:`1.5px solid ${isActive ? G.green : G.border}`, background:isActive ? G.greenBg : G.white, cursor:"pointer", marginBottom:6, textAlign:"left", position:"relative", zIndex:1, transition:"all 0.12s", fontFamily:FONT }}>
                  <div style={{ width:24, height:24, borderRadius:"50%", background:ss.bg, border:`2px solid ${ss.ring}`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:11, fontWeight:700, color:ss.iconColor, flexShrink:0, marginTop:1 }}>
                    {ss.icon || step.id}
                  </div>
                  <div style={{ flex:1 }}>
        ...
        <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between" }}>
          <div>
            <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:6 }}>
              ...
              <span style={{ fontSize:11, fontWeight:700, background:ss.bg, color:ss.labelColor, borderRadius:99, padding:"3px 10px" }}>
                {ss.label}
              </span>
            </div>
            <p style={{ fontSize:12, color:"rgba(255,255,255,0.45)" }}>{step.desc}</p>
          </div>
          {step.status !== "under_review" && step.status !== "pending" && !isEditing && (
            <button onClick={onEdit}
              style={{ fontSize:12, fontWeight:700, color:"#86efac", background:"rgba(34,197,94,0.12)", border:"1px solid rgba(34,197,94,0.25)", borderRadius:8, padding:"7px 14px", cursor:"pointer", fontFamily:FONT }}>
              ✏️ Edit
            </button>
          )}
        </div>
          ...
          <div style={{ marginBottom:24 }}>
            ...
            {isEditing && (
              <div style={{ display:"flex", gap:8, marginTop:14 }}>
                ...
                <button onClick={onSave}       style={{ padding:"8px 16px", fontSize:13, fontWeight:700, border:"none", background:G.green, color:G.white, borderRadius:8, cursor:"pointer", fontFamily:FONT }}>Save Changes</button>
              </div>
            )}
          </div>
        )}
          ...
          <div>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
              {step.docs.map((doc, i) => {
                ...
                return (
                  <div key={i} style={{ display:"flex", alignItems:"center", gap:14, border:`1px solid ${doc.status === "rejected" ? G.redBorder : G.border}`, borderRadius:12, padding:"14px 16px", background:doc.status === "rejected" ? G.redBg : doc.status === "verified" ? G.greenBg : G.white }}>
                    ...
                    {(doc.status === "not_uploaded" || doc.status === "rejected") && (
                      <label style={{ fontSize:12, fontWeight:700, color:G.greenDark, background:G.greenBg, border:`1.5px solid ${G.greenBorder}`, borderRadius:8, padding:"6px 12px", cursor:"pointer", fontFamily:FONT }}>
  ...
  return (
    <div style={{ background:G.white, border:`1px solid ${G.border}`, borderRadius:14, overflow:"hidden" }}>
      <div style={{ background:"linear-gradient(135deg,#14532d,#166534)", padding:"18px 22px" }}>
        ...
        <p style={{ fontSize:12, color:"rgba(255,255,255,0.45)" }}>Review all steps and submit for platform approval</p>
      </div>
      ...
      <div style={{ padding:"22px" }}>
        {submitted ? (
          <div style={{ textAlign:"center", padding:"40px 20px" }}>
            ...
            <div style={{ marginTop:20, background:G.greenBg, border:`1px solid ${G.greenBorder}`, borderRadius:12, padding:"14px 20px", maxWidth:320, margin:"20px auto 0" }}>
              <p style={{ fontSize:12, color:G.sub }}>Reference ID</p>
              <p style={{ fontSize:16, fontWeight:700, color:G.greenDark, fontFamily:FONT }}>KYC-2026-TEC-001</p>
            </div>
          </div>
        ) : (
          <>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:10, marginBottom:22 }}>
              {steps.slice(0, 4).map(step => {
                ...
                return (
                  <div key={step.id}
                    style={{ display:"flex", alignItems:"center", gap:14, border:`1.5px solid ${step.status === "verified" ? G.greenBorder : step.status === "rejected" ? G.redBorder : G.border}`, borderRadius:12, padding:"14px 16px", background:step.status === "verified" ? G.greenBg : step.status === "rejected" ? G.redBg : G.white }}>
                    <div style={{ width:28, height:28, borderRadius:"50%", background:ss.bg, border:`2px solid ${ss.ring}`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:13, fontWeight:700, color:ss.iconColor, flexShrink:0 }}>
                      {ss.icon || step.id}
                    </div>
                    <div style={{ flex:1 }}>
            ...
            <button onClick={() => allDone && setSubmitted(true)} disabled={!allDone}
              style={{ width:"100%", padding:"14px", fontSize:14, fontWeight:700, border:"none", borderRadius:10, background:allDone ? G.green : "#e5e7eb", color:allDone ? G.white : G.muted, cursor:allDone ? "pointer" : "not-allowed", fontFamily:FONT, transition:"background 0.15s" }}>
