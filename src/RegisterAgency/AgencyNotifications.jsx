...
//   kyc:        { icon:"🔔", color:"#16a34a", bg:G.greenBg,  border:G.greenBorder, label:"Platform"  },
...
//                 {unreadCount > 0 && (
//                   <span style={{ background: G.green, color: G.white, fontSize: 12, fontWeight: 700, padding:"3px 10px", borderRadius: 99 }}>
//                     {unreadCount} new
//                   </span>
//                 )}
//               </div>
...
//                           borderLeft: `3px solid ${n.read ? G.border : G.green}`,
//                           borderRadius: 12, padding:"14px 16px",
//                           display:"flex", alignItems:"flex-start", gap: 14,
//                           cursor:"pointer", transition:"all 0.12s" }}
//                         onMouseEnter={e => e.currentTarget.style.boxShadow = "0 2px 12px rgba(34,197,94,0.1)"}
...
//                             </div>
//                             <p style={{ fontSize: 11, color: G.muted, flexShrink: 0, marginLeft: 12 }}>{n.time}</p>
//                           </div>
...
//           padding:"2px 8px", borderRadius: 99, marginLeft: 2 }}>{unread}</span>
//       )}
      ...
      <header style={{ background: G.white, borderBottom: `1px solid ${G.border}` }}>
        <div style={{ maxWidth: 800, margin: "0 auto", padding: "0 28px" }}>
          <div style={{ display:"flex", alignItems:"flex-end", justifyContent:"space-between", padding:"20px 0 0" }}>
            <div>
              <div style={{ display:"flex", alignItems:"center", gap: 10 }}>
                <h1 style={{ fontSize: 22, fontWeight: 800, color: G.navy, margin: 0, letterSpacing:"-0.4px" }}>Notifications</h1>
                {unreadCount > 0 && (
            ...
            {TABS.map(t => {
              ...
              return (
                <button key={t.id} onClick={() => setTab(t.id)}
                  ...
                  {t.label}
                  {unread > 0 && (
                    <span style={{ width: 7, height: 7, borderRadius:"50%", background: G.blue }} />
                  )}
                  <span style={{ fontSize: 11, fontWeight: 700,
                        ...
                        <div style={{ flex: 1 }}>
                          <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", marginBottom: 3 }}>
                            <div style={{ display:"flex", alignItems:"center", gap: 8 }}>
                              <p style={{ fontSize: 13, fontWeight: n.read ? 600 : 700, color: G.text }}>{n.title}</p>
                              {!n.read && (
                                <span style={{ width: 7, height: 7, borderRadius:"50%", background: G.blue, flexShrink: 0 }} />
                              )}
                              <span style={{ fontSize: 10, fontWeight: 700,
                                background: tm.bg, color: tm.color, border:`1px solid ${tm.border}`,
                                padding:"1px 7px", borderRadius: 99 }}>{tm.label}</span>
