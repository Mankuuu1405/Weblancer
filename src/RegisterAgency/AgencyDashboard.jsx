  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="border border-gray-200 rounded-xl p-6">
        ...
        <div className="w-full bg-gray-100 rounded-full h-2"><div className="h-2 rounded-full" style={{width:"87%", background:"linear-gradient(135deg,#0D2855,#1B72C0)"}} /></div>
      </div>
      <div className="border border-gray-200 rounded-xl p-6">
        <h3 className="font-semibold text-gray-900 mb-5">Skill Coverage Breakdown</h3>
        <div className="flex flex-col gap-4">
          {skills.map((s, i) => (
            <div key={i}>
              ...
              <div className="w-full bg-gray-100 rounded-full h-1.5"><div className="h-1.5 rounded-full" style={{width:`${s.score}%`, background:"linear-gradient(135deg,#0D2855,#1B72C0)"}} /></div>
            </div>
          ))}
        </div>
      </div>
...
function ProjectsTab() {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {projects.map((p) => (
        <div key={p.id} className="border border-gray-200 rounded-xl overflow-hidden">
          <div onClick={() => navigate(`/agency/project/${p.id}`)} className="p-5 sm:p-6 cursor-pointer hover:bg-gray-50 transition-colors group">
            <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3 mb-3">
              <div>
                <h2 className="text-base font-bold text-gray-900 group-hover:text-green-700 transition-colors flex items-center gap-2">
                  {p.name}
              ...
              <div className="flex flex-wrap items-center gap-2">
                <span className="flex items-center gap-1.5 bg-green-500 text-white text-xs font-bold px-3 py-1 rounded-full">
                  <span className="w-1.5 h-1.5 rounded-full bg-white inline-block animate-pulse"></span>ACTIVE
                </span>
                <span className="border border-blue-200 bg-blue-50 text-blue-600 text-xs font-semibold px-3 py-1 rounded-full">{p.projectStatus}</span>
                <span className={`flex items-center gap-1.5 border text-xs font-semibold px-3 py-1 rounded-full ${p.riskColor}`}>
                  <span className={`w-1.5 h-1.5 rounded-full ${p.dot} inline-block`}></span>{p.risk}
                </span>
              </div>
            </div>
            <div className="flex flex-wrap items-center gap-4 text-sm text-gray-600 mb-2">
              <span>Budget: {p.budget}</span><span>{p.progress}% complete</span>
            </div>
            <div className="w-full bg-gray-100 rounded-full h-1.5">
              <div className="h-1.5 rounded-full transition-all" style={{ width:`${p.progress}%`, background:"linear-gradient(90deg,#0D2855,#1B72C0)" }} />
            </div>
            <div className="flex items-center justify-between mt-2">
              <div className="flex flex-wrap items-center gap-1.5 text-sm text-blue-500 mt-1">
                <span>Raj Kumar (Lead)</span><span className="text-gray-300">·</span>
          ...
          <div className="border-t border-gray-100 px-5 py-3 flex flex-wrap gap-2 bg-gray-50">
            <button onClick={(e) => { e.stopPropagation(); alert("Opening ProjectStream..."); }} className="border border-gray-200 bg-white text-gray-700 text-xs sm:text-sm px-3 sm:px-4 py-2 rounded-lg hover:bg-gray-100 transition-colors">Open ProjectStream</button>
            <button onClick={(e) => { e.stopPropagation(); alert(`Submitting milestone for ${p.name}...`); }} className="border border-gray-200 bg-white text-gray-700 text-xs sm:text-sm px-3 sm:px-4 py-2 rounded-lg hover:bg-gray-100 transition-colors">Submit M2</button>
            <button onClick={(e) => { e.stopPropagation(); alert(`Messaging client for ${p.name}...`); }} className="border border-gray-200 bg-white text-gray-700 text-xs sm:text-sm px-3 sm:px-4 py-2 rounded-lg hover:bg-gray-100 transition-colors">Message Client</button>
          </div>
        </div>
      ))}
    </div>
                ...
                <div className="flex flex-col gap-3">
                  {teamAllocation.map((m, i) => (
                    <div key={i} className="flex items-center justify-between border border-gray-200 rounded-lg px-4 py-3">
                      <div className="flex items-center gap-3">
                        <div className={`w-8 h-8 rounded-full ${m.bg} flex items-center justify-center text-xs font-bold ${m.color}`}>{m.initial}</div>
                        <div><div className="text-sm font-semibold text-gray-900">{m.name}</div><div className="text-xs text-gray-400">{m.role}</div></div>
                      </div>
                      <div className="flex items-center gap-3">
                        <span className="text-sm text-gray-500">{m.hours}</span>
                        <button className="text-sm text-blue-500 font-medium hover:text-blue-600 transition-colors">Edit</button>
...
function RevenueChart() {
  ...
  return (
    <div className="w-full overflow-x-auto">
      <svg viewBox={`0 0 ${W} ${H+40}`} className="w-full" style={{minWidth:320}}>
        <defs><linearGradient id="chartGrad" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#1B72C0" stopOpacity="0.15"/><stop offset="100%" stopColor="#1B72C0" stopOpacity="0"/></linearGradient></defs>
        <path d={areaPath} fill="url(#chartGrad)"/>
        <path d={linePath} fill="none" stroke="#1B72C0" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/>
        {data.map((d,i) => (<g key={i}><circle cx={xs[i]} cy={ys[i]} r="4" fill="#1B72C0"/><text x={xs[i]} y={ys[i]-12} textAnchor="middle" fontSize="11" fill="#6b7280" fontFamily="sans-serif">${(d.val/1000).toFixed(1)}K</text><text x={xs[i]} y={H+15} textAnchor="middle" fontSize="11" fill="#9ca3af" fontFamily="sans-serif">{d.month}</text></g>))}
      </svg>
    </div>
  );
}
        ...
        <div className="flex flex-col gap-4">
          {performance.map((p,i) => (
            <div key={i}>
              ...
              <div className="w-full bg-gray-100 rounded-full h-1.5">
                <div className="h-1.5 rounded-full" style={{width:`${p.pct}%`, background:"linear-gradient(135deg,#0D2855,#1B72C0)"}} />
              </div>
            </div>
          ))}
        </div>
...
export default function AgencyDashboard() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans">
      ...
      {/* ── Navbar — Weblance logo + theme ── */}
      <header className="bg-white border-b border-gray-200 px-4 sm:px-8 py-3 flex items-center justify-between"
        style={{ boxShadow: "0 1px 4px rgba(13,40,85,0.07)" }}>
        <img src="/weblance.jpeg" alt="Weblance" style={{ height: 54, width:155 }} />
        <div className="flex items-center gap-2 sm:gap-4">
          <span className="hidden sm:inline text-xs font-semibold px-3 py-1 rounded-full"
            style={{ border: "1px solid #bfdbfe", color: "#0D2855", background: "#eff6ff" }}>AGENCY</span>
          <span className="hidden sm:flex items-center gap-1 text-sm text-gray-600 cursor-pointer hover:text-blue-600">👁 Public Profile</span>
          <div className="w-8 h-8 rounded-full text-white flex items-center justify-center font-bold text-sm"
            style={{ background: "linear-gradient(135deg, #0D2855 0%, #1B72C0 100%)" }}>T</div>
        </div>
      </header>
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-8 py-6 sm:py-8">
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm mb-6">
          ...
          <div className="sm:hidden px-4 py-3 border-b border-gray-100">
            ...
            {mobileTabOpen && (
              <div className="absolute left-4 right-4 mt-1 bg-white border border-gray-200 rounded-xl shadow-lg z-30 py-1">
                {tabs.map(tab => (
                  <button key={tab} onClick={() => { setActiveTab(tab); setMobileTabOpen(false); }}
                    className={`w-full text-left px-4 py-3 text-sm transition-colors ${activeTab===tab?"bg-blue-50 font-semibold":"text-gray-700 hover:bg-gray-50"}`}
                    style={activeTab===tab ? { color: "#0D2855" } : {}}>
                    {tab} {tab==="Team" && <span className="ml-1 text-white text-xs rounded-full px-1.5 py-0.5" style={{ background:"linear-gradient(135deg,#0D2855,#1B72C0)" }}>5</span>}
                  </button>
                ))}
              </div>
            )}
          ...
          <div className="hidden sm:flex gap-0 border-b border-gray-100 px-4 overflow-x-auto">
            {tabs.map(tab => (
              <button key={tab} onClick={() => setActiveTab(tab)}
                className={`px-4 py-3 text-sm font-medium relative flex items-center gap-1.5 transition-colors whitespace-nowrap ${activeTab===tab?"border-b-2 text-gray-900":"text-gray-500 hover:text-gray-700"}`}
                style={activeTab===tab ? { borderBottomColor:"#1B72C0" } : { borderBottom:"2px solid transparent" }}>
                {tab}
                {tab==="Team" && <span className="text-white text-xs rounded-full w-4 h-4 flex items-center justify-center leading-none" style={{ background:"linear-gradient(135deg,#0D2855,#1B72C0)" }}>5</span>}
              </button>
            ))}
          </div>
          ...
          {activeTab==="AI Score" && (
            <div className="p-4 sm:p-6">
              <div className="flex flex-wrap gap-2 sm:gap-3 mb-6">
                {["Capability Score","Delivery Risk"].map(t => (
                  <button key={t} onClick={() => setAiSubTab(t)}
                    className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${aiSubTab===t?"text-white":"border border-gray-200 text-gray-600 hover:bg-gray-50"}`}
                    style={aiSubTab===t ? { background:"linear-gradient(135deg,#0D2855,#1B72C0)" } : {}}>
                    {t==="Capability Score"?"🤖":"🎯"} {t}
                  </button>
                ))}
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-2 flex flex-col gap-6">
                  ...
                  <div>
                    ...
                    <div className="flex flex-col gap-3">
                      {projects.map((p,i) => (
                        <div key={i} onClick={() => navigate(`/agency/project/${p.id}`)}
                          className="border border-gray-200 rounded-lg px-4 py-4 cursor-pointer hover:border-blue-300 hover:shadow-sm transition-all group">
                          <div className="flex flex-wrap items-center justify-between gap-2 mb-2">
                            <div>
                              <div className="font-semibold text-gray-900 text-sm group-hover:text-blue-700 transition-colors">{p.name}</div>
                              <div className="text-xs text-gray-400 mt-0.5">{p.client} · {p.budget}</div>
                            </div>
                            <div className="flex items-center gap-3">
                              <span className="text-sm text-gray-600 font-medium">{p.progress}%</span>
                          ...
                          <div className="w-full bg-gray-100 rounded-full h-1.5">
                            <div className="h-1.5 rounded-full" style={{width:`${p.progress}%`, background:"linear-gradient(90deg,#0D2855,#1B72C0)"}} />
                          </div>
                        </div>
                      ))}
                    </div>
