...
(() => {
  if (document.getElementById("wl-lp-responsive")) return;
  const s = document.createElement("style");
  s.id = "wl-lp-responsive";
  s.textContent = `
    ...
    .lp-hero-btns { display: flex; flex-direction: column; align-items: center; gap: 12px; }
    .lp-feat-grid { display: grid; gap: 16px; grid-template-columns: 1fr; }
    .lp-stats-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 24px; text-align: center; }
    .lp-check-grid { display: grid; gap: 16px; grid-template-columns: 1fr; }

    @media (min-width: 480px) {
      .lp-hero-btns { flex-direction: row; justify-content: center; }
    }
    @media (min-width: 640px) {
      .lp-feat-grid { grid-template-columns: 1fr 1fr; }
      .lp-check-grid { grid-template-columns: 1fr 1fr; }
    }
    @media (min-width: 1024px) {
      .lp-feat-grid { grid-template-columns: repeat(4, 1fr); }
      .lp-stats-grid { grid-template-columns: repeat(4, 1fr); }
    }

    /* Nav pill: shrink on mobile */
    @media (max-width: 480px) {
      .lp-sign-in { display: none; }
      .lp-join-btn { font-size: 12px; padding: 8px 16px; }
      .lp-nav { padding: 5px 8px 5px 12px; }
    }
...
const AgencyLandingPage = () => {
  ...
  const onJoinClick = () => navigate('/agency/onboarding');

  return (
    <div className="lp-root" style={{ minHeight:"100vh", background:"#FFFFFF" }}>

      {/* ══ FLOATING PILL NAVBAR ══ */}
      <div className="lp-nav-anim" style={{ position:"fixed", top:16, left:0, right:0, zIndex:50, display:"flex", justifyContent:"center", padding:"0 16px", pointerEvents:"none" }}>
      ...
      {/* ══ HERO ══ */}
      <section style={{ position:"relative", overflow:"hidden", textAlign:"center", padding:"60px 20px" }}>
        <NeuralBg />
        <div style={{
        ...
        <div style={{ maxWidth:700, margin:"0 auto", position:"relative", zIndex:10 }}>
          ...
          }}>AGENCY PARTNERS</span>

          <h2 style={{
            fontFamily:"'Poppins',sans-serif", fontSize:"clamp(28px,6vw,58px)",
          ...
          <p style={{ color:"#6B7280", fontSize:"clamp(14px,2.5vw,17px)", maxWidth:520, margin:"0 auto 36px" }}>
          ...
          <div className="lp-hero-btns">
            <button className="lp-btn-primary" onClick={onJoinClick}>Register Your Agency →</button>
            <button className="lp-btn-ghost">▶ Learn How It Works</button>
          </div>
        </div>
      </section>

      {/* ══ FEATURES ══ */}
      <section style={{ padding:"0 20px 60px", background:"#FFFFFF" }}>
        <div style={{ maxWidth:1100, margin:"0 auto" }} className="lp-feat-grid">
      ...
      {/* ══ WHY SECTION ══ */}
      <section style={{ padding:"0 20px 60px", background:"#FFFFFF" }}>
        <div style={{ maxWidth:860, margin:"0 auto", borderRadius:20, padding:"clamp(24px,5vw,40px)",
          background:"#fff", border:"1px solid #DBEAFE", boxShadow:"0 4px 24px rgba(27,114,192,0.07)" }}>
          <h3 style={{ fontFamily:"'Poppins',sans-serif", fontWeight:700, fontSize:"clamp(18px,3vw,22px)", color:"#0D2855", textAlign:"center", marginBottom:28 }}>
      ...
      <section style={{ padding:"0 20px 80px", background:"#FFFFFF" }}>
        <div style={{ maxWidth:900, margin:"0 auto" }} className="lp-stats-grid">
...
const Stat = ({ number, label }) => (
  <div className="lp-stat">
    <h4 style={{ fontSize:"clamp(22px,4vw,30px)", fontWeight:800, color:"#0D2855", fontFamily:"'Poppins',sans-serif" }}>{number}</h4>
