            ...
            {STAGES.map(s => {
              ...
              return (
                <button key={s} onClick={() => setStage(s)} style={{ display:"flex", alignItems:"center", gap:5, padding:"10px 12px", fontSize:13, fontWeight:active ? 700 : 500, color:active ? G.navy : G.sub, background:"none", border:"none", borderBottom:active ? `2px solid ${G.blue}` : "2px solid transparent", cursor:"pointer", marginBottom:-1, transition:"all 0.12s", fontFamily:FONT }}>
                  {S_LABEL[s]}
                  <span style={{ fontSize:11, fontWeight:700, background:active ? G.blue : "#f3f4f6", color:active ? G.white : G.muted, padding:"1px 7px", borderRadius:99, transition:"all 0.12s" }}>{cnt}</span>
                </button>
              );
            })}
            <div style={{ flex:1 }} />
  ...
  return (
    <div>
      <div style={{ background:G.grad, borderRadius:14, padding:"24px 28px", marginBottom:20, position:"relative", overflow:"hidden" }}>
        ...
        <div style={{ position:"absolute", right:60, bottom:-60, width:150, height:150, borderRadius:"50%", background:"rgba(255,255,255,0.04)", pointerEvents:"none" }} />
        <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between" }}>
          <div>
            <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:8 }}>
              <span style={{ fontSize:22 }}>🔍</span>
        ...
        <div style={{ display:"flex", gap:12, marginTop:20 }}>
          {[
            ...
            <div key={i} style={{ background:"rgba(255,255,255,0.10)", border:"1px solid rgba(255,255,255,0.12)", borderRadius:10, padding:"12px 16px", flex:1 }}>
              <p style={{ fontSize:10, color:"rgba(255,255,255,0.4)", fontWeight:700, textTransform:"uppercase", letterSpacing:"0.07em", marginBottom:6 }}>{s.label}</p>
              <p style={{ fontSize:20, fontWeight:800, color:G.white }}>
                {s.val} <span style={{ fontSize:12, fontWeight:400, color:"rgba(255,255,255,0.35)" }}>{s.unit}</span>
              </p>
          ...
          {proposals.filter(p => p.reviewDeadline).map(p => {
            ...
            return (
              <div key={p.id} style={{ display:"flex", alignItems:"center", gap:14 }}>
                ...
                <div style={{ background:urgent ? G.redBg : G.blueBg, border:`1px solid ${urgent ? G.redBorder : G.blueBorder}`, borderRadius:6, padding:"4px 10px" }}>
  ...
  return (
    <div onClick={onOpen} onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
      ...
      {p.milestones?.length > 0 && (
        <div style={{ display:"flex", gap:4, flexWrap:"wrap", marginBottom:14 }}>
          {p.milestones.map((m, i) => (
            <span key={i} style={{ fontSize:10, fontWeight:600, background:G.blueBg, color:G.navy, border:`1px solid ${G.blueBorder}`, padding:"2px 8px", borderRadius:6 }}>
              M{i + 1}: ₹{m.amount.toLocaleString("en-IN")}
            </span>
          ))}
        </div>
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-end", paddingTop:12, borderTop:`1px solid #f3f4f6` }}>
        <div>
          <p style={{ fontSize:10, color:G.muted, fontWeight:700, textTransform:"uppercase", letterSpacing:"0.06em", marginBottom:2 }}>Budget</p>
          <p style={{ fontSize:18, fontWeight:800, color:G.navy, letterSpacing:"-0.3px" }}>{fmt(p.amount, p.currency, p.pricingType)}</p>
        </div>
        <div style={{ textAlign:"right" }}>
          <p style={{ fontSize:10, color:G.muted, fontWeight:700, textTransform:"uppercase", letterSpacing:"0.06em", marginBottom:2 }}>In Review For</p>
          <p style={{ fontSize:12, color:G.sub, fontWeight:500 }}>{p.daysAgo} day{p.daysAgo !== 1 ? "s" : ""}</p>
    ...
    <Overlay onClose={onClose}>
      <div style={{ width:"100%", maxWidth:680, maxHeight:"88vh", background:G.white, borderRadius:16, display:"flex", flexDirection:"column", overflow:"hidden", boxShadow:"0 24px 64px rgba(0,0,0,0.2)" }}>
        ...
        <div style={{ background:G.grad, padding:"18px 22px 0" }}>
          ...
          <div style={{ display:"flex", gap:0 }}>
            {["overview","timeline","negotiation","notes"].map(t => (
              <button key={t} onClick={() => setTab(t)} style={{ padding:"9px 12px", fontSize:13, fontWeight:tab === t ? 700 : 400, color:tab === t ? G.white : "rgba(255,255,255,0.4)", background:"none", border:"none", borderBottom:tab === t ? `2px solid rgba(255,255,255,0.7)` : "2px solid transparent", cursor:"pointer", textTransform:"capitalize", marginBottom:-1, transition:"all 0.1s", fontFamily:FONT }}>
                {t}
              </button>
            ))}
          </div>
          ...
          {tab === "overview" && (
            <div style={{ display:"flex", flexDirection:"column", gap:18 }}>
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
                ...
                <Box style={{ background:G.blueBg, border:`1px solid ${G.blueBorder}` }}>
                  <Lbl>Budget</Lbl>
                  <p style={{ fontSize:22, fontWeight:800, color:G.navy, letterSpacing:"-0.5px" }}>{fmt(p.amount, p.currency, p.pricingType)}</p>
                  <p style={{ fontSize:12, color:G.sub, marginTop:4 }}>{p.pricingType} · {p.timeline}</p>
                  <p style={{ fontSize:12, color:G.sub }}>{p.paymentTerms}</p>
                </Box>
              </div>
                ...
                <>
                  ...
                  {p.negotiations.map(n => (
                    <div key={n.round} style={{ border:`1px solid ${G.border}`, borderRadius:12, padding:16, marginBottom:12 }}>
                      ...
                      <div style={{ display:"grid", gridTemplateColumns:"1fr auto 1fr", gap:10, alignItems:"center", marginBottom:10 }}>
                        <div style={{ background:G.blueBg, border:`1px solid ${G.blueBorder}`, borderRadius:9, padding:"9px 12px" }}>
                          <p style={{ fontSize:11, color:G.blue, fontWeight:600 }}>Our Ask</p>
                          <p style={{ fontSize:15, fontWeight:700, color:G.navy }}>₹{n.ask.toLocaleString("en-IN")}</p>
                        </div>
                        <svg width="14" height="14" fill="none" stroke="#d1d5db" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4"/></svg>
                        <div style={{ background:G.redBg, border:`1px solid ${G.redBorder}`, borderRadius:9, padding:"9px 12px" }}>
                          <p style={{ fontSize:11, color:G.red, fontWeight:600 }}>Their Offer</p>
    ...
    <Overlay onClose={onClose}>
      <div style={{ width:"100%", maxWidth:500, maxHeight:"92vh", background:G.white, borderRadius:16, display:"flex", flexDirection:"column", overflow:"hidden", boxShadow:"0 24px 64px rgba(0,0,0,0.2)" }}>
        ...
        <div style={{ background:G.grad, padding:"18px 22px 0" }}>
          ...
          <div style={{ display:"flex", gap:4, paddingBottom:18 }}>
            {Array.from({ length:T }).map((_, i) => (
              <div key={i} style={{ flex:1, height:3, borderRadius:99, background:i < step ? "rgba(255,255,255,0.8)" : "rgba(255,255,255,0.15)", transition:"background 0.3s" }} />
            ))}
          </div>
        </div>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
            ...
            {step === 3 && <>
              <FI label="Pricing Type" req>
                <div style={{ display:"flex", gap:8 }}>
                  {["Fixed","Hourly"].map(t => (
                    <button key={t} onClick={() => u("pricingType", t)}
                      style={{ flex:1, padding:9, fontSize:13, fontWeight:600, borderRadius:8, border:`1.5px solid ${f.pricingType === t ? G.blue : G.border}`, background:f.pricingType === t ? G.blueBg : G.white, color:f.pricingType === t ? G.navy : G.muted, cursor:"pointer", fontFamily:FONT }}>
                      {t === "Fixed" ? "💰 Fixed" : "⏱ Hourly"}
                    </button>
                  ))}
                </div>
            ...
            {step === 4 && <>
              ...
              <FI label="Attach Document">
                <label style={{ display:"flex", alignItems:"center", gap:10, border:`1.5px dashed ${G.border}`, borderRadius:9, padding:"11px 14px", cursor:"pointer" }}>
                  <span style={{ fontSize:18 }}>📎</span>
                  <span style={{ fontSize:13, color:f.doc ? G.navy : G.muted, fontWeight:f.doc ? 600 : 400 }}>{f.doc || "Upload PDF"}</span>
                  <input type="file" accept=".pdf,.doc,.docx" style={{ display:"none" }} onChange={e => { if (e.target.files[0]) u("doc", e.target.files[0].name); }} />
                </label>
              </FI>
            </>}
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
                <div style={{ background:G.blueBg, border:`1px solid ${G.blueBorder}`, borderRadius:12, padding:18 }}>
                  ...
                  <div style={{ marginTop:14, paddingTop:14, borderTop:`1px solid ${G.blueBorder}` }}>
                    <p style={{ fontSize:11, color:G.muted, fontWeight:500, marginBottom:3 }}>Budget</p>
                    <p style={{ fontSize:22, fontWeight:800, color:G.navy, letterSpacing:"-0.5px" }}>{f.currency}{Number(f.amount || 0).toLocaleString("en-IN")}{f.pricingType === "Hourly" ? "/hr" : ""}</p>
                  </div>
                </div>
                <FI label="Save as">
                  <div style={{ display:"flex", gap:8 }}>
                    {["draft","sent"].map(s => (
                      <button key={s} onClick={() => u("stage", s)}
                        style={{ flex:1, padding:9, fontSize:13, fontWeight:600, borderRadius:8, border:`1.5px solid ${f.stage === s ? G.blue : G.border}`, background:f.stage === s ? G.blueBg : G.white, color:f.stage === s ? G.navy : G.muted, cursor:"pointer", fontFamily:FONT }}>
                        {s === "draft" ? "💾 Draft" : "📤 Send Now"}
                      </button>
                    ))}
                  </div>
...
function Navbar({ onNew }) {
  return (
    <nav style={{ height:56, background:G.white, borderBottom:`1px solid ${G.border}`, display:"flex", alignItems:"center", padding:"0 24px", gap:12, position:"sticky", top:0, zIndex:40 }}>
      ...
      <button onClick={onNew} style={{ display:"flex", alignItems:"center", gap:6, background:G.grad, color:G.white, border:"none", borderRadius:8, padding:"7px 14px", fontSize:12, fontWeight:700, cursor:"pointer", fontFamily:FONT, marginLeft:4 }}>
        <svg width="12" height="12" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M12 4v16m8-8H4"/></svg>
        New Proposal
      </button>
    </nav>
