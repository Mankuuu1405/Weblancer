      ...
      <header style={{ background:G.white, borderBottom:`1px solid ${G.border}` }}>
        <div style={{ maxWidth:1240, margin:"0 auto", padding:"0 28px" }}>
          ...
          <div style={{ display:"flex", gap:0, marginTop:18, paddingTop:16, borderTop:`1px solid #f3f4f6`, paddingBottom:20 }}>
            {[
              { label:"Available to Withdraw", val:fmt(AVAILABLE_BALANCE), color:G.greenDark, big:true },
      ...
      <main style={{ maxWidth:1240, margin:"0 auto", padding:"24px 28px 64px" }}>
        ...
        <div style={{ background:G.yellowBg, border:`1px solid ${G.yellowBorder}`, borderRadius:10, padding:"10px 16px", marginBottom:20, display:"flex", alignItems:"center", gap:10 }}>
          ...
          <span style={{ fontSize:13, color:"#92400e", fontWeight:500 }}>
            ...
            <a href="/agency/kyc" style={{ color:G.greenDark, fontWeight:700 }}>Check KYC status →</a>
          </span>
        </div>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 370px", gap:20 }}>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:20 }}>
            ...
            {/* Success banner */}
            {showSuccess && (
              <div style={{ background:G.greenBg, border:`1.5px solid ${G.greenBorder}`, borderRadius:10, padding:"12px 16px", display:"flex", alignItems:"center", gap:10 }}>
                <span style={{ fontSize:18 }}>✅</span>
                <div>
                  <p style={{ fontSize:13, fontWeight:700, color:G.greenDark }}>Withdrawal request submitted!</p>
            ...
            <div style={{ background:G.white, border:`1px solid ${G.border}`, borderRadius:12, overflow:"hidden" }}>
              <div style={{ padding:"16px 20px", borderBottom:`1px solid #f3f4f6`, display:"flex", alignItems:"center", justifyContent:"space-between" }}>
                <div>
                  <p style={{ fontSize:14, fontWeight:700, color:G.text }}>Saved Bank Accounts</p>
                  <p style={{ fontSize:12, color:G.muted, marginTop:2 }}>Manage your payout destinations</p>
                </div>
                <button onClick={() => setShowNewAcc(true)}
                  style={{ display:"flex", alignItems:"center", gap:6, fontSize:12, fontWeight:700, color:G.greenDark, background:G.greenBg, border:`1.5px solid ${G.greenBorder}`, borderRadius:8, padding:"7px 14px", cursor:"pointer", fontFamily:FONT }}>
              ...
              {accounts.map((a, i) => (
                <div key={a.id} style={{ padding:"15px 20px", borderBottom:i < accounts.length - 1 ? `1px solid #f9fafb` : "none", display:"flex", alignItems:"center", gap:14 }}>
                  <div style={{ width:40, height:40, borderRadius:10, background:G.greenBg, border:`1px solid ${G.greenBorder}`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:20, flexShrink:0 }}>🏦</div>
                  <div style={{ flex:1 }}>
                    <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:3 }}>
                      <p style={{ fontSize:13, fontWeight:700, color:G.text }}>{a.nick}</p>
                      {a.primary && <span style={{ fontSize:10, fontWeight:700, background:G.greenBg, color:G.greenDark, border:`1px solid ${G.greenBorder}`, padding:"2px 8px", borderRadius:99 }}>PRIMARY</span>}
                    </div>
                    ...
                    {!a.primary && (
                      <>
                        <button onClick={() => setPrimary(a.id)}
                          style={{ fontSize:11, fontWeight:600, color:G.greenDark, background:G.greenBg, border:`1px solid ${G.greenBorder}`, borderRadius:7, padding:"5px 10px", cursor:"pointer", fontFamily:FONT }}>
                          Set Primary
                        </button>
            ...
            <div style={{ background:G.white, border:`1px solid ${G.border}`, borderRadius:12, overflow:"hidden" }}>
              <div style={{ padding:"16px 20px", borderBottom:`1px solid #f3f4f6` }}>
                <p style={{ fontSize:14, fontWeight:700, color:G.text }}>Payout History</p>
              </div>
              {/* Table head */}
                    ...
                    <div>
                      {p.status === "failed" && (
                        <div>
                          ...
                          <button style={{ fontSize:11, fontWeight:700, color:G.greenDark, background:G.greenBg, border:`1px solid ${G.greenBorder}`, borderRadius:6, padding:"3px 8px", cursor:"pointer", fontFamily:FONT }}>Retry</button>
                        </div>
                      )}
                    </div>
                  </div>
          ...
          <div>
            <div style={{ background:G.white, border:`1px solid ${G.border}`, borderRadius:12, overflow:"hidden", position:"sticky", top:72 }}>
              ...
              {/* Green header strip */}
              <div style={{ background:"linear-gradient(135deg,#14532d,#166534)", padding:"20px 22px" }}>
                ...
                <div style={{ marginTop:16, background:"rgba(255,255,255,0.07)", borderRadius:10, padding:"12px 16px" }}>
                  ...
                  <p style={{ fontSize:28, fontWeight:800, color:G.green, letterSpacing:"-0.5px" }}>{fmt(AVAILABLE_BALANCE)}</p>
                </div>
              </div>
              ...
              <div style={{ padding:"20px 22px", display:"flex", flexDirection:"column", gap:16 }}>
                ...
                <div>
                  ...
                  <div style={{ position:"relative" }}>
                    ...
                    <input
                      ...
                      style={{ width:"100%", paddingLeft:28, paddingRight:12, paddingTop:12, paddingBottom:12, fontSize:18, fontWeight:800, border:`1.5px solid ${amtNum > 0 && !isValid ? G.red : amtNum > 0 && isValid ? G.green : G.border}`, borderRadius:10, outline:"none", color:G.text, boxSizing:"border-box", fontFamily:FONT, transition:"border-color 0.15s" }}
                    />
                  </div>
                  ...
                  {isValid && <p style={{ fontSize:11, color:G.greenDark, marginTop:5, fontWeight:600 }}>✓ Valid amount</p>}
                  ...
                  <div style={{ display:"flex", gap:6, marginTop:8 }}>
                    {[50000, 100000, AVAILABLE_BALANCE].map(v => (
                      <button key={v} onClick={() => setAmount(String(v))}
                        ...
                        onMouseEnter={e => { e.currentTarget.style.borderColor = G.green; e.currentTarget.style.color = G.greenDark; }}
                  ...
                  <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
                    {accounts.map(a => (
                      <div key={a.id} onClick={() => setSelAcc(a.id)}
                        style={{ display:"flex", alignItems:"center", gap:10, padding:"10px 14px", borderRadius:10, border:`1.5px solid ${selAcc === a.id ? G.green : G.border}`, background:selAcc === a.id ? G.greenBg : G.white, cursor:"pointer", transition:"all 0.12s" }}>
                        <div style={{ width:18, height:18, borderRadius:"50%", border:`2px solid ${selAcc === a.id ? G.green : G.border}`, background:selAcc === a.id ? G.green : G.white, display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0, transition:"all 0.15s" }}>
                        ...
                        {a.primary && <span style={{ fontSize:10, fontWeight:700, background:G.greenBg, color:G.greenDark, border:`1px solid ${G.greenBorder}`, padding:"2px 7px", borderRadius:99 }}>PRIMARY</span>}
                      </div>
                    ))}
                  </div>
                </div>
                ...
                {/* Summary */}
                {isValid && (
                  <div style={{ background:G.greenBg, border:`1.5px solid ${G.greenBorder}`, borderRadius:10, padding:"12px 14px" }}>
                    {[
                ...
                <button onClick={handleWithdraw} disabled={!isValid}
                  style={{ background:isValid ? G.green : "#e5e7eb", color:isValid ? G.white : G.muted, border:"none", borderRadius:10, padding:"13px", fontSize:14, fontWeight:700, cursor:isValid ? "pointer" : "not-allowed", fontFamily:FONT, transition:"background 0.15s" }}>
...
function AddAccountForm({ onClose, onAdd }) {
  ...
  return (
    <div style={{ background:G.white, border:`1px solid ${G.border}`, borderRadius:12, overflow:"hidden" }}>
      <div style={{ background:"linear-gradient(135deg,#14532d,#166534)", padding:"15px 20px", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
      ...
      <div style={{ padding:"18px 20px", display:"flex", flexDirection:"column", gap:12 }}>
          ...
          <div style={{ display:"flex", gap:8 }}>
            {["Savings","Current"].map(t => (
              <button key={t} onClick={() => u("type", t)}
                style={{ flex:1, padding:"8px", fontSize:12, fontWeight:700, borderRadius:8, border:`1.5px solid ${f.type === t ? G.green : G.border}`, background:f.type === t ? G.greenBg : G.white, color:f.type === t ? G.greenDark : G.sub, cursor:"pointer", fontFamily:FONT, transition:"all 0.12s" }}>
                {t}
              </button>
            ))}
          </div>
        ...
        <div style={{ display:"flex", gap:8, marginTop:4 }}>
          ...
          <button onClick={() => ok && onAdd({ ...f, last4:f.accNum.slice(-4) })} disabled={!ok}
            style={{ flex:1, padding:"10px", fontSize:13, fontWeight:700, border:"none", background:ok ? G.green : "#e5e7eb", color:ok ? G.white : G.muted, borderRadius:9, cursor:ok ? "pointer" : "not-allowed", fontFamily:FONT }}>
            Save Account
          </button>
        </div>
      </div>
  ...
  return (
    <nav style={{ height:52, background:G.white, borderBottom:`1px solid ${G.border}`, display:"flex", alignItems:"center", padding:"0 28px", gap:12, position:"sticky", top:0, zIndex:40 }}>
      ...
      <div style={{ display:"flex", alignItems:"center", gap:5, background:G.greenBg, border:`1px solid ${G.greenBorder}`, borderRadius:99, padding:"3px 10px", marginLeft:4 }}>
        <svg width="10" height="10" fill={G.greenDark} viewBox="0 0 20 20"><path fillRule="evenodd" d="M5 9V7a5 5 0 0110 0v2a2 2 0 012 2v5a2 2 0 01-2 2H5a2 2 0 01-2-2v-5a2 2 0 012-2zm8-2v2H7V7a3 3 0 016 0z" clipRule="evenodd"/></svg>
