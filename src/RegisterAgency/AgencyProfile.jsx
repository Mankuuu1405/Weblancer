...
const portfolio = [
  {
    ...
    client: "ByteEats Co. (verified)",
    budget: "$42,000",
    duration: "5.5 months",
    tags: ["React Native", "Node", "PostgreSQL"],
    result: "10K+ active users at launch",
    rating: 5,
    review: "Exceptional work",
  },
  {
    title: "E-commerce Platform",
    client: "GlobalShop (verified)",
    budget: "$28,000",
    duration: "4 months",
    tags: ["React", "Django", "AWS"],
    result: "50% faster than old system",
    rating: 5,
    review: "Highly professional team",
  },
  {
    title: "Healthcare Dashboard",
    client: "MedTrack (verified)",
    budget: "$35,000",
    duration: "6 months",
    tags: ["Vue.js", "Python", "Docker"],
    result: "Used by 200+ clinics",
    rating: 4,
    review: "Great delivery, minor delays",
  },
];
...
const SkillBadge = ({ name, type }) => (
  <span className={`flex items-center gap-1 text-xs font-medium px-2.5 py-1 rounded-full ${
    type === "check" ? "bg-green-50 text-green-700 border border-green-100" : "bg-blue-50 text-blue-600 border border-blue-100"
  }`}>
    {name}
    {type === "check" ? " ✅" : " 🔵"}
  </span>
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans">
      ...
      <div className="bg-white border-b border-gray-200 px-8 py-4 flex justify-between items-center">
        ...
        <button onClick={() => navigate(-1)} className="text-sm font-semibold text-gray-600 hover:text-gray-900 flex items-center gap-1">
          ← Back
        </button>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-6 py-8 flex flex-col gap-5">
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-7 shadow-sm">
          <div className="flex items-start gap-5 mb-5">
            <div className="w-20 h-20 rounded-xl bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white text-2xl font-extrabold flex-shrink-0">
              TV
            </div>
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <button className="flex items-center gap-2 bg-white hover:bg-gray-50 text-gray-700 text-sm font-semibold px-5 py-2.5 rounded-lg border border-gray-200 transition-colors">
          ...
          <div className="flex border-b border-gray-100 overflow-x-auto">
            {TABS.map((tab) => (
              <button
                ...
                className={`px-6 py-3.5 text-sm font-semibold transition-colors border-b-2 -mb-px whitespace-nowrap ${
                  activeTab === tab
            ...
            <div className="bg-white border border-gray-200 rounded-2xl p-7 shadow-sm">
              ...
              <div className="flex flex-wrap gap-2">
                {agency.techStack.map((tech) => (
                  <span key={tech} className="px-4 py-1.5 rounded-full border border-gray-200 text-sm font-medium text-gray-700 hover:border-blue-300 hover:text-blue-600 transition-colors cursor-default">
                    {tech}
                  </span>
                ))}
              </div>
              ...
              <div className="bg-white border border-gray-200 rounded-2xl p-7 shadow-sm">
                ...
                <div className="flex flex-wrap gap-2">
                  {agency.industries.map((ind) => (
                    <span key={ind} className="px-3 py-1.5 rounded-full bg-blue-50 border border-blue-100 text-xs font-semibold text-blue-600">
                      {ind}
                    </span>
                  ))}
                </div>
                ...
                <div className="flex flex-col gap-3">
                  {agency.certifications.map((cert) => (
                    <div key={cert} className="flex items-center gap-2 text-sm text-gray-600">
                      <BsAward className="text-blue-500 text-base" /> {cert}
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div className="flex flex-col gap-4">
            {portfolio.map((p, i) => (
              <div key={i} className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
                ...
                <div className="flex flex-wrap gap-2 mb-3">
                  {p.tags.map((t) => (
                    <span key={t} className="px-2.5 py-1 rounded-full bg-blue-50 border border-blue-100 text-xs font-medium text-blue-600">{t}</span>
                  ))}
                </div>
        ...
        {activeTab === "Team" && (
          <div className="flex flex-col gap-3">
            {team.map((member, i) => (
              <div key={i} className="bg-white border border-gray-200 rounded-2xl px-6 py-4 shadow-sm flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {member.skills.map((sk, j) => (
                      <SkillBadge key={j} name={sk.name} type={sk.type} />
                    ))}
                  </div>
                </div>
                <span className="text-xs font-semibold text-gray-400 whitespace-nowrap">{member.hours}</span>
              </div>
            ))}
          </div>
        )}
