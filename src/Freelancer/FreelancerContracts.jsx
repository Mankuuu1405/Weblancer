...
const DL_STYLE = {
  ...
  approved:    { bg:G.greenBg, text:G.greenDark,   label:"Approved ✅" },
};
      ...
      <header style={{ background:G.white, borderBottom:`1px solid ${G.border}` }}>
        <div style={{ maxWidth:1160, margin:"0 auto", padding:"0 28px" }}>
          ...
          <div style={{ display:"flex", gap:0, marginTop:18, paddingTop:18, borderTop:"1px solid #f3f4f6" }}>
            {[
              { label:"Active",              val:stats.active,       accent:G.greenDark  },
      ...
      <main style={{ maxWidth:1160, margin:"0 auto", padding:"24px 28px 64px" }}>
        <div style={{ display:"grid", gridTemplateColumns:"1fr 280px", gap:20 }}>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
            ...
            {/* Earnings summary — dark green gradient */}
            <div style={{ background:"linear-gradient(135deg,#14532d,#166534)", borderRadius:14, padding:"18px 20px", position:"relative", overflow:"hidden" }}>
              ...
              <p style={{ fontSize:11, color:"rgba(255,255,255,0.35)", fontWeight:700, textTransform:"uppercase", letterSpacing:"0.07em", marginBottom:14 }}>Earnings Summary</p>
              {[
                ["Released",    fmt(stats.earned),                                             "#86efac"               ],
                ["In Escrow",   fmt(CONTRACTS.reduce((s, c) => s + c.escrow, 0)),              "#93c5fd"               ],
                ["On Hold",     fmt(CONTRACTS.reduce((s, c) => s + c.onHold, 0)),              "#fca5a5"               ],
                ["Total Value", fmt(stats.total),                                              "rgba(255,255,255,0.8)" ],
              ].map(([k, v, color]) => (
              ...
              <a href="/freelancer/earnings" style={{ display:"block", textAlign:"center", marginTop:12, fontSize:12, fontWeight:700, color:"#86efac", textDecoration:"none" }}>
                View Full Earnings →
              </a>
            </div>
              ...
              {allDeadlines.slice(0, 4).map((m, i) => {
                ...
                return (
                  <div key={i} style={{ display:"flex", gap:10, marginBottom:12, alignItems:"flex-start" }}>
                    ...
                    <span style={{ fontSize:12, fontWeight:700, color:G.greenDark }}>{fmt(m.amount)}</span>
                  </div>
                );
              })}
            </div>
              ...
              {[
                ...
                <a key={i} href={a.href}
                  style={{ display:"flex", alignItems:"center", gap:9, padding:"9px 12px",
                    fontSize:13, fontWeight:600, color:G.greenDark,
                    background:G.greenBg, border:`1px solid ${G.greenBorder}`,
                    borderRadius:9, textDecoration:"none", marginBottom:8, transition:"all 0.12s" }}
                  onMouseEnter={e => e.currentTarget.style.background = G.greenBorder}
...
function ContractCard({ contract:c, onOpen, onRaiseInvoice }) {
  ...
  return (
    <div style={{ background:G.white,
      border:`1.5px solid ${c.stage === "disputed" ? "#ef4444" : c.stage === "on_hold" ? "#f97316" : hov ? G.greenBorder : G.border}`,
      ...
      <div style={{ padding:"18px 20px" }}>
          ...
          <div style={{ flex:1 }}>
            <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:4 }}>
              ...
              <span style={{ display:"inline-flex", alignItems:"center", gap:4, fontSize:11, fontWeight:700,
                background:ss.bg, color:ss.text, padding:"3px 9px", borderRadius:99 }}>
                <span style={{ width:5, height:5, borderRadius:"50%", background:ss.dot }} />
                {ss.label}
              </span>
            </div>
            <div style={{ display:"flex", alignItems:"center", gap:8 }}>
              <div style={{ width:24, height:24, borderRadius:7, background:c.avatarColor + "20",
                display:"flex", alignItems:"center", justifyContent:"center",
                fontSize:10, fontWeight:700, color:c.avatarColor }}>{c.avatar}</div>
              <p style={{ fontSize:12, color:G.sub }}>{c.client} · {c.company}</p>
              {c.clientVerified && (
                <span style={{ fontSize:10, fontWeight:700, background:G.greenBg, color:G.greenDark, border:`1px solid ${G.greenBorder}`, padding:"1px 7px", borderRadius:99 }}>✓ Verified</span>
              )}
            </div>
          </div>
          <div style={{ textAlign:"right", flexShrink:0, marginLeft:12 }}>
            <p style={{ fontSize:20, fontWeight:800, color:G.greenDark }}>{fmt(c.total)}</p>
        ...
        <div style={{ marginBottom:14 }}>
          ...
          <div style={{ background:"#f3f4f6", borderRadius:99, height:8, overflow:"hidden", display:"flex" }}>
            ...
            <div style={{ width:`${escrowPct}%`, height:"100%", background:"#3b82f6", transition:"width 0.3s" }} />
          </div>
          <div style={{ display:"flex", gap:12, marginTop:5 }}>
            ...
            <span style={{ fontSize:10, color:G.muted, display:"flex", alignItems:"center", gap:4 }}><span style={{ width:7, height:7, borderRadius:"50%", background:"#3b82f6" }} /> Escrow</span>
        ...
        <div style={{ display:"flex", gap:16, marginBottom:14 }}>
          ...
          {activeMilestone && (
            <div>
              ...
              <p style={{ fontSize:12, color:G.greenDark, fontWeight:600 }}>{activeMilestone.name} · {activeMilestone.due}</p>
            </div>
          )}
          <div>
        ...
        <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
          <button onClick={onOpen} style={{ fontSize:12, fontWeight:700, color:G.greenDark, background:G.greenBg, border:`1px solid ${G.greenBorder}`, borderRadius:8, padding:"7px 14px", cursor:"pointer", fontFamily:FONT }}>
            View Details →
          </button>
          {canInvoice && (
            <button onClick={onRaiseInvoice} style={{ fontSize:12, fontWeight:700, color:G.white, background:G.green, border:"none", borderRadius:8, padding:"7px 14px", cursor:"pointer", fontFamily:FONT }}>
              💰 Raise Invoice
            </button>
          )}
          <a href="/project-stream" style={{ fontSize:12, fontWeight:700, color:"#4f46e5", background:"#eef2ff", border:"1px solid #c7d2fe", borderRadius:8, padding:"7px 12px", textDecoration:"none" }}>
            💬 Message
          </a>
          {c.dispute && (
            <button onClick={onOpen} style={{ fontSize:12, fontWeight:700, color:"#dc2626", background:"#fef2f2", border:"1px solid #fecaca", borderRadius:8, padding:"7px 14px", cursor:"pointer", fontFamily:FONT }}>
              🔴 View Dispute
            </button>
          )}
        </div>
...
function DetailModal({ contract:c, onClose, onRaiseInvoice }) {
  ...
  return (
    <div onClick={onClose} style={{ position:"fixed", inset:0, zIndex:50, background:"rgba(17,24,39,0.45)", backdropFilter:"blur(4px)", display:"flex", alignItems:"center", justifyContent:"center", padding:20 }}>
      <div onClick={e => e.stopPropagation()} style={{ width:"100%", maxWidth:720, maxHeight:"90vh", background:G.white, borderRadius:18, display:"flex", flexDirection:"column", overflow:"hidden", boxShadow:"0 32px 80px rgba(0,0,0,0.18)" }}>
        ...
        <div style={{ background:"linear-gradient(135deg,#14532d,#166534)", padding:"20px 24px 0" }}>
          ...
          <div style={{ display:"flex", gap:20, paddingBottom:14 }}>
            {[["Total",fmt(c.total),"#86efac"],["Earned",fmt(c.earned),"rgba(255,255,255,0.7)"],["Escrow",fmt(c.escrow),"#93c5fd"],["Timeline",c.start+" → "+c.end,"rgba(255,255,255,0.5)"]].map(([k, v, col]) => (
              <div key={k}>
          ...
          {tab === "overview" && (
            <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
                {[
                  ["Contract ID",        c.id],
                  ["Service Type",       c.service],
                  ["Total Value",        fmt(c.total)],
                  ["Start Date",         c.start],
                  ["End Date",           c.end],
                  ["Client",             c.client + " · " + c.company],
                  ["Freelancer Signed",  c.signatures.freelancer ? "✅ Yes" : "⏳ Pending"],
                  ["Client Signed",      c.signatures.client     ? "✅ Yes" : "⏳ Pending"],
                ].map(([k, v]) => (
                  <div key={k} style={{ padding:"11px 14px", border:`1px solid ${G.border}`, borderRadius:9 }}>
          ...
          {tab === "milestones" && (
            <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
              {c.milestones.map(m => {
                ...
                return (
                  <div key={m.id} style={{ border:`1px solid ${m.status === "disputed" ? "#fecaca" : m.status === "released" ? G.greenBorder : G.border}`,
                    borderRadius:12, padding:"14px 16px",
                    background:m.status === "released" ? G.greenBg : m.status === "disputed" ? "#fef2f2" : G.white }}>
                    <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between" }}>
                      ...
                      <div style={{ textAlign:"right", flexShrink:0, marginLeft:12 }}>
                        <p style={{ fontSize:16, fontWeight:800, color:G.greenDark }}>{fmt(m.amount)}</p>
                    ...
                    {(canInvoiceThis || m.status === "in_escrow") && (
                      <div style={{ marginTop:10, display:"flex", gap:8 }}>
                        {canInvoiceThis && (
                          <button onClick={onRaiseInvoice} style={{ fontSize:12, fontWeight:700, background:G.green, color:G.white, border:"none", borderRadius:8, padding:"7px 14px", cursor:"pointer", fontFamily:FONT }}>
                            💰 Raise Invoice
                          </button>
                        )}
                        {m.status === "in_escrow" && (
                          <button style={{ fontSize:12, fontWeight:700, color:"#1d4ed8", background:"#eff6ff", border:"1px solid #bfdbfe", borderRadius:8, padding:"7px 14px", cursor:"pointer", fontFamily:FONT }}>
                            ✓ Mark as Complete
                          </button>
                        )}
                        <button style={{ fontSize:12, fontWeight:600, color:G.sub, background:"#f3f4f6", border:"none", borderRadius:8, padding:"7px 14px", cursor:"pointer", fontFamily:FONT }}>
                          Request Extension
                        </button>
                      </div>
                    )}
          ...
          {tab === "deliverables" && (
            <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
              {c.deliverables.map((d, i) => {
                ...
                return (
                  <div key={i} style={{ border:`1px solid ${G.border}`, borderRadius:12, padding:"14px 16px", borderLeft:`3px solid ${ds.text}` }}>
                    ...
                    {(d.status === "in_progress" || d.status === "not_started") && (
                      <button style={{ marginTop:8, fontSize:12, fontWeight:700, color:G.greenDark, background:G.greenBg, border:`1px solid ${G.greenBorder}`, borderRadius:8, padding:"6px 12px", cursor:"pointer", fontFamily:FONT }}>
                        📤 Submit Deliverable
                      </button>
                    )}
                  </div>
...
function InvoiceModal({ contract:c, onClose }) {
  ...
  return (
    <div onClick={onClose} style={{ position:"fixed", inset:0, zIndex:50, background:"rgba(17,24,39,0.45)", backdropFilter:"blur(4px)", display:"flex", alignItems:"center", justifyContent:"center", padding:20 }}>
      <div onClick={e => e.stopPropagation()} style={{ width:"100%", maxWidth:500, background:G.white, borderRadius:18, overflow:"hidden", boxShadow:"0 32px 80px rgba(0,0,0,0.18)" }}>
        ...
        <div style={{ padding:"20px 22px" }}>
          <div style={{ background:G.greenBg, border:`1px solid ${G.greenBorder}`, borderRadius:12, padding:"16px", marginBottom:16 }}>
            {[
              ...
              ["Gross Amount", fmt(milestone.amount)],
              ["Platform Fee", "-" + fmt(Math.round(milestone.amount * 0.06)) + " (6%)"],
              ["You Receive",  fmt(net)],
            ].map(([k, v], i) => (
              <div key={k} style={{ display:"flex", justifyContent:"space-between", marginBottom:i < 3 ? 8 : 0 }}>
                <span style={{ fontSize:12, color:G.sub }}>{k}</span>
                <span style={{ fontSize:12, fontWeight:i === 3 ? 800 : 600, color:i === 3 ? G.greenDark : i === 2 ? "#dc2626" : G.text }}>{v}</span>
              </div>
            ))}
          </div>
          <div style={{ marginBottom:14 }}>
            ...
            <textarea value={workSummary} onChange={e => setWorkSummary(e.target.value)} placeholder="Describe what you completed for this milestone…" rows={4}
              style={{ width:"100%", fontSize:13, border:`1.5px solid ${G.border}`, borderRadius:9, padding:"10px 12px", outline:"none", resize:"none", fontFamily:FONT, boxSizing:"border-box" }} />
          </div>
          <div style={{ border:`1.5px dashed ${G.greenBorder}`, borderRadius:10, padding:"16px", textAlign:"center", marginBottom:16, cursor:"pointer", background:G.greenBg }}>
            <p style={{ fontSize:13, color:G.greenDark, fontWeight:600 }}>📎 Attach proof of work (optional)</p>
            <p style={{ fontSize:11, color:G.muted, marginTop:3 }}>Screenshots, files, links</p>
          </div>
          <div style={{ display:"flex", gap:8 }}>
            <button onClick={onClose} style={{ flex:1, padding:"11px", fontSize:13, fontWeight:600, border:`1px solid ${G.border}`, background:G.white, color:G.sub, borderRadius:9, cursor:"pointer", fontFamily:FONT }}>Cancel</button>
            <button style={{ flex:2, padding:"11px", fontSize:13, fontWeight:700, background:G.green, color:G.white, border:"none", borderRadius:9, cursor:"pointer", fontFamily:FONT }}>
...
function NewContractModal({ onClose }) {
  ...
  return (
    <div onClick={onClose} style={{ position:"fixed", inset:0, zIndex:50, background:"rgba(17,24,39,0.45)", backdropFilter:"blur(4px)", display:"flex", alignItems:"center", justifyContent:"center", padding:20 }}>
      <div onClick={e => e.stopPropagation()} style={{ width:"100%", maxWidth:580, maxHeight:"90vh", background:G.white, borderRadius:18, display:"flex", flexDirection:"column", overflow:"hidden", boxShadow:"0 32px 80px rgba(0,0,0,0.18)" }}>
        ...
        <div style={{ flex:1, overflowY:"auto", padding:"22px 24px" }}>
            ...
            <div>
              ...
              {milestones.map((m, i) => (
                <div key={i} style={{ display:"grid", gridTemplateColumns:"1fr 110px 110px 32px", gap:8, marginBottom:10 }}>
                  ...
                  {milestones.length > 1 && (
                    <button onClick={() => setMilestones(prev => prev.filter((_, j) => j !== i))} style={{ fontSize:16, background:"#fef2f2", border:"1px solid #fecaca", borderRadius:7, cursor:"pointer", color:"#dc2626" }}>✕</button>
                  )}
                </div>
              ))}
              {milestones.length < 8 && (
                <button onClick={() => setMilestones(p => [...p, { name:"", amount:"", due:"" }])}
              ...
              <div style={{ marginTop:14, padding:"12px 14px", background:G.greenBg, border:`1px solid ${G.greenBorder}`, borderRadius:10 }}>
                <div style={{ display:"flex", justifyContent:"space-between" }}>
                  <span style={{ fontSize:12, color:G.sub }}>Total Contract Value</span>
                  <span style={{ fontSize:14, fontWeight:700, color:G.greenDark }}>{fmt(milestones.reduce((s, m) => s + (Number(m.amount) || 0), 0))}</span>
                </div>
          ...
          {step === 3 && (
            <div>
              ...
              <div style={{ border:`1px solid ${G.greenBorder}`, background:G.greenBg, borderRadius:10, padding:"14px", marginBottom:14 }}>
                <p style={{ fontSize:12, fontWeight:700, color:G.greenDark, marginBottom:8 }}>Contract Summary</p>
        ...
        <div style={{ padding:"14px 24px", borderTop:"1px solid #f3f4f6", display:"flex", justifyContent:"space-between", background:G.bg }}>
          ...
          {step < STEPS.length
            ...
            : <button style={{ fontSize:13, fontWeight:700, padding:"9px 22px", border:"none", background:G.green, color:G.white, borderRadius:9, cursor:"pointer", fontFamily:FONT }}>📋 Send to Client</button>
          }
        </div>
      </div>
    </div>
