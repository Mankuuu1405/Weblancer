...
//               type="button"
//               onClick={() => { onChange(opt); setOpen(false); }}
//               className={`w-full text-left px-4 py-2.5 text-sm flex items-center gap-2 hover:bg-green-50 transition
//                 ${opt === value ? "wbl-text-blue font-semibold bg-blue-50" : "text-gray-700"}`}
//             >
//               {showCheck && opt === value && (
...
//                       <div key={m.label}
//                         className={`rounded-lg py-2.5 px-1 transition
//                           ${m.active ? "bg-blue-50 border border-green-200" : ""}`}>
//                         <p className={`text-base font-bold ${m.active ? "text-blue-700" : "text-gray-800"}`}>
//                           {sym}{m.val}/hr
//                         </p>
//                         <p className="text-xs text-gray-500 mt-0.5">{m.label}</p>
...
//                   <div className="mt-3 relative h-2 bg-gradient-to-r from-gray-200 via-blue-300 to-gray-200 rounded-full">
//                     <div
//                       className="absolute top-1/2 -translate-y-1/2 w-3 h-3 rounded-full wbl-bg border-2 border-white shadow transition-all duration-300"
//                       style={{
//                         left: `${Math.min(98, Math.max(2, ((hourlyRate - market.low) / (market.high - market.low)) * 100))}%`,
//                         transform: "translateX(-50%) translateY(-50%)"
//                       }}
...
//                     onChange={e => setMinBudget(e.target.value)}
//                     placeholder="e.g. 500"
//                     className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm outline-none bg-gray-50 focus:border-green-400 focus:ring-2 focus:ring-green-100 transition"
//                   />
//                   <p className="text-xs text-gray-400 mt-1.5">Projects below this won't be shown to you</p>
//                 </div>
...
//                       value={duration}
//                       onChange={e => setDuration(e.target.value)}
//                       className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm outline-none bg-gray-50 appearance-none focus:border-green-400 focus:ring-2 focus:ring-green-100 transition"
//                     >
//                       {durationOptions.map(d => <option key={d}>{d}</option>)}
//                     </select>
//                     <svg className="absolute right-3 top-3 w-4 h-4 text-gray-400 pointer-events-none" fill="none" stroke="currentColor" viewBox="0 0 24 24">
