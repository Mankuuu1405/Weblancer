...
export default function Step4_Profile({ onNext, onBack, currentStep = 4, totalSteps = 12 }) {
  ...
  const strength      = calcStrength();
  const strengthColor = strength < 40 ? "#ef4444" : strength < 70 ? "#eab308" : "#2AB836";
  const strengthLabel = strength < 40 ? "Weak" : strength < 70 ? "Good" : "Strong";
  ...
  const inputClass = (field) =>
    `w-full p-3 border rounded-xl text-sm outline-none transition focus:ring-2 focus:ring-blue-100
    ${errors[field] ? "border-red-400 bg-red-50" : "border-gray-200 bg-gray-50 focus:border-blue-500"}`;
          ...
          <div className="w-full xl:flex-1 min-w-0">
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 sm:p-8">
              ...
              <form onSubmit={handleSubmit} noValidate className="space-y-6">
                ...
                <div>
                  <div className="flex items-center gap-5">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-500 mt-0.5">Min 400×400px, JPG or PNG</p>
                      <button type="button" onClick={() => fileRef.current.click()}
                        className="mt-2 text-xs font-medium hover:underline" style={{ color: "#1960d2" }}>
                        {photo ? "Change photo" : "Upload photo"}
                      </button>
                    </div>
                  </div>
                ...
                <div>
                  <label className="block text-sm font-semibold mb-3">
                    Availability: <span style={{ color: "#1960d2" }}>{availability} hrs/week</span>
                  </label>
                  <input type="range" min={5} max={60} step={5} value={availability}
                    onChange={e => setAvailability(Number(e.target.value))}
                    className="w-full h-2 rounded-lg appearance-none cursor-pointer bg-gray-200"
                    style={{
                      background: `linear-gradient(to right, #1960d2 0%, #1960d2 ${((availability - 5) / 55) * 100}%, #e5e7eb ${((availability - 5) / 55) * 100}%, #e5e7eb 100%)`
                    }}
                  />
                  <div className="flex justify-between text-xs text-gray-400 mt-1.5">
                    <span>Part-time</span>
                  ...
                  <p className="text-xs font-medium mt-1" style={{ color: "#1960d2" }}>{getAvailLabel(availability)}</p>
                </div>
                ...
                <div>
                  ...
                  <div className="space-y-2.5">
                    {/* GitHub */}
                    <div className="flex items-center gap-3 border border-gray-200 rounded-xl px-3 py-2.5 bg-gray-50 focus-within:border-blue-500 focus-within:ring-2 focus-within:ring-blue-100 transition">
                      <svg className="w-5 h-5 text-gray-500 flex-shrink-0" fill="currentColor" viewBox="0 0 24 24">
                        <path d="M12 .5C5.73.5.5 5.73.5 12c0 5.08 3.29 9.38 7.86 10.9.58.1.79-.25.79-.56v-2c-3.2.7-3.87-1.54-3.87-1.54-.52-1.33-1.28-1.69-1.28-1.69-1.04-.71.08-.7.08-.7 1.15.08 1.76 1.18 1.76 1.18 1.02 1.75 2.68 1.24 3.33.95.1-.74.4-1.24.73-1.53-2.55-.29-5.23-1.28-5.23-5.7 0-1.26.45-2.29 1.18-3.09-.12-.29-.51-1.46.11-3.05 0 0 .97-.31 3.17 1.18a11.05 11.05 0 012.89-.39c.98 0 1.97.13 2.89.39 2.2-1.49 3.16-1.18 3.16-1.18.63 1.59.23 2.76.11 3.05.74.8 1.18 1.83 1.18 3.09 0 4.43-2.69 5.41-5.25 5.69.41.36.78 1.06.78 2.14v3.17c0 .31.21.67.8.56A10.51 10.51 0 0023.5 12C23.5 5.73 18.27.5 12 .5z"/>
                      </svg>
                      <input type="url" value={github} placeholder="https://github.com/username"
                        onChange={e => setGithub(e.target.value)}
                        className="flex-1 bg-transparent text-sm outline-none text-gray-700 placeholder-gray-400"/>
                      <span className="text-xs font-bold whitespace-nowrap" style={{ color: "#2AB836" }}>+15 pts</span>
                    </div>
                    ...
                    <div className="flex items-center gap-3 border border-gray-200 rounded-xl px-3 py-2.5 bg-gray-50 focus-within:border-blue-500 focus-within:ring-2 focus-within:ring-blue-100 transition">
                      ...
                      <input type="url" value={portfolio} placeholder="https://yourportfolio.com"
                        onChange={e => setPortfolio(e.target.value)}
                        className="flex-1 bg-transparent text-sm outline-none text-gray-700 placeholder-gray-400"/>
                      <span className="text-xs font-bold whitespace-nowrap" style={{ color: "#2AB836" }}>+10 pts</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="border border-gray-200 rounded-xl p-4">
                  ...
                  <div className="w-full bg-gray-100 rounded-full h-2.5">
                    <div className="h-2.5 rounded-full transition-all duration-500"
                      style={{ width: `${strength}%`, background: strength >= 70 ? "linear-gradient(90deg,#1960d2,#2AB836)" : strengthColor }}/>
                  </div>
                  <p className="text-xs mt-1.5 font-medium" style={{ color: strength === 0 ? "#9ca3af" : strengthColor }}>
                    {strength === 0 ? "Fill in your profile to increase strength" : `${strengthLabel} profile`}
                  </p>
                </div>
          ...
          <div className="w-full xl:w-72 flex-shrink-0">
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 xl:sticky xl:top-6">
              ...
              <div className="mt-4 pt-4 border-t border-gray-100">
                <div className="flex justify-between text-xs text-gray-500 mb-1.5">
                  <span>Profile strength</span>
                  <span className="font-semibold" style={{ color: "#1960d2" }}>{strength}%</span>
                </div>
                <div className="w-full bg-gray-100 rounded-full h-1.5">
                  <div className="h-1.5 rounded-full transition-all duration-500"
                    style={{
                      width: `${strength}%`,
                      background: strength >= 70 ? "linear-gradient(90deg,#1960d2,#2AB836)" : strengthColor
                    }}/>
                </div>
              </div>
            </div>
