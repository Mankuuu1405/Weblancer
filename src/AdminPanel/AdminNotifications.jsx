...
const mockNotifications = [
  ...
  { id:"N-003", category:"Payment",  priority:"High",   title:"Escrow frozen — dispute detected",                 desc:"PAY-007 (₹1,40,000) frozen automatically due to DSP-002. Admin review required.", time:"3h ago",  read:false, actionRoute:"/admin/payments",         actionLabel:"View Payment"   },
  { id:"N-004", category:"Payment",  priority:"Medium", title:"Payout request — Arjun Dev",                       desc:"Withdrawal of ₹85,000 requested. KYC verified. Awaiting approval.", time:"5h ago",  read:false, actionRoute:"/admin/payouts",          actionLabel:"Approve Payout" },
  { id:"N-005", category:"Payment",  priority:"Low",    title:"Commission collected — ₹39,000",                   desc:"Platform commission auto-collected from HR Automation Dashboard.", time:"1d ago",  read:true,  actionRoute:"/admin/commission",       actionLabel:"View Commission"},
  { id:"N-006", category:"AI Flag",  priority:"High",   title:"AI flagged suspicious account — FakeUser999",      desc:"Duplicate signup pattern detected. Risk score: 10/100. Recommended action: Permanent ban.", time:"30m ago", read:false, actionRoute:"/admin/users/USR-010",    actionLabel:"Review Account" },
  { id:"N-007", category:"AI Flag",  priority:"Medium", title:"Agency overload detected — TechNova Solutions",    desc:"AI detected team capacity at 95%. 4 active projects vs 6 limit. Risk of delivery failure.", time:"4h ago",  read:false, actionRoute:"/admin/agencies/AG-001",  actionLabel:"View Agency"    },
  { id:"N-008", category:"AI Flag",  priority:"Medium", title:"High dispute rate — Priya Menon",                  desc:"Freelancer dispute rate crossed 30% threshold. AI auto-reduced visibility.", time:"6h ago",  read:true,  actionRoute:"/admin/freelancers/FL-004",actionLabel:"View Freelancer"},
  { id:"N-009", category:"KYC",      priority:"Medium", title:"New KYC submission — PixelCraft Studio",            desc:"Agency submitted business documents for verification. Waiting 2 days for review.", time:"2d ago",  read:false, actionRoute:"/admin/kyc",              actionLabel:"Review KYC"    },
  { id:"N-010", category:"KYC",      priority:"Low",    title:"KYC re-upload requested — Meera Joshi",             desc:"Client submitted updated identity documents after initial rejection.", time:"3d ago",  read:true,  actionRoute:"/admin/kyc",              actionLabel:"Review KYC"    },
  { id:"N-011", category:"Project",  priority:"High",   title:"Project at risk — E-Commerce Revamp",               desc:"PRJ-003 health dropped to Delayed. Deadline Mar 25. Client unresponsive for 5 days.", time:"8h ago",  read:false, actionRoute:"/admin/projects/PRJ-003", actionLabel:"View Project"   },
  { id:"N-012", category:"Project",  priority:"Low",    title:"Project completed — HR Automation Dashboard",        desc:"PRJ-004 marked complete. All milestones done. Client satisfaction: 4.8/5.", time:"2d ago",  read:true,  actionRoute:"/admin/projects/PRJ-004", actionLabel:"View Project"   },
...
const CATEGORY_CFG = {
  ...
  "Payment": { icon:"⊕", iconBg:G.blueBg,   iconColor:G.blue,    chipBg:G.blueBg,   chipBorder:G.blueBorder,   chipText:G.blue      },
  "AI Flag": { icon:"◎", iconBg:G.amberBg,  iconColor:"#b45309", chipBg:G.amberBg,  chipBorder:G.amberBorder,  chipText:"#b45309"   },
  "KYC":     { icon:"◉", iconBg:"#f5f3ff",  iconColor:"#7c3aed", chipBg:"#f5f3ff",  chipBorder:"#ddd6fe",      chipText:"#7c3aed"   },
  "Project": { icon:"⊟", iconBg:G.amberBg,  iconColor:"#92400e", chipBg:G.amberBg,  chipBorder:G.amberBorder,  chipText:"#92400e"   },
  "System":  { icon:"⊙", iconBg:G.bg,       iconColor:G.muted,   chipBg:G.bg,       chipBorder:G.border,       chipText:G.muted     },
};
const PRIORITY_DOT = { High: G.red, Medium: G.amber, Low: G.border };
...
export function AdminNotifications() {
  ...
  const [activePriority,  setActivePriority]  = useState("All");
  const [activeRead,      setActiveRead]      = useState("All");
  const [showSidebar,     setShowSidebar]     = useState(false); // mobile filter drawer
  ...
  // Sidebar content extracted as component to reuse in both desktop + mobile drawer
  const SidebarContent = () => (
    <>
      {/* Categories */}
      <div style={{ padding:"12px 14px", borderBottom:`1px solid ${G.greenBorder}`, background:G.greenBg }}>
        <p style={{ fontSize:10, fontWeight:800, color:G.greenDeep, margin:0, textTransform:"uppercase", letterSpacing:"0.07em" }}>Categories</p>
      </div>
      <div style={{ padding:8 }}>
        {categories.map(cat => {
          ...
          const count = countByCategory(cat);
          return (
            <button key={cat} onClick={() => { setActiveCategory(cat); setShowSidebar(false); }} style={sideNavBtn(activeCategory === cat)}>
              {cfg
                ? <span style={{ width:24, height:24, borderRadius:6, background:cfg.iconBg, color:cfg.iconColor, display:"flex", alignItems:"center", justifyContent:"center", fontSize:12, flexShrink:0 }}>{cfg.icon}</span>
                : <span style={{ width:24, height:24, borderRadius:6, background:G.bg, color:G.muted, display:"flex", alignItems:"center", justifyContent:"center", fontSize:12, flexShrink:0 }}>◈</span>
              }
              <span style={{ flex:1, fontSize:12 }}>{cat}</span>
              {count > 0 && <span style={{ fontSize:10, fontWeight:800, background:G.red, color:G.white, padding:"1px 6px", borderRadius:99 }}>{count}</span>}
            </button>
          );
        })}
      </div>
      {/* Priority */}
      <div style={{ padding:"12px 14px", borderTop:`1px solid ${G.greenBorder}` }}>
        <p style={{ fontSize:10, fontWeight:800, color:G.greenDeep, margin:"0 0 8px", textTransform:"uppercase", letterSpacing:"0.07em" }}>Priority</p>
        {["All","High","Medium","Low"].map(p => (
          <button key={p} onClick={() => { setActivePriority(p); setShowSidebar(false); }} style={sideNavBtn(activePriority === p)}>
            {p !== "All" && <span style={{ width:7, height:7, borderRadius:"50%", background:PRIORITY_DOT[p], flexShrink:0 }} />}
      ...
      <div style={{ padding:"12px 14px", borderTop:`1px solid ${G.greenBorder}` }}>
        <p style={{ fontSize:10, fontWeight:800, color:G.greenDeep, margin:"0 0 8px", textTransform:"uppercase", letterSpacing:"0.07em" }}>Status</p>
        {["All","Unread","Read"].map(r => (
          <button key={r} onClick={() => { setActiveRead(r); setShowSidebar(false); }} style={sideNavBtn(activeRead === r)}>
            {r === "Unread" && <span style={{ width:7, height:7, borderRadius:"50%", background:G.green, flexShrink:0 }} />}
            {r === "Read"   && <span style={{ width:7, height:7, borderRadius:"50%", background:G.border, flexShrink:0 }} />}
    ...
    <div style={{ padding:"12px 12px 64px", fontFamily:FONT, background:G.bg, minHeight:"100%" }}>
      <style>{`
        ...
        * { font-family:'Poppins',sans-serif; box-sizing:border-box; }
        ...
        @media (min-width:640px)  { .notif-page { padding:20px 20px 64px !important; } }
        @media (min-width:1024px) { .notif-page { padding:28px 28px 64px !important; } }

        /* Desktop layout: sidebar + feed side by side */
        .notif-layout { display:flex; flex-direction:column; gap:16px; }
        @media (min-width:1024px) {
          .notif-layout { flex-direction:row; gap:20px; }
          .notif-sidebar-desktop { display:block !important; }
          .notif-filter-btn { display:none !important; }
        }
        .notif-sidebar-desktop { display:none; width:200px; flex-shrink:0; }

        /* Mobile filter drawer */
        .notif-filter-backdrop {
          position:fixed; inset:0; z-index:50;
          background:rgba(15,26,59,0.25);
        ...
        /* Horizontal scroll for category chips on mobile */
        .notif-chips { display:flex; gap:8px; overflow-x:auto; padding-bottom:4px; -webkit-overflow-scrolling:touch; }
        .notif-chips::-webkit-scrollbar { display:none; }
        @media (min-width:1024px) { .notif-chips { display:none !important; } }
      `}</style>

      {/* Header */}
      <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", marginBottom:14, gap:10, flexWrap:"wrap" }}>
        <div>
          <h1 style={{ fontSize:18, fontWeight:800, color:G.text, margin:0, letterSpacing:"-0.3px" }}>Notifications</h1>
          <p style={{ fontSize:11, color:G.muted, marginTop:2 }}>All system alerts, AI flags &amp; action items</p>
        </div>
        <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
          ...
          <button
            ...
            style={{ fontSize:11, fontWeight:700, fontFamily:FONT, background:G.greenBg, color:G.greenDeep, border:`1px solid ${G.greenBorder}`, borderRadius:100, padding:"7px 14px", cursor:"pointer" }}>
            ⚙ Filters{(activeCategory!=="All"||activePriority!=="All"||activeRead!=="All") ? " •" : ""}
          </button>
          {unread > 0 && (
            <button onClick={markAllRead} style={{ fontSize:11, fontWeight:700, fontFamily:FONT, background:G.greenBg, color:G.greenDeep, border:`1px solid ${G.greenBorder}`, borderRadius:100, padding:"7px 14px", cursor:"pointer", whiteSpace:"nowrap" }}>
              Mark all read ({unread})
            </button>
          )}
          <button onClick={dismissAll} style={{ fontSize:11, fontWeight:700, fontFamily:FONT, background:G.bg, color:G.sub, border:`1px solid ${G.border}`, borderRadius:100, padding:"7px 14px", cursor:"pointer", whiteSpace:"nowrap" }}>
            Clear read
          </button>
        </div>
      </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(2, 1fr)", gap:10, marginBottom:14 }}
        ...
        <StatCard label="Total"         value={notifications.length}                                             color="gray"   bg="gray"   border="gray"   />
        <StatCard label="Unread"        value={unread}                         sub="Needs attention"             color="red"    bg="red"    border="red"    />
        <StatCard label="High Priority" value={notifications.filter(n=>n.priority==="High"&&!n.read).length}    color="orange" bg="orange" border="orange" />
        <StatCard label="AI Flags"      value={notifications.filter(n=>n.category==="AI Flag"&&!n.read).length} color="blue"   bg="blue"   border="blue"   />
      </div>

      {/* Mobile horizontal category chips */}
      <div className="notif-chips" style={{ marginBottom:12 }}>
        {categories.map(cat => {
          const cfg = CATEGORY_CFG[cat];
          ...
          return (
            <button key={cat} onClick={() => setActiveCategory(cat)}
              style={{
                ...
                color:active ? G.greenDeep : G.sub,
                fontWeight:active ? 700 : 500, fontSize:11, fontFamily:FONT,
                cursor:"pointer", flexShrink:0,
                boxShadow: active ? `0 0 0 1.5px ${G.green}` : `0 0 0 1px ${G.border}`,
              }}>
              {cfg && <span>{cfg.icon}</span>}
              {cat}
              {count > 0 && <span style={{ fontSize:9, fontWeight:800, background:G.red, color:G.white, padding:"1px 5px", borderRadius:99 }}>{count}</span>}
            </button>
          );
        })}
      </div>
      ...
      <div className="notif-layout">
        ...
        {/* ── Desktop Sidebar ── */}
        <div className="notif-sidebar-desktop">
          <div style={{ background:G.white, border:`1px solid ${G.greenBorder}`, borderRadius:16, overflow:"hidden", boxShadow:"0 2px 12px rgba(110,192,48,0.06)", position:"sticky", top:24 }}>
            <SidebarContent />
          </div>
        </div>
        ...
        <div style={{ flex:1, minWidth:0 }}>
          <div style={{ background:G.white, border:`1px solid ${G.greenBorder}`, borderRadius:16, overflow:"hidden", boxShadow:"0 2px 12px rgba(110,192,48,0.06)" }}>
            {/* Feed header */}
            <div style={{ padding:"11px 16px", borderBottom:`1px solid ${G.greenBorder}`, background:G.greenBg, display:"flex", alignItems:"center", justifyContent:"space-between" }}>
              <p style={{ fontSize:12, fontWeight:700, color:G.text, margin:0 }}>
                {filtered.length} notification{filtered.length !== 1 ? "s" : ""}
                {activeCategory !== "All" && <span style={{ color:G.muted, fontWeight:400 }}> in {activeCategory}</span>}
              </p>
              <span style={{ fontSize:10, color:G.muted }}>Latest first</span>
            </div>
                  ...
                  return (
                    <div key={n.id} style={{
                      ...
                      <div style={{ marginTop:8, flexShrink:0 }}>
                        <div style={{ width:6, height:6, borderRadius:"50%", background:!n.read ? G.green : "transparent" }} />
                      </div>
                      {/* Icon */}
                      <div style={{ width:32, height:32, borderRadius:9, background:cfg.iconBg, color:cfg.iconColor, display:"flex", alignItems:"center", justifyContent:"center", fontSize:13, flexShrink:0 }}>
                        {cfg.icon}
                      </div>
                      {/* Content */}
                      <div style={{ flex:1, minWidth:0 }}>
                        <div style={{ display:"flex", alignItems:"center", gap:5, flexWrap:"wrap", marginBottom:3 }}>
                          <span style={{ fontSize:9, fontWeight:700, background:cfg.chipBg, color:cfg.chipText, border:`1px solid ${cfg.chipBorder}`, padding:"2px 7px", borderRadius:99, whiteSpace:"nowrap" }}>{n.category}</span>
                          <span style={{ display:"flex", alignItems:"center", gap:3, fontSize:9, fontWeight:600, color:G.muted }}>
                            <span style={{ width:5, height:5, borderRadius:"50%", background:PRIORITY_DOT[n.priority] }} />
                            {n.priority}
                          </span>
                          {!n.read && <span style={{ fontSize:9, fontWeight:800, color:G.greenDeep }}>NEW</span>}
                        </div>
                        <p style={{ fontSize:12, fontWeight:700, color:G.text, margin:"0 0 3px", lineHeight:1.4 }}>{n.title}</p>
                        <p style={{ fontSize:11, color:G.muted, margin:"0 0 8px", lineHeight:1.6 }}>{n.desc}</p>
                        <div style={{ display:"flex", alignItems:"center", gap:10, flexWrap:"wrap" }}>
                          <span style={{ fontSize:10, color:G.muted }}>{n.time}</span>
                          <button onClick={() => { markRead(n.id); navigate(n.actionRoute); }}
                            style={{ fontSize:11, fontWeight:700, color:G.greenDeep, background:"none", border:"none", cursor:"pointer", fontFamily:FONT, padding:0, whiteSpace:"nowrap" }}>
                            {n.actionLabel} →
                          </button>
                          {!n.read && (
                            <button onClick={() => markRead(n.id)}
                              style={{ fontSize:11, color:G.muted, background:"none", border:"none", cursor:"pointer", fontFamily:FONT, padding:0, whiteSpace:"nowrap" }}>
                              Mark read
                            </button>
                          )}
                          <button onClick={() => dismiss(n.id)}
                            style={{ fontSize:11, color:G.border, background:"none", border:"none", cursor:"pointer", fontFamily:FONT, padding:0, marginLeft:"auto" }}>
                            ✕
                          </button>
                        </div>
                      </div>
      ...
      {/* Mobile Filter Drawer */}
      {showSidebar && (
        <div className="notif-filter-backdrop" onClick={() => setShowSidebar(false)}>
          <div className="notif-filter-drawer" onClick={e => e.stopPropagation()}>
            <div className="notif-drawer-handle" />
            <div style={{ background:G.white, borderRadius:"0 0 20px 20px" }}>
              <SidebarContent />
            </div>
          </div>
        </div>
