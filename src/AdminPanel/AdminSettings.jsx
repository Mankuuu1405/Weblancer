...
const RESPONSIVE_CSS = `
  ...
  * { font-family: 'Poppins', sans-serif; }
  ...
  /* ── Main grids ── */
  .set-platform-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 20px; }
  .set-security-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 20px; }
  .set-legal-grid    { display: grid; grid-template-columns: 1fr 1fr; gap: 20px; }

  /* ── Admin roles table ── */
  .set-perm-table { overflow-x: auto; }
  .set-perm-table table { min-width: 500px; }
  ...
  .set-row-left  { flex: 1; margin-right: 16px; min-width: 0; }
  .set-row-right { flex-shrink: 0; }
  ...
  /* ── Header actions ── */
  .set-header-actions { display: flex; gap: 8px; flex-wrap: wrap; }

  /* ─────────────────────────────────
     TABLET  ≤900px
  ───────────────────────────────── */
  @media (max-width: 900px) {
    .set-platform-grid { grid-template-columns: 1fr; }
    .set-security-grid { grid-template-columns: 1fr; }
    .set-legal-grid    { grid-template-columns: 1fr; }
  }
  ...
  @media (max-width: 480px) {
    .set-padding { padding: 16px 14px 48px !important; }
    .set-footer  { justify-content: stretch; }
    .set-footer > * { flex: 1; justify-content: center; }
    /* Input widths: let them grow full on tiny screens */
    .set-row-right input,
    .set-row-right select { width: 100% !important; max-width: 180px; }
  }
`;
...
const btnOutline = {
  ...
  fontSize:12, fontWeight:700, fontFamily:FONT,
  background:G.greenBg, color:G.greenDeep,
  border:`1px solid ${G.greenBorder}`, borderRadius:100, padding:"8px 18px", cursor:"pointer", whiteSpace:"nowrap",
};
...
export default function AdminSettings() {
  ...
  const [platform, setPlatform] = useState({
    platformName:"Weblance", platformEmail:"admin@weblance.com",
    supportEmail:"support@weblance.com", legalEmail:"legal@weblance.com",
    commissionRate:"6", currency:"INR", maintenanceMode:false, registrationOpen:true,
  });
  ...
  const [notifications, setNotifications] = useState({
    emailOnDispute:true, emailOnSuspension:true, emailOnHighRisk:true,
    emailOnPaymentAnomaly:true, slackNotifications:false, dailySummary:true,
  });

  const [security, setSecurity] = useState({
    twoFactorRequired:true, sessionTimeout:"8", ipWhitelist:false, loginAudit:true, rateLimit:true,
  });
  ...
  return (
    <div className="set-padding p-6">
      ...
      <div className="set-tabs">
        {tabs.map(tab => (
          <button key={tab} onClick={() => setActiveTab(tab)}
            className={`px-4 py-2 text-sm font-medium capitalize whitespace-nowrap transition-colors flex-shrink-0 ${activeTab===tab?"text-green-600 border-b-2 border-green-500 -mb-px":"text-gray-500 hover:text-gray-700"}`}>
            {tab}
          </button>
        ))}
      </div>
      ...
      {/* Platform Tab */}
      {activeTab==="platform" && (
        <div className="set-platform-grid">
          <SectionCard title="Platform Identity">
            <SettingRow label="Platform Name" desc="Displayed in emails and UI">
              <TextInput value={platform.platformName} onChange={set(platform,setPlatform)("platformName")} />
            </SettingRow>
      ...
      {activeTab==="admin roles" && (
        <div className="space-y-5">
          <SectionCard title="Admin Team">
            <div className="space-y-2 mb-4">
              {adminRoles.map((admin,i) => (
                <div key={i} className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg border border-gray-100 flex-wrap">
                  <div className="w-8 h-8 rounded-full bg-green-500 flex items-center justify-center text-white text-xs font-bold shrink-0">
                    {admin.name.split(" ").map(n=>n[0]).join("").slice(0,2)}
                  </div>
                  <div className="flex-1 min-w-0">
          ...
          <SectionCard title="Permission Matrix">
            <div className="set-perm-table">
              <table className="w-full text-xs">
                <thead>
                  <tr className="border-b border-gray-100">
                    <th className="text-left text-gray-400 pb-2 pr-4">Permission</th>
                    {["Super Admin","Platform Admin","Finance Admin","Support Admin"].map(r => (
                      <th key={r} className="text-center text-gray-500 font-semibold pb-2 px-3 whitespace-nowrap">{r}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {[
                    ...
                    <tr key={row.perm} className="border-b border-gray-50 last:border-0">
                      <td className="py-2 pr-4 text-gray-600 font-medium whitespace-nowrap">{row.perm}</td>
                      {row.access.map((has,i) => (
                        <td key={i} className="py-2 px-3 text-center">
                          <span className={has?"text-green-500":"text-gray-200"}>{has?"✓":"✕"}</span>
                        </td>
                      ))}
                    </tr>
                  ))}
          ...
          <SectionCard title="Danger Zone">
            <div className="space-y-3">
              {[
                ...
                <div key={a.label} className="flex items-center justify-between p-3 rounded-lg border border-gray-100 flex-wrap gap-2">
                  <div>
                    <p className="text-sm font-medium text-gray-800">{a.label}</p>
                    <p className="text-xs text-gray-400 mt-0.5">{a.desc}</p>
                  </div>
          ...
          {[
            ...
            <SectionCard key={doc.title} title={doc.title}>
              <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
                <div>
                  <p className="text-xs text-gray-400">Last updated: {doc.lastUpdated}</p>
                  <span className="text-[11px] bg-green-50 text-green-700 border border-green-200 px-2 py-0.5 rounded-full font-semibold mt-1 inline-block">{doc.status}</span>
                </div>
                <div className="flex gap-2">
                  <ActionBtn label="Edit" style={btnOutline}/>
                  <ActionBtn label="Preview" style={btnNavy} />
