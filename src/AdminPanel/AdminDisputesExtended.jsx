...
const evidenceIcon = {
  Deliverable: "📦", Contract: "📄", Proof: "🖼", "Chat Log": "💬",
  "System Log": "⊙", Brief: "📋", Financial: "💰", Resolution: "✅", "AI Report": "🤖",
};
...
export function AdminDisputeDetail() {
  ...
  return (
    <div className="wl-page-padding" style={{ padding: "24px" }}>

      <style>{`
        .wl-page-padding { padding: 24px; }

        /* Quick stats */
        .wl-dispute-quick-stats {
          display: grid;
        ...
        /* Summary grid */
        .wl-dispute-summary-grid {
          display: grid;
          grid-template-columns: 1fr 1fr 1fr;
          gap: 20px;
        }
        .wl-dispute-summary-main { grid-column: span 2; }
        .wl-dispute-summary-aside { grid-column: span 1; }

        /* Decision grid */
        .wl-dispute-decision-grid {
          display: grid;
        ...
        @media (max-width: 1024px) {
          ...
          .wl-dispute-summary-main { grid-column: span 1; }
          .wl-dispute-summary-aside { grid-column: span 1; }
          .wl-dispute-decision-grid {
        ...
        @media (max-width: 640px) {
          .wl-page-padding { padding: 16px !important; }
          ...
          .wl-dispute-header-actions {
            width: 100%;
            display: flex;
            gap: 8px;
            flex-wrap: wrap;
          }
          .wl-dispute-header-actions button {
            flex: 1 1 auto;
          }
          ...
          .wl-pending-card-actions button {
      ...
      {/* Header */}
      <div className="wl-dispute-header-row">
        <div>
          <div className="flex items-center gap-2.5 flex-wrap mb-1">
            <h1 className="text-xl font-bold text-gray-900">{d.id}</h1>
            <span className={`text-[11px] font-semibold px-2 py-0.5 rounded-full ${statusStyle[d.status]}`}>{d.status}</span>
        ...
        <div className="wl-dispute-header-actions" style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          <ActionBtn label="View Project" onClick={() => navigate(`/admin/projects/${d.projectId}`)} />
          {isPending && <ActionBtn label="Pending Decisions" variant="warning" onClick={() => navigate("/admin/disputes/pending")} />}
        </div>
      </div>
      ...
      {/* Quick Stats */}
      <div className="wl-dispute-quick-stats">
        {[
          { label: "Amount Disputed",    value: `₹${d.amountDisputed.toLocaleString()}`,  color: "text-orange-500" },
          { label: "Escrow Available",   value: d.escrowAvailable > 0 ? `₹${d.escrowAvailable.toLocaleString()}` : "Released", color: "text-blue-600" },
          { label: "AI Confidence",      value: `${d.aiConfidence}%`,                     color: d.aiConfidence >= 80 ? "text-green-600" : d.aiConfidence >= 60 ? "text-yellow-600" : "text-red-500" },
          ...
          <div key={s.label} className="bg-white rounded-xl border border-gray-100 p-3 shadow-sm text-center">
            <p className="text-xs text-gray-400 mb-1" style={{ fontSize: 10 }}>{s.label}</p>
            <p className={`text-lg font-bold ${s.color}`}>{s.value}</p>
          </div>
        ))}
      </div>
      ...
      {/* Tabs */}
      <div className="flex gap-1 border-b border-gray-100 mb-6" style={{ overflowX: "auto", WebkitOverflowScrolling: "touch" }}>
        {tabs.map(tab => (
          <button key={tab} onClick={() => setActiveTab(tab)}
            className={`px-4 py-2 text-sm font-medium capitalize whitespace-nowrap transition-colors ${activeTab === tab ? "text-green-600 border-b-2 border-green-500 -mb-px" : "text-gray-500 hover:text-gray-700"}`}>
            {tab}{tab === "evidence" ? ` (${d.evidence.length})` : tab === "decision" && isPending ? " ⚡" : ""}
      ...
      {/* ── SUMMARY ── */}
      {activeTab === "summary" && (
        <div className="wl-dispute-summary-grid">
          <div className="wl-dispute-summary-main" style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            <SectionCard title="Dispute Details">
              <InfoRow label="Dispute ID"  value={d.id} />
              <InfoRow label="Project"     value={d.project} />
              <InfoRow label="Raised By"   value={d.raisedBy} />
          ...
          <div className="wl-dispute-summary-aside" style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            <SectionCard title="Client">
              <div className="flex items-center gap-3 mb-3">
                <Avatar name={d.client.name} size="md" />
                <div>
      ...
      {activeTab === "evidence" && (
        <SectionCard title={`Evidence Files (${d.evidence.length})`}>
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {d.evidence.map((ev, i) => (
              <div key={i} className="wl-evidence-item">
                <div style={{ width: 40, height: 40, background: "#fff", borderRadius: 8, border: "1px solid #e5e7eb", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20, flexShrink: 0 }}>
                  {evidenceIcon[ev.type] || "📄"}
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <p style={{ fontSize: 13, fontWeight: 700, color: "#1C1C1C", margin: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{ev.name}</p>
                  <p style={{ fontSize: 11, color: "#9ca3af", margin: 0 }}>Submitted by {ev.submittedBy} · {ev.date}</p>
                </div>
                <div className="wl-evidence-actions">
                  <span className={`text-[11px] font-semibold px-2 py-0.5 rounded-full ${
                    ev.submittedBy === "Client"     ? "bg-green-50 text-green-700 border border-green-200" :
                    ev.submittedBy === "Freelancer" || ev.submittedBy === "Agency" ? "bg-blue-50 text-blue-700 border border-blue-200" :
                    ev.submittedBy === "AI"         ? "bg-purple-50 text-purple-700 border border-purple-200" :
        ...
        <SectionCard title="ProjectStream — Chat History (Read-only)">
          ...
          <div className="p-3 bg-blue-50 rounded-xl border border-blue-100">
            <p className="text-xs text-blue-700 font-medium mb-2">
              Full chat history is available in ProjectStream.
            </p>
            <ActionBtn label="Monitor Full Chat →" size="sm"
              onClick={() => navigate(`/admin/projectstream/${d.projectId}`)} />
          </div>
      ...
      {/* ── DECISION ── */}
      {activeTab === "decision" && (
        <div className="wl-dispute-decision-grid">
          <div>
            {d.adminDecision ? (
              <div className="bg-green-50 border border-green-200 rounded-xl p-5">
                <p className="text-base font-bold text-green-800 mb-2">✓ Decision Already Made</p>
              ...
              <SectionCard title="Make Admin Decision">
                <div className="space-y-2 mb-4">
                  {[
                    { key: `Rule in favor of Client — Full refund (₹${d.amountDisputed.toLocaleString()})`,  label: "Rule in favor of Client — Full refund" },
                    { key: `Rule in favor of ${d.talent.type} — Full release`,                               label: `Rule in favor of ${d.talent.type} — Full release` },
                    { key: `Split — Follow AI verdict: ${d.aiVerdict}`,                                      label: `Split — Follow AI: ${d.aiVerdict}` },
                    { key: "Request more evidence from both parties",                                         label: "Request more evidence" },
                    { key: "Mediation — schedule a meeting with both parties",                                label: "Mediation — schedule meeting" },
                    { key: "Close dispute — no action required",                                              label: "Close — no action required" },
                  ].map(opt => (
                    <button key={opt.key}
                      onClick={() => setDecision(decision === opt.key ? "" : opt.key)}
                      className={`w-full text-left px-3 py-2.5 rounded-lg border text-sm font-medium transition-colors ${decision === opt.key ? "border-green-400 bg-green-50 text-green-700" : "border-gray-200 text-gray-700 hover:bg-gray-50"}`}>
                      {decision === opt.key && <span className="mr-1.5">✓</span>}
                      {opt.label}
                    </button>
                  ))}
                ...
                <div className="mb-4">
                  <label className="text-xs font-semibold text-gray-500 block mb-1.5">Decision note *</label>
                  <textarea
                    value={decisionNote}
                    onChange={e => setDecisionNote(e.target.value)}
  ...
  return (
    <div className="wl-page-padding" style={{ padding: "24px" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24, flexWrap: "wrap", gap: 12 }}>
        <div>
          <h1 className="text-xl font-bold text-gray-900">Pending Decisions</h1>
          <p className="text-sm text-gray-400 mt-1">Disputes requiring admin decision — sorted by urgency</p>
        </div>
        <ActionBtn label="← All Disputes" onClick={() => navigate("/admin/disputes")} />
      </div>

      <div className="wl-pending-stats">
        <StatCard label="Awaiting Decision"  value={pending.length}                                      sub="Admin action needed" color="red"    />
        <StatCard label="High Priority"      value={pending.filter(d => d.priority === "High").length}   color="orange" />
        <StatCard label="AI Ready (≥80%)"    value={pending.filter(d => d.aiConfidence >= 80).length}    sub="Can auto-follow AI"  color="green"  />
        <StatCard label="Total at Stake"     value={`₹${(pending.reduce((s, d) => s + d.amountDisputed, 0) / 1000).toFixed(0)}k`} color="orange" />
            ...
            .map(d => (
              <div key={d.id} className="bg-white rounded-xl border border-gray-100 shadow-sm hover:shadow-md transition-all">
                <div className="p-5">
                  ...
                  {/* Parties */}
                  <div className="wl-parties-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 16 }}>
                    <div className="flex items-center gap-2 bg-green-50 rounded-lg p-2.5 border border-green-100">
                      <Avatar name={d.client.name} size="sm" />
                      <div>
                        <p className="text-[10px] text-gray-400">CLIENT</p>
                  ...
                  {/* Actions */}
                  <div className="wl-pending-card-actions" style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                    <ActionBtn label="Review & Decide" variant="primary" size="md" onClick={() => navigate(`/admin/disputes/${d.id}`)} />
                    <ActionBtn label="Follow AI Verdict" variant={d.aiConfidence >= 80 ? "primary" : "default"} size="md" onClick={() => navigate(`/admin/disputes/${d.id}`)} />
                    <ActionBtn label="View Project" size="md" onClick={() => navigate(`/admin/projects/${d.projectId}`)} />
                    <ActionBtn label="Monitor Chat" size="md" onClick={() => navigate(`/admin/projectstream/${d.projectId}`)} />
                    <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 6, fontSize: 11, color: "#9ca3af" }}>
                      <span>{d.evidence.length} evidence</span>
                      <span>·</span>
                      <span>{d.messages.length} messages</span>
                    </div>
                  </div>
