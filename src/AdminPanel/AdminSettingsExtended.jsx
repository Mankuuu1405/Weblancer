...
const G = {
  greenLight:"#A8E063", green:"#6EC030", greenDeep:"#2E7D1F", greenBg:"#f1fce8", greenBorder:"#d4edbb",
...
const RESPONSIVE_CSS = `
  ...
  * { font-family: 'Poppins', sans-serif; }
  ...
  .s2-nav::-webkit-scrollbar { display: none; }
  ...
  .s2-row-left  { flex: 1; margin-right: 16px; min-width: 0; }
  .s2-row-right { flex-shrink: 0; }

  /* ── General page 2-col ── */
  .s2-general-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 20px; }

  /* ── Commission 3-col ── */
  .s2-comm-grid { display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 20px; }
  .s2-comm-main { grid-column: span 2; }

  /* ── Policies layout ── */
  .s2-policies-layout { display: flex; gap: 20px; align-items: flex-start; }
  .s2-policies-list   { width: 256px; flex-shrink: 0; }
  .s2-policies-editor { flex: 1; min-width: 0; }

  /* ── Email layout ── */
  .s2-email-layout { display: flex; gap: 20px; align-items: flex-start; }
  .s2-email-list   { flex: 1; min-width: 0; }
  .s2-email-panel  { width: 320px; flex-shrink: 0; }

  /* ── Rules: IF/THEN grid ── */
  .s2-rule-cond-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 8px; }
  ...
  @media (max-width: 900px) {
    .s2-general-grid { grid-template-columns: 1fr; }
    .s2-comm-grid    { grid-template-columns: 1fr; }
    .s2-comm-main    { grid-column: span 1; }
    .s2-policies-layout { flex-direction: column; }
    .s2-policies-list   { width: 100%; }
    .s2-email-layout { flex-direction: column; }
    .s2-email-panel  { width: 100%; }
  }
  ...
  @media (max-width: 480px) {
    .s2-padding { padding: 16px 14px 48px !important; }
    .s2-footer  { justify-content: stretch; }
    .s2-footer > * { flex: 1; justify-content: center; }
    .s2-rule-cond-grid { grid-template-columns: 1fr; }
    .s2-row-right input { width: 100% !important; max-width: 180px; }
  }
`;
...
function SettingRow({ label, desc, children }) {
  return (
    <div className="s2-row">
      ...
      <div className="s2-row-right">{children}</div>
    </div>
  );
}
...
function SaveBar({ onSave, saved }) {
  return (
    <div className="s2-footer">
      <ActionBtn label="Discard" style={btnDanger}/>
      <ActionBtn label={saved?"✓ Saved!":"Save Changes"} variant="primary" size="md" style={btnGreen} onClick={onSave} />
    </div>
  );
}
...
function SettingsNav({ active }) {
  ...
  return (
    <div className="s2-nav">
      {tabs.map(tab => (
        <button key={tab.key} onClick={() => navigate(tab.path)}
          className={`px-4 py-2 text-sm font-medium whitespace-nowrap transition-colors flex-shrink-0 ${active===tab.key?"text-green-600 border-b-2 border-green-500 -mb-px":"text-gray-500 hover:text-gray-700"}`}>
          {tab.label}
        </button>
      ))}
    </div>
...
export function AdminSettingsGeneral() {
  const [saved, setSaved] = useState(false);
  const [form, setForm] = useState({
    platformName:"Weblance", tagline:"AI-powered freelance platform",
    platformEmail:"admin@weblance.com", supportEmail:"support@weblance.com",
    legalEmail:"legal@weblance.com", noreplyEmail:"noreply@weblance.com",
    timezone:"Asia/Kolkata", currency:"INR", language:"English",
    registrationOpen:true, maintenanceMode:false, twoFactorRequired:true, sessionTimeout:"8",
  });
  const set = key => val => setForm(p => ({ ...p, [key]:val }));
  const save = () => { setSaved(true); setTimeout(() => setSaved(false), 2000); };

  return (
    <div className="s2-padding p-6">
      <style>{RESPONSIVE_CSS}</style>
      <PageHeader title="Settings" subtitle="Platform configuration & system preferences" />
      <SettingsNav active="general" />
      <div className="s2-general-grid">
        <SectionCard title="Platform Identity">
          <SettingRow label="Platform Name" desc="Shown in emails and UI">
            <TextInput value={form.platformName} onChange={set("platformName")} className="w-40" />
          </SettingRow>
        ...
        <SectionCard title="Contact Emails">
          <SettingRow label="Admin Email"    desc="Primary ops contact"><TextInput value={form.platformEmail} onChange={set("platformEmail")} className="w-48" /></SettingRow>
          <SettingRow label="Support Email"  desc="Shown to users for help"><TextInput value={form.supportEmail} onChange={set("supportEmail")} className="w-48" /></SettingRow>
          <SettingRow label="Legal Email"    desc="For compliance & legal notices"><TextInput value={form.legalEmail} onChange={set("legalEmail")} className="w-48" /></SettingRow>
          <SettingRow label="No-Reply Email" desc="System transactional emails"><TextInput value={form.noreplyEmail} onChange={set("noreplyEmail")} className="w-48" /></SettingRow>
        </SectionCard>
        <SectionCard title="Access Control">
          <SettingRow label="New Registrations"        desc="Allow new users to sign up"><Toggle value={form.registrationOpen} onChange={set("registrationOpen")} /></SettingRow>
          <SettingRow label="Maintenance Mode"         desc="Platform hidden from all users"><Toggle value={form.maintenanceMode} onChange={set("maintenanceMode")} /></SettingRow>
          <SettingRow label="2FA Required for Admins"  desc="Enforce 2FA on all admin accounts"><Toggle value={form.twoFactorRequired} onChange={set("twoFactorRequired")} /></SettingRow>
          <SettingRow label="Admin Session Timeout"    desc="Hours before auto logout">
            <select value={form.sessionTimeout} onChange={e => set("sessionTimeout")(e.target.value)}
              className="text-sm border border-gray-200 rounded-lg px-3 py-1.5 focus:outline-none focus:ring-2 focus:ring-green-500 text-gray-700">
          ...
          {form.maintenanceMode && <div className="mt-2 p-3 bg-red-50 border border-red-200 rounded-lg"><p className="text-xs text-red-600 font-semibold">⚠ Maintenance mode is ON — platform hidden from all users</p></div>}
        </SectionCard>
        <SectionCard title="Danger Zone">
          <div className="space-y-2">
            {[
              ...
              <div key={a.label} className="flex items-center justify-between p-3 rounded-lg border border-gray-100 hover:bg-gray-50 flex-wrap gap-2">
                <div><p className="text-sm font-medium text-gray-800">{a.label}</p><p className="text-xs text-gray-400">{a.desc}</p></div>
                <ActionBtn label="Execute" variant={a.variant} />
              </div>
            ))}
          </div>
...
export function AdminSettingsCommission() {
  ...
  return (
    <div className="s2-padding p-6">
      ...
      <PageHeader title="Settings" subtitle="Platform configuration & system preferences" />
      <SettingsNav active="commission" />
      <div className="s2-comm-grid">
        <div className="s2-comm-main space-y-5">
          <SectionCard title="Base Commission Rate">
            ...
            <div className="bg-green-50 border border-green-100 rounded-xl p-3 mt-2">
              <p className="text-xs text-green-700">At current rate, a ₹1,00,000 project generates <span className="font-bold">₹{(100000*Number(baseRate)/100).toLocaleString()}</span> in platform commission.</p>
            </div>
          </SectionCard>
          <SectionCard title="Commission Tiers">
            <p className="text-xs text-gray-400 mb-4">Tier overrides are applied instead of the base rate when conditions are met.</p>
                ...
                <div key={tier.id} className={`p-4 rounded-xl border transition-colors ${tier.active?"border-gray-200 bg-white":"border-gray-100 bg-gray-50 opacity-60"}`}>
                  <div className="flex items-start justify-between gap-3 flex-wrap">
                    ...
                    <div className="flex items-center gap-3 shrink-0">
                      ...
                      <Toggle value={tier.active} onChange={val => setTiers(prev => prev.map(t => t.id===tier.id?{...t,active:val}:t))} />
                    </div>
                  </div>
                </div>
              ))}
...
export function AdminSettingsPolicies() {
  ...
  const [editMode, setEditMode] = useState(false);
  const [policies, setPolicies] = useState([
    { id:"POL-001", name:"Privacy Policy",      version:"v2.1", status:"Published", updatedBy:"Super Admin",    updatedAt:"Jan 1, 2026",  content:"This Privacy Policy describes how Weblance collects, uses, and protects your personal information..." },
    { id:"POL-002", name:"Terms of Service",    version:"v3.0", status:"Published", updatedBy:"Super Admin",    updatedAt:"Jan 1, 2026",  content:"By accessing or using Weblance, you agree to be bound by these Terms of Service..." },
    { id:"POL-003", name:"Refund Policy",       version:"v1.2", status:"Published", updatedBy:"Finance Admin",  updatedAt:"Mar 1, 2026",  content:"Weblance operates an escrow-based payment system. Refunds are processed based on dispute resolution..." },
    { id:"POL-004", name:"Cookie Policy",       version:"v1.0", status:"Published", updatedBy:"Super Admin",    updatedAt:"Jan 1, 2026",  content:"Weblance uses cookies and similar tracking technologies to enhance your experience..." },
    { id:"POL-005", name:"Freelancer Policy",   version:"v2.0", status:"Published", updatedBy:"Platform Admin", updatedAt:"Feb 15, 2026", content:"Freelancers on Weblance must comply with the following professional standards..." },
    { id:"POL-006", name:"Agency Policy",       version:"v2.0", status:"Published", updatedBy:"Platform Admin", updatedAt:"Feb 15, 2026", content:"Agencies registered on Weblance are held to enterprise-grade standards..." },
    { id:"POL-007", name:"Community Guidelines",version:"v1.1", status:"Draft",     updatedBy:"Super Admin",    updatedAt:"Mar 10, 2026", content:"Draft: Community guidelines for professional conduct..." },
  ]);
  const save = () => { setSaved(true); setTimeout(() => setSaved(false), 2000); };
  ...
  return (
    <div className="s2-padding p-6">
      ...
      />
      <SettingsNav active="policies" />
      <div className="s2-policies-layout">
        <div className="s2-policies-list">
          <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-3">Documents ({policies.length})</p>
          <div className="space-y-2">
            {policies.map(p => (
              <button key={p.id} onClick={() => { setSelected(p); setEditMode(false); }}
                className={`w-full text-left p-3 rounded-xl border transition-all ${selected?.id===p.id?"border-green-300 bg-green-50 shadow-sm":"border-gray-100 bg-white hover:border-gray-200 hover:shadow-sm"}`}>
                <div className="flex items-center justify-between mb-0.5 flex-wrap gap-1">
                  <p className="text-sm font-semibold text-gray-800">{p.name}</p>
                  <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full ${p.status==="Published"?"bg-green-50 text-green-700 border border-green-200":"bg-gray-50 text-gray-500 border border-gray-200"}`}>{p.status}</span>
                </div>
                <p className="text-[10px] text-gray-400">{p.version} · {p.updatedAt}</p>
              </button>
            ))}
        ...
        <div className="s2-policies-editor">
          {selected ? (
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
              <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between flex-wrap gap-3">
                <div>
                  <div className="flex items-center gap-2 flex-wrap">
                    <h3 className="text-base font-bold text-gray-900">{selected.name}</h3>
                    <span className={`text-[11px] font-semibold px-2 py-0.5 rounded-full ${selected.status==="Published"?"bg-green-50 text-green-700 border border-green-200":"bg-gray-50 text-gray-500 border border-gray-200"}`}>{selected.status}</span>
                  </div>
                  <p className="text-xs text-gray-400 mt-0.5">{selected.version} · Updated {selected.updatedAt} by {selected.updatedBy}</p>
                </div>
              ...
              <div className="p-5">
                {editMode ? (
                  <textarea defaultValue={selected.content} rows={16}
                    className="w-full text-sm border border-gray-200 rounded-xl p-4 focus:outline-none focus:ring-2 focus:ring-green-500 resize-none text-gray-700 leading-relaxed" />
                ) : (
                  <div>
                    <p className="text-sm text-gray-700 leading-relaxed whitespace-pre-wrap">{selected.content}</p>
                    <div className="mt-6 p-3 bg-gray-50 rounded-xl border border-gray-100">
                      <p className="text-xs text-gray-400">This is a preview. Click <strong>Edit</strong> to modify the content.</p>
                    </div>
                  </div>
                )}
              </div>
            ...
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm h-64 flex items-center justify-center">
              <div className="text-center"><span className="text-3xl mb-3 block">📄</span><p className="text-sm text-gray-500">Select a policy to view or edit</p></div>
            </div>
          )}
        </div>
      </div>
...
export function AdminSettingsEmail() {
  ...
  const [templates] = useState([
    { id:"ET-001", name:"Welcome Email",        trigger:"On signup",              category:"Onboarding", subject:"Welcome to Weblance, {{user_name}}!",                active:true  },
    { id:"ET-002", name:"Email Verification",   trigger:"On signup",              category:"Onboarding", subject:"Verify your email — {{otp_code}}",                  active:true  },
    { id:"ET-003", name:"Password Reset",       trigger:"On request",             category:"Auth",       subject:"Reset your Weblance password",                      active:true  },
    { id:"ET-004", name:"KYC Approved",         trigger:"On KYC approval",        category:"KYC",        subject:"Your KYC is verified — you're all set!",            active:true  },
    { id:"ET-005", name:"KYC Rejected",         trigger:"On KYC rejection",       category:"KYC",        subject:"KYC verification failed — action required",         active:true  },
    { id:"ET-006", name:"Escrow Funded",        trigger:"On escrow deposit",      category:"Payments",   subject:"Escrow funded — project can begin",                 active:true  },
    { id:"ET-007", name:"Milestone Released",   trigger:"On milestone approval",  category:"Payments",   subject:"Payment received — ₹{{amount}} released",          active:true  },
    { id:"ET-008", name:"Dispute Raised",       trigger:"On dispute creation",    category:"Disputes",   subject:"Dispute {{dispute_id}} raised — admin notified",    active:true  },
    { id:"ET-009", name:"Dispute Resolved",     trigger:"On dispute resolution",  category:"Disputes",   subject:"Dispute {{dispute_id}} resolved",                   active:true  },
    { id:"ET-010", name:"Admin Warning",        trigger:"On admin action",        category:"Warnings",   subject:"⚠ Official warning from Weblance Admin",           active:true  },
    { id:"ET-011", name:"Silence Reminder",     trigger:"After 72h inactivity",   category:"Automation", subject:"Action required — your project needs attention",    active:true  },
    { id:"ET-012", name:"Daily Admin Summary",  trigger:"Daily at 8:00 AM",       category:"Reports",    subject:"Weblance Daily Summary — {{date}}",                 active:true  },
    { id:"ET-013", name:"Payout Processed",     trigger:"On payout completion",   category:"Payments",   subject:"Payout of ₹{{amount}} processed successfully",     active:true  },
    { id:"ET-014", name:"Account Suspended",    trigger:"On suspension",          category:"Warnings",   subject:"Your Weblance account has been suspended",          active:false },
  ]);
      ...
      <div className="s2-email-layout">
        ...
        <div className="s2-email-list">
          ...
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
            <div className="divide-y divide-gray-50">
              {filtered.map(t => (
                <div key={t.id} onClick={() => { setSelected(t); setEditMode(false); setPreviewMode(false); }}
                  className={`flex items-center gap-4 px-5 py-3.5 cursor-pointer hover:bg-gray-50/50 transition-colors ${selected?.id===t.id?"bg-green-50/40":""}`}>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-0.5 flex-wrap">
                      <p className="text-sm font-semibold text-gray-800">{t.name}</p>
                      <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full ${categoryColor[t.category]}`}>{t.category}</span>
                      {!t.active && <span className="text-[10px] bg-gray-100 text-gray-400 px-1.5 py-0.5 rounded-full">Disabled</span>}
                    </div>
                    <p className="text-xs text-gray-400 truncate">{t.subject}</p>
                    <p className="text-[10px] text-gray-300 mt-0.5">{t.trigger}</p>
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <span className={`w-2 h-2 rounded-full ${t.active?"bg-green-400":"bg-gray-300"}`} />
                    <span className="text-[10px] text-gray-400 hidden sm:block">{t.lastUpdated}</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        {/* Editor panel */}
        <div className="s2-email-panel">
          {selected ? (
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden sticky top-6">
              <div className="px-5 py-3.5 border-b border-gray-100 flex items-center justify-between">
                <div><p className="text-sm font-bold text-gray-800">{selected.name}</p><p className="text-xs text-gray-400">{selected.id}</p></div>
                <button onClick={() => setSelected(null)} className="text-gray-400 hover:text-gray-600">✕</button>
              </div>
              <div className="p-5 space-y-4">
                <div className="flex gap-2 mb-3">
                  <button onClick={() => setPreviewMode(false)} className={`flex-1 py-1.5 text-xs font-medium rounded-lg border transition-colors ${!previewMode?"bg-green-500 text-white border-green-500":"bg-white text-gray-600 border-gray-200"}`}>Edit</button>
                  <button onClick={() => setPreviewMode(true)}  className={`flex-1 py-1.5 text-xs font-medium rounded-lg border transition-colors ${previewMode?"bg-green-500 text-white border-green-500":"bg-white text-gray-600 border-gray-200"}`}>Preview</button>
                </div>
                {!previewMode ? (
                  <>
                    <div>
                      <label className="text-xs font-semibold text-gray-500 block mb-1.5">Subject</label>
                      <input defaultValue={selected.subject} className="w-full text-sm border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-green-500" />
                    </div>
                    <div>
                      <label className="text-xs font-semibold text-gray-500 block mb-1.5">Body</label>
                      <textarea defaultValue={sampleBody} rows={10} className="w-full text-xs border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-green-500 resize-none font-mono leading-relaxed" />
                    </div>
                    <div className="p-3 bg-blue-50 rounded-lg border border-blue-100">
                      <p className="text-[10px] font-semibold text-blue-700 mb-1.5">Available variables:</p>
                      <div className="flex flex-wrap gap-1">
                  ...
                  <div className="border border-gray-200 rounded-xl overflow-hidden">
                    <div className="bg-gray-800 px-4 py-2.5">
                      <p className="text-xs text-gray-300">Subject: <span className="text-white font-medium">{selected.subject.replace("{{user_name}}","Rahul Sharma").replace("{{amount}}","₹85,000")}</span></p>
                    </div>
                    <div className="p-4 bg-gray-50">
                      <div className="bg-white rounded-lg p-4 shadow-sm">
                        <div className="w-16 h-6 bg-green-500 rounded mb-4 flex items-center justify-center"><span className="text-white text-[10px] font-bold">weblance</span></div>
                        <p className="text-xs text-gray-700 leading-relaxed whitespace-pre-wrap">
                          {sampleBody.replace(/{{user_name}}/g,"Rahul Sharma").replace(/{{project_name}}/g,"Food Delivery App").replace(/{{action_url}}/g,"https://weblance.com/dashboard")}
                        </p>
                      </div>
...
export function AdminSettingsRules() {
  ...
  return (
    <div className="s2-padding p-6">
      ...
      {/* Stats */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-5">
        <div className="bg-white rounded-xl border border-gray-100 p-3 shadow-sm text-center"><p className="text-xl font-bold text-gray-800">{rules.length}</p><p className="text-xs text-gray-400">Total Rules</p></div>
        <div className="bg-white rounded-xl border border-gray-100 p-3 shadow-sm text-center"><p className="text-xl font-bold text-green-600">{rules.filter(r=>r.active).length}</p><p className="text-xs text-gray-400">Active</p></div>
        <div className="bg-white rounded-xl border border-gray-100 p-3 shadow-sm text-center"><p className="text-xl font-bold text-blue-600">{rules.reduce((s,r)=>s+r.executions,0)}</p><p className="text-xs text-gray-400">Total Executions</p></div>
        <div className="bg-white rounded-xl border border-gray-100 p-3 shadow-sm text-center"><p className="text-xl font-bold text-orange-500">{Math.round(rules.reduce((s,r)=>s+r.confidence,0)/rules.length)}%</p><p className="text-xs text-gray-400">Avg Confidence</p></div>
      </div>
      ...
      <div className="space-y-3">
        {filtered.map(rule => (
          <div key={rule.id} className={`bg-white rounded-xl border shadow-sm transition-all ${rule.active?"border-gray-100":"border-gray-100 opacity-60"}`}>
            <div className="p-4">
              <div className="flex items-start gap-4 flex-wrap">
                <div className="mt-0.5 shrink-0">
                  <Toggle value={rule.active} onChange={val => setRules(prev => prev.map(r => r.id===rule.id?{...r,active:val}:r))} />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap mb-2">
                    <p className="text-sm font-bold text-gray-800">{rule.name}</p>
                    <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full ${categoryColor[rule.category]}`}>{rule.category}</span>
                    <span className="text-[10px] text-gray-400">{rule.executions} executions</span>
                  </div>
                  <div className="s2-rule-cond-grid">
                    <div className="flex items-start gap-2 p-2.5 bg-yellow-50 rounded-lg border border-yellow-100">
                      <span className="text-yellow-500 text-xs shrink-0 mt-0.5">IF</span>
                      <p className="text-xs text-yellow-800 font-medium">{rule.trigger}</p>
                    </div>
                ...
                <div className="flex items-center gap-3 shrink-0">
                  <div className="text-center">
                    <div className="flex items-center gap-1.5">
                      <div className="w-12 h-1.5 bg-gray-100 rounded-full overflow-hidden">
                        <div className={`h-full rounded-full ${rule.confidence>=90?"bg-green-500":"bg-yellow-400"}`} style={{ width:`${rule.confidence}%` }} />
                      </div>
                      <span className="text-xs font-bold text-gray-600">{rule.confidence}%</span>
                    </div>
                    <p className="text-[10px] text-gray-400 mt-0.5">AI confidence</p>
              ...
              {editingRule===rule.id && (
                <div className="mt-4 pt-4 border-t border-gray-100 grid grid-cols-1 md:grid-cols-2 gap-3">
                  <div>
                    <label className="text-xs font-semibold text-gray-500 block mb-1.5">IF condition</label>
                    <input defaultValue={rule.trigger} className="w-full text-sm border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-green-500" />
                  </div>
                  <div>
                    <label className="text-xs font-semibold text-gray-500 block mb-1.5">THEN action</label>
                    <input defaultValue={rule.action} className="w-full text-sm border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-green-500" />
                  </div>
