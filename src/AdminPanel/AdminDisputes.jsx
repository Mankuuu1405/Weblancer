...
function Avatar({ name }) {
  ...
  const initials = name.split(" ").map(w => w[0]).join("").slice(0,2).toUpperCase();
  return (
    <div style={{ width:26, height:26, borderRadius:"50%", background:colors[idx]+"22", border:`1.5px solid ${colors[idx]}44`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:9, fontWeight:700, color:colors[idx], flexShrink:0 }}>{initials}</div>
  );
}
  ...
  return (
    <div style={{ background:G.white, border:`1px solid ${G.greenBorder}`, borderRadius:16, overflow:"hidden", boxShadow:"0 2px 12px rgba(110,192,48,0.06)" }}>
      <div style={{ padding:"11px 16px", borderBottom:`1px solid ${G.greenBorder}`, background:G.greenBg }}>
        <p style={{ fontSize:11, fontWeight:800, color:G.greenDeep, margin:0, textTransform:"uppercase", letterSpacing:"0.06em" }}>{title}</p>
      </div>
...
function InfoRow({ label, value }) {
  return (
    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", padding:"8px 0", borderBottom:`1px solid ${G.border}`, gap:8 }}>
      <span style={{ fontSize:11, color:G.muted, fontWeight:600, flexShrink:0 }}>{label}</span>
      <span style={{ fontSize:11, color:G.text, fontWeight:700, textAlign:"right", wordBreak:"break-word", maxWidth:"60%" }}>{value}</span>
    </div>
  );
}
...
function StatusBadge({ status }) {
  const s = STATUS_MAP[status] || STATUS_MAP["Closed"];
  return (
    <span style={{ display:"inline-flex", alignItems:"center", gap:4, fontSize:10, fontWeight:700, background:s.bg, color:s.text, border:`1px solid ${s.border}`, padding:"2px 8px", borderRadius:99, whiteSpace:"nowrap" }}>
...
function PriorityBadge({ priority }) {
  const s = PRIORITY_MAP[priority] || PRIORITY_MAP.Low;
  return (
    <span style={{ fontSize:9, fontWeight:700, background:s.bg, color:s.text, border:`1px solid ${s.border}`, padding:"2px 7px", borderRadius:99, whiteSpace:"nowrap" }}>{priority}</span>
  );
}
...
function ConfidenceBar({ pct }) {
  ...
  return (
    <div style={{ display:"flex", alignItems:"center", gap:6 }}>
      <div style={{ width:38, height:4, background:G.border, borderRadius:99, overflow:"hidden" }}>
        <div style={{ width:`${pct}%`, height:"100%", background:color, borderRadius:99 }} />
      </div>
      <span style={{ fontSize:10, fontWeight:700, color:textColor }}>{pct}%</span>
    </div>
  );
}
...
export default function AdminDisputes() {
  ...
  const filtered = mockDisputes.filter(d => {
    const q = search.toLowerCase();
    return (
      (d.project.toLowerCase().includes(q)||d.client.toLowerCase().includes(q)||d.talent.toLowerCase().includes(q)||d.id.toLowerCase().includes(q)) &&
      (!statusFilter   || d.status   === statusFilter) &&
      (!priorityFilter || d.priority === priorityFilter)
    );
  });
  ...
  const getDecisions = (d) => d ? [
    { key:"client",   label:`Rule for Client — Full refund (₹${(d.amountDisputed/1000).toFixed(0)}k)`, activeBg:G.redBg,   activeBorder:G.red,     activeText:"#dc2626"   },
    { key:"talent",   label:"Rule for Talent — Full release",                                           activeBg:G.greenBg, activeBorder:G.green,   activeText:G.greenDeep },
    { key:"split",    label:`Split — AI verdict (${d.aiVerdict})`,                                      activeBg:G.blueBg,  activeBorder:G.blue,    activeText:G.blue      },
    { key:"escalate", label:"Escalate — Request more evidence",                                         activeBg:G.amberBg, activeBorder:G.amber,   activeText:"#92400e"   },
    { key:"close",    label:"Close dispute — No action",                                                activeBg:G.bg,      activeBorder:G.border,  activeText:G.sub       },
  ] : [];

  // Mobile dispute card
  const MobileDisputeCard = ({ d }) => (
    <div
      onClick={() => { setSelected(d); setChosenAction(""); setDecisionNote(""); }}
    ...
    >
      <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", gap:8, marginBottom:10 }}>
        <div>
          <span style={{ fontSize:10, fontWeight:700, color:G.navy, fontFamily:"monospace" }}>{d.id}</span>
          <p style={{ fontSize:13, fontWeight:700, color:G.text, margin:"3px 0 0" }}>{d.project}</p>
        </div>
      ...
      <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:10, flexWrap:"wrap" }}>
        <div style={{ display:"flex", alignItems:"center", gap:5 }}>
          <Avatar name={d.client} />
          <span style={{ fontSize:11, color:G.sub }}>{d.client}</span>
        </div>
        <span style={{ fontSize:10, color:G.muted }}>vs</span>
        <span style={{ fontSize:11, fontWeight:600, color:G.text }}>{d.talent}</span>
      </div>
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:8, paddingTop:10, borderTop:`1px solid ${G.greenBorder}` }}>
        <div>
          <p style={{ fontSize:9, color:G.muted, margin:"0 0 3px", textTransform:"uppercase" }}>Amount</p>
          <span style={{ fontSize:12, fontWeight:700, color:"#b45309" }}>₹{(d.amountDisputed/1000).toFixed(0)}k</span>
        </div>
        <div>
          <p style={{ fontSize:9, color:G.muted, margin:"0 0 3px", textTransform:"uppercase" }}>Confidence</p>
          <ConfidenceBar pct={d.aiConfidence} />
        </div>
        <div>
          <p style={{ fontSize:9, color:G.muted, margin:"0 0 3px", textTransform:"uppercase" }}>Activity</p>
          <span style={{ fontSize:11, color:G.muted }}>{d.lastActivity}</span>
        </div>
      </div>
    </div>
  );
    ...
    <div style={{ padding:"12px 12px 64px", fontFamily:FONT, background:G.bg, minHeight:"100%" }}>
      <style>{`
        ...
        * { font-family:'Poppins',sans-serif; box-sizing:border-box; }
        ...
        @media (min-width: 640px)  { .dsp-page { padding: 20px 20px 64px !important; } }
        @media (min-width: 1024px) { .dsp-page { padding: 28px 28px 64px !important; } }

        .dsp-table-wrap { display: none; }
        .dsp-cards-wrap { display: flex; flex-direction: column; gap: 10px; }
        @media (min-width: 900px) {
          .dsp-table-wrap { display: block; overflow-x: auto; }
          .dsp-cards-wrap { display: none; }
        }
        ...
        .dsp-drawer {
          ...
          background: white;
          overflow-y: auto;
          box-shadow: -8px 0 40px rgba(15,26,59,0.15);
      ...
      <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", marginBottom:14 }}>
        <div>
          <h1 style={{ fontSize:18, fontWeight:800, color:G.text, margin:0, letterSpacing:"-0.3px" }}>Disputes</h1>
          <p style={{ fontSize:11, color:G.muted, marginTop:2 }}>AI investigates, admin decides — every dispute tracked</p>
        </div>
        <button style={btnNavy}>⬇ Export</button>
      </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(2, 1fr)", gap:10, marginBottom:14 }}
        ...
        <StatCard label="Open"          value={stats.open}   sub="Needs action"              color="red"    />
        <StatCard label="Under Review"  value={stats.review}                                 color="blue"   />
        <StatCard label="Amt Disputed"  value={`₹${(stats.amount/1000).toFixed(0)}k`}       color="orange" />
      </div>
      ...
      <div style={{ background:G.white, border:`1px solid ${G.greenBorder}`, borderRadius:16, overflow:"hidden", boxShadow:"0 2px 12px rgba(110,192,48,0.06)" }}>
        ...
        <div style={{ padding:"12px 14px", borderBottom:`1px solid ${G.greenBorder}`, display:"flex", flexWrap:"wrap", gap:8, alignItems:"center", justifyContent:"space-between", background:G.greenBg }}>
          <div style={{ display:"flex", gap:8, flexWrap:"wrap", alignItems:"center", flex:1 }}>
            <div style={{ position:"relative", flex:"1 1 160px", maxWidth:260 }}>
              ...
              <input
                value={search} onChange={e => setSearch(e.target.value)}
                placeholder="Search dispute, project..."
            ...
            {[
              { value:statusFilter,   setter:setStatusFilter,   label:"Status",   opts:["Open","Under Review","Resolved","Closed"] },
              { value:priorityFilter, setter:setPriorityFilter, label:"Priority", opts:["High","Medium","Low"]                     },
            ].map(({ value, setter, label, opts }) => (
              <select key={label} value={value} onChange={e => setter(e.target.value)} style={selectStyle(!!value)}>
                <option value="">{label}</option>
                {opts.map(o => <option key={o} value={o}>{o}</option>)}
        ...
        <div className="dsp-table-wrap">
          <table style={{ width:"100%", borderCollapse:"collapse", minWidth:1050 }}>
            <thead>
              <tr style={{ background:G.bg, borderBottom:`1px solid ${G.greenBorder}` }}>
                {["ID","Project","Client","Talent","Status","Priority","Raised By","Amount","AI Verdict","Confidence","Evidence","Activity",""].map(h => (
            ...
            <tbody>
              {filtered.map(d => (
                <tr
                  key={d.id}
                  style={{ borderBottom:`1px solid ${G.border}`, background:hovRow===d.id?G.greenBg:G.white, cursor:"pointer", transition:"background 0.1s" }}
                  onMouseEnter={() => setHovRow(d.id)}
                  onMouseLeave={() => setHovRow(null)}
                  onClick={() => { setSelected(d); setChosenAction(""); setDecisionNote(""); }}
                >
                  <td style={{ padding:"10px 11px" }}><span style={{ fontSize:10, fontWeight:700, color:G.navy, fontFamily:"monospace" }}>{d.id}</span></td>
                  <td style={{ padding:"10px 11px" }}><span style={{ fontSize:11, fontWeight:700, color:G.text, maxWidth:130, display:"block", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{d.project}</span></td>
                  ...
                  <td style={{ padding:"10px 11px" }}>
                    <p style={{ fontSize:11, color:G.text, fontWeight:600, margin:0 }}>{d.talent}</p>
                    <p style={{ fontSize:9, color:G.muted, margin:0 }}>{d.talentType}</p>
                  </td>
                  <td style={{ padding:"10px 11px" }}><StatusBadge status={d.status} /></td>
                  <td style={{ padding:"10px 11px" }}><PriorityBadge priority={d.priority} /></td>
                  <td style={{ padding:"10px 11px", fontSize:11, color:G.sub }}>{d.raisedBy}</td>
                  <td style={{ padding:"10px 11px" }}><span style={{ fontSize:12, fontWeight:700, color:"#b45309" }}>₹{(d.amountDisputed/1000).toFixed(0)}k</span></td>
                  <td style={{ padding:"10px 11px" }}><span style={{ fontSize:10, color:G.sub, maxWidth:110, display:"block", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{d.aiVerdict}</span></td>
                  <td style={{ padding:"10px 11px" }}><ConfidenceBar pct={d.aiConfidence} /></td>
                  <td style={{ padding:"10px 11px", textAlign:"center" }}><span style={{ fontSize:11, fontWeight:600, color:G.sub }}>{d.evidence}</span></td>
                  <td style={{ padding:"10px 11px", fontSize:11, color:G.muted, whiteSpace:"nowrap" }}>{d.lastActivity}</td>
                  <td style={{ padding:"10px 11px" }} onClick={e => e.stopPropagation()}>
                    <div style={{ opacity:hovRow===d.id?1:0, transition:"opacity 0.15s" }}>
                      <button
                        onClick={e => { e.stopPropagation(); setSelected(d); setChosenAction(""); setDecisionNote(""); }}
                        style={{ fontSize:10, fontWeight:700, fontFamily:FONT, background:G.gradNavy, color:G.white, border:"none", borderRadius:100, padding:"5px 12px", cursor:"pointer" }}>
                        Review
                      </button>
                    </div>
                  </td>
        ...
        {/* Mobile Cards */}
        <div className="dsp-cards-wrap" style={{ padding:12 }}>
          {filtered.map(d => <MobileDisputeCard key={d.id} d={d} />)}
        </div>
        ...
        {filtered.length === 0 && (
          <div style={{ display:"flex", flexDirection:"column", alignItems:"center", padding:"40px 20px", textAlign:"center" }}>
            ...
            <p style={{ fontSize:13, fontWeight:700, color:G.text }}>No disputes found</p>
          </div>
        )}

        <div style={{ padding:"10px 14px", borderTop:`1px solid ${G.greenBorder}`, background:G.greenBg }}>
          <span style={{ fontSize:10, color:G.muted, fontWeight:600 }}>Showing {filtered.length} of {mockDisputes.length} disputes</span>
        </div>
      </div>
      ...
      {selected && (
        <>
          <div className="dsp-backdrop" onClick={() => setSelected(null)} />
          <div className={`dsp-drawer open`}>
            {/* Sticky header */}
            <div style={{ padding:"14px 16px", borderBottom:`1px solid ${G.greenBorder}`, display:"flex", alignItems:"center", justifyContent:"space-between", position:"sticky", top:0, background:G.white, zIndex:10, boxShadow:"0 2px 8px rgba(110,192,48,0.07)" }}>
              <div>
                <div style={{ display:"flex", alignItems:"center", gap:7, flexWrap:"wrap", marginBottom:3 }}>
                  <span style={{ fontSize:14, fontWeight:800, color:G.text, fontFamily:"monospace" }}>{selected.id}</span>
                  <StatusBadge status={selected.status} />
                  <PriorityBadge priority={selected.priority} />
                </div>
                <p style={{ fontSize:11, color:G.muted, margin:0 }}>{selected.project}</p>
              </div>
              <button onClick={() => setSelected(null)} style={{ background:G.bg, border:`1px solid ${G.border}`, borderRadius:"50%", width:30, height:30, display:"flex", alignItems:"center", justifyContent:"center", fontSize:13, color:G.muted, cursor:"pointer", flexShrink:0 }}>✕</button>
            </div>

            <div style={{ padding:16, display:"flex", flexDirection:"column", gap:16 }}>

              {/* AI Analysis */}
              <div style={{ padding:14, borderRadius:14, background:G.blueBg, border:`1px solid ${G.blueBorder}` }}>
                <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:10, flexWrap:"wrap", gap:8 }}>
                  <div style={{ display:"flex", alignItems:"center", gap:7 }}>
                    <span style={{ fontSize:13, fontWeight:700, color:G.blue }}>◎ AI Analysis</span>
                    <span style={{ fontSize:10, color:G.blue, opacity:0.7 }}>{selected.aiConfidence}% confidence</span>
                  </div>
                  <ConfidenceBar pct={selected.aiConfidence} />
                </div>
                <p style={{ fontSize:12, color:G.navy, lineHeight:1.7, margin:"0 0 12px" }}>{selected.chatSummary}</p>
                <div style={{ padding:"10px 12px", background:G.white, borderRadius:10, border:`1px solid ${G.blueBorder}` }}>
                  <p style={{ fontSize:9, fontWeight:700, color:G.muted, textTransform:"uppercase", letterSpacing:"0.06em", margin:"0 0 4px" }}>AI Verdict</p>
                  <p style={{ fontSize:13, fontWeight:800, color:G.text, margin:0 }}>{selected.aiVerdict}</p>
                </div>
              </div>
              ...
              <SectionCard title="Parties Involved">
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
                  <div style={{ padding:10, background:G.greenBg, borderRadius:10, border:`1px solid ${G.greenBorder}` }}>
                    <p style={{ fontSize:9, fontWeight:700, color:G.muted, textTransform:"uppercase", letterSpacing:"0.06em", margin:"0 0 7px" }}>Client</p>
                    <div style={{ display:"flex", alignItems:"center", gap:7 }}>
                      <Avatar name={selected.client} />
                      <span style={{ fontSize:12, fontWeight:700, color:G.text }}>{selected.client}</span>
                    </div>
                  </div>
                  <div style={{ padding:10, background:G.blueBg, borderRadius:10, border:`1px solid ${G.blueBorder}` }}>
                    <p style={{ fontSize:9, fontWeight:700, color:G.muted, textTransform:"uppercase", letterSpacing:"0.06em", margin:"0 0 7px" }}>{selected.talentType}</p>
                    <div style={{ display:"flex", alignItems:"center", gap:7 }}>
                      <Avatar name={selected.talent} />
                      <span style={{ fontSize:12, fontWeight:700, color:G.text }}>{selected.talent}</span>
                    </div>
                  </div>
                </div>
              </SectionCard>
              ...
              <SectionCard title="Dispute Details">
                ...
                <InfoRow label="Raised By"       value={selected.raisedBy}         />
                <InfoRow label="Reason"          value={selected.reason}           />
                <InfoRow label="Amount"          value={`₹${selected.amountDisputed.toLocaleString()}`} />
                <InfoRow label="Raised On"       value={selected.raisedDate}       />
                <InfoRow label="Evidence"        value={`${selected.evidence} files`} />
              </SectionCard>
              ...
              <SectionCard title="Admin Decision">
                ...
                <textarea
                  value={decisionNote}
                  onChange={e => setDecisionNote(e.target.value)}
                  placeholder="Add decision note (sent to both parties)..."
                  rows={3}
                  style={{ width:"100%", fontSize:12, fontFamily:FONT, border:`1.5px solid ${G.greenBorder}`, borderRadius:10, padding:"10px 12px", background:G.white, color:G.text, resize:"none", boxSizing:"border-box", marginBottom:12 }}
                />
                <button style={{
                  width:"100%", padding:"12px 0",
                  fontSize:13, fontWeight:800, fontFamily:FONT,
