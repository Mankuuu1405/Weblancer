...
const btnWarning2= { ...btnOutline, background:G.amberBg, color:G.amberText, border:`1px solid ${G.amberBorder}` };
...
const mockCommission = {
  totalLifetime:121000, thisMonth:28800, lastMonth:39000, thisYear:121000, rate:6,
  byProject:[
    { project:"HR Automation Dashboard", client:"ByteEats Co.",       amount:39000, date:"Feb 28, 2026",  status:"Collected" },
    { project:"Food Delivery App",       client:"ByteEats Co.",        amount:28800, date:"Mar (ongoing)", status:"Pending"   },
    { project:"Patient Appointment App", client:"HealthFirst Clinic",  amount:19200, date:"Apr (ongoing)", status:"Pending"   },
...
const commonStyles = `
  ...
  * { font-family:'Poppins',sans-serif; box-sizing:border-box; }
  ...
  .fin-table-wrap { display:none; overflow-x:auto; }
  .fin-cards-wrap { display:flex; flex-direction:column; gap:10px; }
  @media (min-width:900px) {
    .fin-table-wrap { display:block; }
    .fin-cards-wrap { display:none; }
  }

  .fin-tabs { display:flex; overflow-x:auto; -webkit-overflow-scrolling:touch; border-bottom:1px solid #f3f4f6; margin-bottom:12px; }
  .fin-tabs::-webkit-scrollbar { display:none; }

  .fin-detail-grid { display:flex; flex-direction:column; gap:16px; }
  @media (min-width:1024px) { .fin-detail-grid { display:grid; grid-template-columns:1fr 320px; gap:20px; } }

  .fin-refund-layout { display:flex; flex-direction:column; gap:16px; }
  @media (min-width:1024px) { .fin-refund-layout { flex-direction:row; gap:20px; } }
  .fin-refund-panel { display:none; }
  @media (min-width:1024px) { .fin-refund-panel { display:block; width:300px; flex-shrink:0; } }

  .fin-drawer-backdrop { position:fixed; inset:0; z-index:50; background:rgba(15,26,59,0.25); display:flex; align-items:flex-end; }
  .fin-drawer { background:white; border-radius:20px 20px 0 0; width:100%; max-height:92vh; overflow-y:auto; padding-bottom:env(safe-area-inset-bottom,16px); }
  .fin-drawer-handle { width:36px; height:4px; background:#d1d5db; border-radius:99px; margin:10px auto 8px; }

  .comm-grid { display:flex; flex-direction:column; gap:16px; }
  @media (min-width:1024px) { .comm-grid { display:grid; grid-template-columns:1fr 320px; gap:20px; } }
`;
...
export function AdminEscrow() {
  ...
  const filtered = mockEscrow.filter(e => {
    const q = search.toLowerCase();
    return (e.project.toLowerCase().includes(q)||e.client.toLowerCase().includes(q)||e.talent.toLowerCase().includes(q)) &&
           (!statusFilter || e.status === statusFilter);
  });
  ...
  const openDetail = (e) => { setSelected(e); setShowDrawer(true); };

  const MobileCard = ({ e }) => (
    <div style={{ background:G.white, border:`1px solid ${G.greenBorder}`, borderRadius:14, padding:14, cursor:"pointer" }}
      onClick={() => openDetail(e)}>
      <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", gap:8, marginBottom:8 }}>
        <div>
          <div style={{ display:"flex", alignItems:"center", gap:5, marginBottom:3 }}>
            <p style={{ fontSize:12, fontWeight:700, color:G.text, margin:0 }}>{e.project}</p>
            {e.disputed && <span className="text-[9px] bg-red-50 text-red-500 border border-red-200 px-1.5 py-0.5 rounded-full font-semibold">⚑</span>}
          </div>
          <p style={{ fontSize:10, color:G.muted, margin:0 }}>{e.id}</p>
        </div>
        <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full flex-shrink-0 ${escrowStatusStyle[e.status]}`}>{e.status}</span>
      </div>
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:8, paddingTop:10, borderTop:`1px solid ${G.greenBorder}` }}>
        <div><p style={{ fontSize:9, color:G.muted, margin:"0 0 2px", textTransform:"uppercase" }}>Total</p><span style={{ fontSize:12, fontWeight:700, color:G.text }}>₹{(e.totalAmount/1000).toFixed(0)}k</span></div>
        <div><p style={{ fontSize:9, color:G.muted, margin:"0 0 2px", textTransform:"uppercase" }}>Locked</p><span style={{ fontSize:12, fontWeight:700, color:e.locked>0?"#b45309":G.muted }}>{e.locked>0?`₹${(e.locked/1000).toFixed(0)}k`:"—"}</span></div>
        <div><p style={{ fontSize:9, color:G.muted, margin:"0 0 2px", textTransform:"uppercase" }}>MS</p><span style={{ fontSize:12, fontWeight:700, color:G.text }}>{e.completedMilestones}/{e.milestones}</span></div>
      </div>
    </div>
  );
  ...
  const DetailContent = ({ e }) => (
    <div className="p-4 sm:p-5 space-y-4 sm:space-y-5">
      <div className="grid grid-cols-2 gap-3">
        {[
          { label:"Total Budget", value:`₹${e.totalAmount.toLocaleString()}`,            color:"text-gray-800"  },
          { label:"Locked",       value:e.locked>0?`₹${e.locked.toLocaleString()}`:"—",  color:"text-orange-500"},
          { label:"Released",     value:e.released>0?`₹${e.released.toLocaleString()}`:"—",color:"text-green-600"},
          { label:"Remaining",    value:e.remaining>0?`₹${e.remaining.toLocaleString()}`:"—",color:"text-blue-600"},
        ].map(s => (
          <div key={s.label} className="bg-gray-50 rounded-xl p-3 text-center">
            <p className="text-[10px] text-gray-400 mb-0.5">{s.label}</p>
      ...
      <SectionCard title="Details">
        <InfoRow label="Client"     value={e.client} />
        <InfoRow label="Talent"     value={`${e.talent} (${e.talentType})`} />
        <InfoRow label="Status"     value={<span className={`text-[11px] font-semibold px-2 py-0.5 rounded-full ${escrowStatusStyle[e.status]}`}>{e.status}</span>} />
        <InfoRow label="Milestones" value={`${e.completedMilestones}/${e.milestones} completed`} />
        <InfoRow label="Deadline"   value={e.deadline} />
        <InfoRow label="Risk"       value={e.riskLevel} />
        <InfoRow label="Disputed"   value={e.disputed?<span className="text-red-500 font-semibold text-xs">Yes</span>:<span className="text-green-600 font-semibold text-xs">No</span>} />
      </SectionCard>
      <SectionCard title="Admin Actions">
        <div className="space-y-2">
          {e.status==="Frozen" && <button style={{ ...btnWarning2, width:"100%", justifyContent:"flex-start", borderRadius:10 }}>Unfreeze Escrow</button>}
          {e.status==="Active" && <button style={{ ...btnNavy,     width:"100%", justifyContent:"flex-start", borderRadius:10 }}>Force Release Next Milestone</button>}
          {e.status!=="Completed" && <button style={{ ...btnDanger, width:"100%", justifyContent:"flex-start", borderRadius:10 }}>Freeze Escrow</button>}
          <button style={{ ...btnOutline, width:"100%", justifyContent:"flex-start", borderRadius:10 }} onClick={()=>navigate(`/admin/projects/${e.projectId}`)}>View Project</button>
        </div>
      </SectionCard>
    </div>
  );
  ...
  return (
    <div className="p-3 sm:p-5 lg:p-6">
      ...
      <PageHeader title="Escrow Status" subtitle="Full visibility into every locked, released & frozen escrow" actions={<ActionBtn label="⬇ Export" style={btnNavy} />} />

      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 mb-4 sm:mb-6">
        <StatCard label="Total Locked"   value={`₹${(totalLocked/100000).toFixed(1)}L`}   color="orange" bg="orange" border="orange" />
        <StatCard label="Total Released" value={`₹${(totalReleased/100000).toFixed(1)}L`} color="green"  bg="green"  border="green"  />
        <StatCard label="Frozen"         value={frozen.length}                              color="red"    bg="red"    border="red"    />
        <StatCard label="Active"         value={mockEscrow.filter(e=>e.status==="Active").length} color="blue" bg="blue" border="blue" />
      </div>

      {frozen.length > 0 && (
        <div className="bg-red-50 border border-red-200 rounded-xl p-3 sm:p-4 mb-4 flex items-start gap-3 flex-wrap">
          <span className="text-red-500 text-base sm:text-lg shrink-0">⚑</span>
          <div className="flex-1 min-w-0">
            <p className="text-xs sm:text-sm font-bold text-red-800">{frozen.length} escrow{frozen.length>1?"s":""} frozen — dispute pending</p>
            <p className="text-[10px] sm:text-xs text-red-600 mt-0.5">₹{frozen.reduce((s,e)=>s+e.locked,0).toLocaleString()} locked. Resolve disputes to release.</p>
          </div>
          <ActionBtn label="View Disputes" variant="danger" onClick={()=>navigate("/admin/disputes")} />
        </div>
      )}
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="p-3 sm:p-4 border-b border-gray-50">
          <div className="flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between">
            <SearchBar value={search} onChange={setSearch} placeholder="Search project, client...">
              <FilterSelect value={statusFilter} onChange={setStatus} label="All Status"
                options={["Active","Frozen","At Risk","Pending","Completed"].map(v=>({value:v,label:v}))} />
            </SearchBar>
            <span className="text-[10px] sm:text-xs text-gray-400">{filtered.length} escrows</span>
          </div>
        </div>
        ...
        <div className="fin-table-wrap">
          <Table headers={["Project","Client","Talent","Status","Total","Locked","Released","Milestones","Deadline","Actions"]}>
            {filtered.map(e => (
              <tr key={e.id} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors cursor-pointer group" onClick={()=>openDetail(e)}>
                <td className="py-3 pr-3">
                  <div className="flex items-center gap-1.5">
                    <div><p className="text-sm font-semibold text-gray-800">{e.project}</p><p className="text-[10px] text-gray-300">{e.id}</p></div>
                    {e.disputed && <span className="text-[10px] bg-red-50 text-red-500 border border-red-200 px-1.5 py-0.5 rounded-full font-semibold shrink-0">⚑</span>}
                  </div>
                </td>
                <td className="py-3 pr-3 text-sm text-gray-600">{e.client}</td>
                <td className="py-3 pr-3"><p className="text-sm text-gray-700">{e.talent}</p><p className="text-[10px] text-gray-400">{e.talentType}</p></td>
                <td className="py-3 pr-3"><span className={`text-[11px] font-semibold px-2 py-0.5 rounded-full ${escrowStatusStyle[e.status]}`}>{e.status}</span></td>
                <td className="py-3 pr-3 text-sm font-semibold text-gray-700">₹{(e.totalAmount/1000).toFixed(0)}k</td>
                <td className="py-3 pr-3">{e.locked>0?<span className="text-sm font-semibold text-orange-500">₹{(e.locked/1000).toFixed(0)}k</span>:<span className="text-xs text-gray-300">—</span>}</td>
                <td className="py-3 pr-3">{e.released>0?<span className="text-sm font-semibold text-green-600">₹{(e.released/1000).toFixed(0)}k</span>:<span className="text-xs text-gray-300">—</span>}</td>
                <td className="py-3 pr-3 text-xs text-gray-600 text-center">{e.completedMilestones}/{e.milestones}</td>
                <td className="py-3 pr-3 text-xs text-gray-500 whitespace-nowrap">{e.deadline}</td>
                <td className="py-3">
                  <div className="opacity-0 group-hover:opacity-100 transition-opacity flex gap-1.5">
                    {e.status==="Frozen" && <ActionBtn label="Unfreeze" variant="warning" style={btnWarning2} onClick={ev=>{ev.stopPropagation();}} />}
                    {e.status==="Active" && <ActionBtn label="Release"  variant="primary"  style={btnNavy}    onClick={ev=>{ev.stopPropagation();}} />}
                  </div>
                </td>
              </tr>
            ))}
        ...
        {/* Mobile Cards */}
        <div className="fin-cards-wrap p-3">
          {filtered.map(e => <MobileCard key={e.id} e={e} />)}
        </div>

        {filtered.length===0 && <div className="py-12 text-center"><p className="text-gray-400 text-sm">No escrows found</p></div>}
        <div className="px-3 sm:px-4 py-2.5 border-t border-gray-100 flex justify-between items-center flex-wrap gap-2">
          <span className="text-[10px] sm:text-xs text-gray-400">Showing {filtered.length} of {mockEscrow.length}</span>
          <span className="text-[10px] sm:text-xs font-semibold text-gray-600">Total locked: ₹{totalLocked.toLocaleString()}</span>
        </div>
      </div>

      {/* Desktop detail drawer */}
      {selected && (
        <div className="fixed inset-0 z-50 flex justify-end" onClick={()=>{setSelected(null);setShowDrawer(false);}}>
          <div className="absolute inset-0 bg-black/20" />
          <div className="relative w-full max-w-md bg-white h-full overflow-y-auto shadow-2xl" onClick={e=>e.stopPropagation()}>
            <div className="px-4 sm:px-5 py-3 sm:py-4 border-b border-gray-100 flex items-center justify-between sticky top-0 bg-white z-10">
              <div><h2 className="text-sm sm:text-base font-bold text-gray-900">{selected.project}</h2><p className="text-[10px] sm:text-xs text-gray-400">{selected.id}</p></div>
              <button onClick={()=>{setSelected(null);setShowDrawer(false);}} className="text-gray-400 hover:text-gray-600 text-lg">✕</button>
            </div>
...
export function AdminPayouts() {
  ...
  const filtered = tabFiltered.filter(p => {
    const q = search.toLowerCase();
    return (p.talent.toLowerCase().includes(q)||p.id.toLowerCase().includes(q)||p.project.toLowerCase().includes(q)) &&
           (!statusFilter || p.status===statusFilter);
  });
  ...
  const TABS = [
    { key:"all",       label:`All (${mockPayouts.length})` },
    { key:"pending",   label:`Pending (${mockPayouts.filter(p=>p.status==="Pending"||p.status==="Processing").length})` },
    { key:"hold",      label:`Hold (${mockPayouts.filter(p=>p.status==="On Hold").length})` },
    { key:"completed", label:`Done (${mockPayouts.filter(p=>p.status==="Completed").length})` },
    { key:"flagged",   label:`Flagged (${mockPayouts.filter(p=>p.flagged).length})` },
  ];
      ...
      <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:10 }}>
        ...
        <div style={{ flex:1, minWidth:0 }}>
          <div style={{ display:"flex", alignItems:"center", gap:6, flexWrap:"wrap" }}>
            <span style={{ fontSize:12, fontWeight:700, color:G.text }}>{p.talent}</span>
            {p.flagged && <span className="text-[9px] bg-red-50 text-red-500 border border-red-200 px-1.5 py-0.5 rounded-full font-semibold">⚑</span>}
          </div>
          <span style={{ fontSize:10, color:G.muted }}>{p.talentType} · {p.requestedDate}</span>
        </div>
        <div style={{ textAlign:"right", flexShrink:0 }}>
          <p style={{ fontSize:14, fontWeight:800, color:G.text, margin:0 }}>₹{p.amount.toLocaleString()}</p>
          <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full ${payoutStatusStyle[p.status]}`}>{p.status}</span>
        </div>
      </div>
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", paddingTop:8, borderTop:`1px solid ${G.greenBorder}` }}>
        <div style={{ display:"flex", gap:10, fontSize:10, color:G.muted }}>
          <span style={{ color:p.kycStatus==="Verified"?G.greenDeep:G.redText, fontWeight:600 }}>KYC: {p.kycStatus}</span>
          <span style={{ color:p.taxStatus==="Filed"?G.greenDeep:"#b45309", fontWeight:600 }}>Tax: {p.taxStatus}</span>
        </div>
        {(p.status==="Pending"||p.status==="Processing") && (
          <button style={{ ...btnNavy, padding:"4px 10px", fontSize:10 }} onClick={e=>{e.stopPropagation();}}>Approve</button>
        )}
      </div>
    </div>
  );
  ...
  return (
    <div className="p-3 sm:p-5 lg:p-6">
      ...
      <PageHeader
        ...
        actions={
          <div className="flex gap-2 flex-wrap">
            <ActionBtn label="⬇ Export"            style={btnNavy}    />
            <ActionBtn label="Approve All Pending" style={btnOutline}  />
          </div>
        }
      />

      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 mb-4 sm:mb-6">
        <StatCard label="Pending" value={mockPayouts.filter(p=>p.status==="Pending"||p.status==="Processing").length} sub={`₹${(pendingAmount/1000).toFixed(0)}k`} color="orange" bg="orange" border="orange" />
        <StatCard label="On Hold"    value={mockPayouts.filter(p=>p.status==="On Hold").length}   color="gray"  bg="gray"  border="gray"  />
        <StatCard label="Completed"  value={mockPayouts.filter(p=>p.status==="Completed").length} color="green" bg="green" border="green" />
        <StatCard label="Flagged"    value={mockPayouts.filter(p=>p.flagged).length}               color="red"   bg="red"   border="red"   />
      </div>

      <div className="fin-tabs mb-3">
        {TABS.map(tab => (
          <button key={tab.key} onClick={()=>setActiveTab(tab.key)}
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="p-3 sm:p-4 border-b border-gray-50">
          <div className="flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between">
            <SearchBar value={search} onChange={setSearch} placeholder="Search talent, payout ID...">
              <FilterSelect value={statusFilter} onChange={setStatus} label="All Status"
                options={["Pending","Processing","Completed","On Hold","Rejected"].map(v=>({value:v,label:v}))} />
            </SearchBar>
            <span className="text-[10px] sm:text-xs text-gray-400">{filtered.length} results</span>
          </div>
        </div>
        ...
        <div className="fin-table-wrap">
          <Table headers={["Payout ID","Talent","Amount","Status","Method","Bank","Project","KYC","Tax","Requested","Actions"]}>
            {filtered.map(p => (
              <tr key={p.id} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors cursor-pointer group" onClick={()=>navigate(`/admin/payouts/${p.id}`)}>
                <td className="py-3 pr-3">
                  <div className="flex items-center gap-1.5">
                    <span className="text-xs font-mono font-semibold text-gray-500">{p.id}</span>
                    {p.flagged && <span className="text-[10px] bg-red-50 text-red-500 border border-red-200 px-1.5 py-0.5 rounded-full font-semibold">⚑</span>}
                ...
                <td className="py-3 pr-3">
                  <div className="flex items-center gap-2">
                    <Avatar name={p.talent} size="sm" />
                    <div><p className="text-sm font-semibold text-gray-800">{p.talent}</p><p className="text-[10px] text-gray-400">{p.talentType}</p></div>
                  </div>
                </td>
                <td className="py-3 pr-3 text-sm font-bold text-gray-800">₹{p.amount.toLocaleString()}</td>
                <td className="py-3 pr-3"><span className={`text-[11px] font-semibold px-2 py-0.5 rounded-full ${payoutStatusStyle[p.status]}`}>{p.status}</span></td>
                <td className="py-3 pr-3 text-xs text-gray-600">{p.method}</td>
                <td className="py-3 pr-3 text-xs text-gray-500">{p.bank}</td>
                <td className="py-3 pr-3 text-xs text-gray-600 max-w-[100px] truncate">{p.project}</td>
                <td className="py-3 pr-3"><span className={`text-[11px] font-semibold ${p.kycStatus==="Verified"?"text-green-600":"text-red-500"}`}>{p.kycStatus}</span></td>
                <td className="py-3 pr-3"><span className={`text-[11px] font-semibold ${p.taxStatus==="Filed"?"text-green-600":"text-orange-500"}`}>{p.taxStatus}</span></td>
                <td className="py-3 pr-3 text-xs text-gray-500 whitespace-nowrap">{p.requestedDate}</td>
                <td className="py-3">
                  <div className="opacity-0 group-hover:opacity-100 transition-opacity flex gap-1.5">
                    {(p.status==="Pending"||p.status==="Processing") && <ActionBtn label="Approve" variant="primary" style={btnNavy} onClick={e=>{e.stopPropagation();}} />}
                    <ActionBtn label="View" style={btnOutline} onClick={e=>{e.stopPropagation();navigate(`/admin/payouts/${p.id}`);}} />
        ...
        {filtered.length===0 && <div className="py-12 text-center"><p className="text-gray-400 text-sm">No payouts found</p></div>}
        <div className="px-3 sm:px-4 py-2.5 border-t border-gray-100">
          <span className="text-[10px] sm:text-xs text-gray-400">Showing {filtered.length} of {mockPayouts.length}</span>
        </div>
      </div>
    </div>
  );
...
export function AdminPayoutDetail() {
  ...
  return (
    <div className="p-3 sm:p-5 lg:p-6">
      <style>{commonStyles}</style>
      <button onClick={()=>navigate("/admin/payouts")} className="flex items-center gap-1.5 text-xs sm:text-sm text-gray-500 hover:text-gray-800 mb-4 sm:mb-5">← All Payouts</button>
        ...
        <div className="flex items-center gap-3 sm:gap-4">
          ...
          <div>
            <div className="flex items-center gap-2 flex-wrap mb-1">
              <h1 className="text-lg sm:text-xl font-bold text-gray-900">{p.talent}</h1>
              <span className={`text-[11px] font-semibold px-2 py-0.5 rounded-full ${payoutStatusStyle[p.status]}`}>{p.status}</span>
              {p.flagged && <span className="text-[10px] bg-red-50 text-red-500 border border-red-200 px-2 py-0.5 rounded-full font-semibold">⚑ Flagged</span>}
            </div>
            <p className="text-xs sm:text-sm text-gray-500">{p.id} · {p.talentType} · {p.method}</p>
          </div>
        </div>
        <div className="text-left sm:text-right">
          <p className="text-2xl sm:text-3xl font-black text-gray-900">₹{p.amount.toLocaleString()}</p>
          <p className="text-xs text-gray-400 mt-0.5">Payout amount</p>
        </div>
      </div>
          ...
          <SectionCard title="Compliance Check">
            <div className="grid grid-cols-2 gap-2 sm:gap-3">
              {[
                ...
                <div key={c.label} className={`p-2.5 sm:p-3 rounded-xl border flex items-center justify-between ${c.ok?"bg-green-50 border-green-100":"bg-red-50 border-red-100"}`}>
                  <p className="text-[10px] sm:text-xs text-gray-500">{c.label}</p>
                  <span className={`text-[10px] sm:text-xs font-bold ${c.ok?"text-green-600":"text-red-500"}`}>{c.ok?"✓ ":"⚠ "}{c.value}</span>
                </div>
              ))}
            </div>
          </SectionCard>
          ...
          <SectionCard title="Admin Decision">
            <div className="space-y-2 mb-4">
              {[
                ...
                <button key={a.label} disabled={a.disabled}
                  className={`w-full px-3 py-2.5 rounded-lg text-xs sm:text-sm font-medium border transition-colors text-left ${
                    a.variant==="primary"?"border-green-300 bg-green-50 text-green-700 hover:bg-green-100":
                    a.variant==="warning"?"border-yellow-200 bg-yellow-50 text-yellow-700 hover:bg-yellow-100":
                    "border-red-200 bg-red-50 text-red-600 hover:bg-red-100"
                  } disabled:opacity-40 disabled:cursor-not-allowed`}>
          ...
          <SectionCard title="Admin Notes">
            <div className="space-y-2 mb-3">
              ...
              {notes.map((n,i) => (
                <div key={i} className="p-2.5 bg-gray-50 rounded-lg border border-gray-100">
                  <p className="text-xs text-gray-700">{n}</p>
                  <p className="text-[10px] text-gray-400 mt-0.5">Super Admin · Now</p>
                </div>
              ))}
            </div>
...
export function AdminRefunds() {
  ...
  const filtered = mockRefunds.filter(r => {
    const q = search.toLowerCase();
    return (r.client.toLowerCase().includes(q)||r.project.toLowerCase().includes(q)||r.id.toLowerCase().includes(q)) &&
           (!statusFilter || r.status===statusFilter);
  });

  const openDetail = (r) => { setSelected(r); setShowDrawer(true); };
  ...
  const MobileCard = ({ r }) => (
    <div style={{ background:G.white, border:`1px solid ${G.greenBorder}`, borderRadius:14, padding:14, cursor:"pointer" }}
      ...
      <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", gap:8, marginBottom:8 }}>
        <div>
          <div style={{ display:"flex", alignItems:"center", gap:5, marginBottom:3 }}>
            <span style={{ fontSize:10, fontFamily:"monospace", color:G.muted }}>{r.id}</span>
            {r.flagged && <span className="text-[9px] bg-red-50 text-red-500 border border-red-200 px-1 py-0.5 rounded-full font-semibold">⚑</span>}
          </div>
          <p style={{ fontSize:12, fontWeight:700, color:G.text, margin:0 }}>{r.client}</p>
          <p style={{ fontSize:10, color:G.muted, marginTop:1 }}>{r.project}</p>
        </div>
        <div style={{ textAlign:"right", flexShrink:0 }}>
          <p style={{ fontSize:14, fontWeight:800, color:"#f97316", margin:0 }}>₹{r.amount.toLocaleString()}</p>
          <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full ${refundStatusStyle[r.status]}`}>{r.status}</span>
        </div>
      </div>
      <div style={{ paddingTop:8, borderTop:`1px solid ${G.greenBorder}` }}>
        <p style={{ fontSize:10, color:G.muted, margin:0 }}>
          AI: <span style={{ fontWeight:600, color:r.aiRecommendation.includes("Reject")?G.redText:r.aiRecommendation.includes("Partial")?"#b45309":G.greenDeep }}>{r.aiRecommendation}</span>
        </p>
      </div>
    </div>
  );

  const RefundDetailPanel = ({ r }) => r ? (
    <div style={{ padding:16, display:"flex", flexDirection:"column", gap:16 }}>
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:8 }}>
        <div><p style={{ fontSize:13, fontWeight:700, color:G.text, margin:0 }}>{r.client}</p><p style={{ fontSize:11, color:G.muted, marginTop:2 }}>{r.project}</p></div>
        <span className={`text-[11px] font-semibold px-2 py-0.5 rounded-full ${refundStatusStyle[r.status]}`}>{r.status}</span>
      </div>
      <div style={{ padding:12, background:"#fff7ed", borderRadius:12, border:"1px solid #fed7aa", textAlign:"center" }}>
        <p style={{ fontSize:11, color:G.muted, margin:"0 0 4px" }}>Refund Requested</p>
        <p style={{ fontSize:22, fontWeight:800, color:"#f97316", margin:0 }}>₹{r.amount.toLocaleString()}</p>
        {r.escrowAvailable>0 && <p style={{ fontSize:10, color:G.muted, marginTop:4 }}>₹{r.escrowAvailable.toLocaleString()} in escrow</p>}
      </div>
      <div style={{ padding:12, background:G.blueBg, borderRadius:12, border:`1px solid ${G.blueBorder}` }}>
        <p style={{ fontSize:10, fontWeight:700, color:G.blue, marginBottom:5 }}>◎ AI Recommendation</p>
        <p style={{ fontSize:12, fontWeight:600, color:r.aiRecommendation.includes("Reject")?G.redText:r.aiRecommendation.includes("Partial")?"#92400e":G.greenDeep, margin:0 }}>{r.aiRecommendation}</p>
      </div>
      <div>
        <p style={{ fontSize:11, fontWeight:600, color:G.muted, marginBottom:6 }}>Client's Reason</p>
        <p style={{ fontSize:12, color:G.text, lineHeight:1.6, background:G.bg, borderRadius:10, padding:"10px 12px", border:`1px solid ${G.border}`, margin:0 }}>{r.reason}</p>
      </div>
      ...
      {r.disputeId && <ActionBtn label={`View Dispute ${r.disputeId}`} size="md" onClick={()=>navigate("/admin/disputes")} />}
    </div>
  ) : null;
  ...
  return (
    <div className="p-3 sm:p-5 lg:p-6">
      <style>{commonStyles}</style>
      <PageHeader title="Refund Requests" subtitle="Review & process client refund requests" actions={<ActionBtn label="⬇ Export" style={btnNavy} />} />

      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 mb-4 sm:mb-6">
        <StatCard label="Total"    value={mockRefunds.length}                                                                      color="gray"   bg="gray"   border="gray"   />
        <StatCard label="Pending"  value={mockRefunds.filter(r=>r.status==="Pending"||r.status==="Under Review").length}            color="orange" bg="orange" border="orange" />
        <StatCard label="Approved" value={mockRefunds.filter(r=>r.status==="Approved").length}                                     color="green"  bg="green"  border="green"  />
        <StatCard label="Rejected" value={mockRefunds.filter(r=>r.status==="Rejected").length}                                     color="red"    bg="red"    border="red"    />
      </div>

      <div className="fin-refund-layout">
        <div style={{ flex:1, minWidth:0 }}>
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
            <div className="p-3 sm:p-4 border-b border-gray-50">
              <div className="flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between">
                <SearchBar value={search} onChange={setSearch} placeholder="Search client, project...">
                  <FilterSelect value={statusFilter} onChange={setStatus} label="All Status"
                    options={["Pending","Under Review","Approved","Rejected"].map(v=>({value:v,label:v}))} />
                </SearchBar>
                <span className="text-[10px] sm:text-xs text-gray-400">{filtered.length} results</span>
            ...
            <div className="fin-table-wrap">
              <Table headers={["Refund ID","Client","Project","Amount","Status","AI Rec.","Dispute","Requested","Actions"]}>
                {filtered.map(r => (
                  <tr key={r.id} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors cursor-pointer group" onClick={()=>openDetail(r)}>
                    <td className="py-3 pr-3">
                      <div className="flex items-center gap-1.5">
                        <span className="text-xs font-mono font-semibold text-gray-500">{r.id}</span>
                        {r.flagged && <span className="text-[10px] bg-red-50 text-red-500 border border-red-200 px-1.5 py-0.5 rounded-full font-semibold">⚑</span>}
                    ...
                    <td className="py-3 pr-3">
                      <div className="flex items-center gap-2"><Avatar name={r.client} size="sm" /><span className="text-sm text-gray-700">{r.client}</span></div>
                    </td>
                    <td className="py-3 pr-3 text-xs text-gray-600 max-w-[130px] truncate">{r.project}</td>
                    <td className="py-3 pr-3 text-sm font-bold text-orange-500">₹{r.amount.toLocaleString()}</td>
                    <td className="py-3 pr-3"><span className={`text-[11px] font-semibold px-2 py-0.5 rounded-full ${refundStatusStyle[r.status]}`}>{r.status}</span></td>
                    <td className="py-3 pr-3 text-xs font-semibold" style={{ color:r.aiRecommendation.includes("Reject")?"#ef4444":r.aiRecommendation.includes("Partial")?"#b45309":G.greenDeep }}>{r.aiRecommendation}</td>
                    <td className="py-3 pr-3 text-xs text-gray-500">{r.disputeId||"—"}</td>
                    <td className="py-3 pr-3 text-xs text-gray-500 whitespace-nowrap">{r.requestedDate}</td>
                    <td className="py-3">
                      <div className="opacity-0 group-hover:opacity-100 transition-opacity">
                        <ActionBtn label="Review" variant="primary" style={btnNavy} onClick={e=>{e.stopPropagation();openDetail(r);}} />
                      </div>
                    </td>
                  </tr>
                ))}
            ...
            {filtered.length===0 && <div className="py-12 text-center"><p className="text-gray-400 text-sm">No refund requests found</p></div>}
            <div className="px-3 sm:px-4 py-2.5 border-t border-gray-100"><span className="text-[10px] sm:text-xs text-gray-400">Showing {filtered.length} of {mockRefunds.length}</span></div>
          </div>
        </div>

        {/* Desktop sticky panel */}
        <div className="fin-refund-panel">
          {selected ? (
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden sticky top-6">
              ...
              <RefundDetailPanel r={selected} />
            </div>
          ) : (
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-8 text-center sticky top-6">
              <div className="w-10 h-10 rounded-full bg-gray-100 flex items-center justify-center mx-auto mb-3 text-gray-400 text-lg">◎</div>
              <p className="text-xs text-gray-400">Select a refund to review</p>
            </div>
          )}
        </div>
      </div>
      ...
      {/* Mobile drawer */}
      {showDrawer && selected && (
        <div className="fin-drawer-backdrop" onClick={()=>setShowDrawer(false)}>
          <div className="fin-drawer" onClick={e=>e.stopPropagation()}>
            <div className="fin-drawer-handle" />
            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"0 16px 12px" }}>
              <p style={{ fontSize:13, fontWeight:700, color:G.text, margin:0 }}>Refund Review</p>
              <button onClick={()=>setShowDrawer(false)} style={{ background:"none", border:"none", fontSize:16, color:G.muted, cursor:"pointer" }}>✕</button>
            </div>
            <RefundDetailPanel r={selected} />
          </div>
        </div>
...
export function AdminCommission() {
  ...
  return (
    <div className="p-3 sm:p-5 lg:p-6">
      ...
      <PageHeader
        title="Platform Commission"
        subtitle="Weblance earnings — 6% on all completed transactions"
        actions={
          <div className="flex gap-2 flex-wrap">
            <ActionBtn label="⬇ Export" style={btnNavy} />
            <ActionBtn label="Rules →"  style={btnOutline} />
          </div>
        }
      />

      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 mb-4 sm:mb-6">
        <StatCard label="Lifetime"    value={`₹${(mockCommission.totalLifetime/1000).toFixed(0)}k`} color="green" bg="green" border="green" />
        <StatCard label="This Month"  value={`₹${(mockCommission.thisMonth/1000).toFixed(1)}k`}     color="green" bg="green" border="green" sub="Mar 2026" />
        <StatCard label="Last Month"  value={`₹${(mockCommission.lastMonth/1000).toFixed(0)}k`}     color="blue"  bg="blue"  border="blue"  sub="Feb 2026" />
        <StatCard label="Rate"        value={`${mockCommission.rate}%`}                              color="gray"  bg="gray"  border="gray"  sub="Fixed rate" />
      </div>
      ...
      <div className="comm-grid">
            ...
            <div className="flex items-end gap-2 sm:gap-3 h-32 sm:h-40 px-1 sm:px-2">
              {mockCommission.monthly.map((m,i) => (
                <div key={i} className="flex-1 flex flex-col items-center gap-1">
                  <p className="text-[9px] sm:text-xs font-bold text-green-600">₹{(m.amount/1000).toFixed(0)}k</p>
                  <div className="w-full bg-gray-100 rounded-t-lg overflow-hidden flex items-end" style={{ height:"80px" }}>
                    <div className={`w-full rounded-t-lg ${i===mockCommission.monthly.length-1?"bg-green-500":"bg-green-300"}`}
                      style={{ height:`${(m.amount/maxBar)*100}%` }} />
                  </div>
                  <p className="text-[8px] sm:text-[10px] text-gray-400 text-center">{m.month.split(" ")[0]}</p>
                </div>
              ))}
            </div>
            <div className="mt-3 sm:mt-4 pt-3 sm:pt-4 border-t border-gray-100 flex items-center justify-between flex-wrap gap-3">
              <div><p className="text-xs text-gray-400">Total this year</p><p className="text-base sm:text-lg font-bold text-green-600">₹{mockCommission.thisYear.toLocaleString()}</p></div>
              <div className="text-right"><p className="text-xs text-gray-400">MoM growth</p><p className="text-base sm:text-lg font-bold text-gray-800">{`${(((mockCommission.thisMonth-mockCommission.lastMonth)/mockCommission.lastMonth)*100).toFixed(0)}%`}</p></div>
            </div>
          </SectionCard>
            ...
            <div className="space-y-2">
              {mockCommission.byProject.map((p,i) => (
                <div key={i} className="flex items-center gap-3 sm:gap-4 py-2.5 border-b border-gray-50 last:border-0">
                  <div className="flex-1 min-w-0">
                    <p className="text-xs sm:text-sm font-semibold text-gray-800 truncate">{p.project}</p>
                    <p className="text-[10px] sm:text-xs text-gray-400">{p.client} · {p.date}</p>
                  </div>
                  <div className="text-right shrink-0">
                    <p className={`text-xs sm:text-sm font-bold ${p.status==="Collected"?"text-green-600":p.status==="Frozen"?"text-red-500":"text-orange-500"}`}>₹{p.amount.toLocaleString()}</p>
                    <span className={`text-[9px] sm:text-[10px] font-semibold px-1.5 py-0.5 rounded-full ${p.status==="Collected"?"bg-green-50 text-green-700 border border-green-200":p.status==="Frozen"?"bg-red-50 text-red-600 border border-red-200":"bg-orange-50 text-orange-600 border border-orange-200"}`}>{p.status}</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="space-y-4 sm:space-y-5">
          ...
          <SectionCard title="Revenue Summary">
            {[
              { label:"Collected",  value:`₹${mockCommission.byProject.filter(p=>p.status==="Collected").reduce((s,p)=>s+p.amount,0).toLocaleString()}`, color:"text-green-600"  },
              { label:"Pending",    value:`₹${mockCommission.byProject.filter(p=>p.status==="Pending").reduce((s,p)=>s+p.amount,0).toLocaleString()}`,    color:"text-orange-500" },
              { label:"Frozen",     value:`₹${mockCommission.byProject.filter(p=>p.status==="Frozen").reduce((s,p)=>s+p.amount,0).toLocaleString()}`,     color:"text-red-500"    },
            ].map(s => (
              <div key={s.label} className="flex items-center justify-between py-2.5 border-b border-gray-50 last:border-0">
                <span className="text-xs text-gray-400">{s.label}</span>
                <span className={`text-sm font-bold ${s.color}`}>{s.value}</span>
          ...
          <SectionCard title="Commission Rules">
            <div className="space-y-2 sm:space-y-3">
              {[
                { plan:"All Plans",  rate:"6%", desc:"Standard commission" },
                { plan:"Elite++",    rate:"4%", desc:"Top freelancer benefit" },
                { plan:"Enterprise", rate:"5%", desc:"Large volume clients" },
              ].map(r => (
                <div key={r.plan} className="flex items-center justify-between p-2.5 sm:p-3 bg-gray-50 rounded-lg border border-gray-100">
                  <div><p className="text-xs sm:text-sm font-semibold text-gray-800">{r.plan}</p><p className="text-[10px] sm:text-xs text-gray-400">{r.desc}</p></div>
                  <span className="text-base sm:text-lg font-black text-green-600">{r.rate}</span>
                </div>
              ))}
            </div>
            <div className="mt-3"><ActionBtn label="Edit Commission Rules" size="md" /></div>
          </SectionCard>
        </div>
      </div>
    </div>
