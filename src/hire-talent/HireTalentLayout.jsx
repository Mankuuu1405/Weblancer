...
import {
  ...
  CreditCard, Flag, Bell, Star, Menu, X,
} from 'lucide-react';
...
const styles = `
  ...
  .htl-overlay.open { display: block; }
...
const PAGE_TITLES = {
  ...
  '/hire-talent/post-project':  'Post New Project',
};
...
export default function HireTalentLayout() {
  ...
  // Resolve page title — handle dynamic routes like /hire-talent/projects/1
  let pageTitle = 'Hire Talent';
  ...
  // More specific match for project detail
  if (/^\/hire-talent\/projects\/\d+$/.test(location.pathname)) pageTitle = 'Project Details';
  if (/^\/hire-talent\/payments\//.test(location.pathname)) pageTitle = 'Transaction Details';

  const handleNavClick = (route) => {
    navigate(route);
    setMobileOpen(false); // close on mobile after nav
  };
  ...
  return (
    <>
      ...
      <div className="htl-root">
        ...
        {/* Mobile overlay */}
        <div
          className={`htl-overlay${mobileOpen ? ' open' : ''}`}
          onClick={() => setMobileOpen(false)}
        />

        {/* ── SIDEBAR ── */}
        <div className={`htl-sidebar${collapsed ? ' col' : ''}${mobileOpen ? ' mobile-open' : ''}`}>
              ...
              <div key={sec.label}>
                ...
                {sec.items.map(item => {
                  ...
                  const isActive = location.pathname === item.route ||
                    (item.route !== '/hire-talent/payments' && location.pathname.startsWith(item.route + '/'));
                  return (
        ...
        <div className="htl-main">
          <div className="htl-topbar">
            <div style={{ display: 'flex', alignItems: 'center' }}>
              ...
              <button
                className="htl-menu-btn"
                onClick={() => setMobileOpen(true)}
              >
                <Menu size={18} color="#1e3a5f" />
              </button>
              <div>
      ...
      <style>{`
        @media (max-width: 768px) {
          .htl-mobile-close { display: flex !important; }
        }
      `}</style>
    </>
  );
