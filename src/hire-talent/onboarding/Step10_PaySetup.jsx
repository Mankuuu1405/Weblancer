...
const PAYMENT_METHODS = [
  { id:"card",   label:"Credit/Debit Card", icon:<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><rect x="1" y="4" width="22" height="16" rx="2" strokeWidth="1.8"/><path d="M1 10h22" strokeWidth="1.8" strokeLinecap="round"/></svg> },
  { id:"bank",   label:"Bank Transfer",     icon:<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M3 21h18M3 10h18M5 6l7-3 7 3M4 10v11M8 10v11M12 10v11M16 10v11M20 10v11" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/></svg> },
  { id:"paypal", label:"PayPal",            icon:<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><rect x="2" y="5" width="20" height="14" rx="2" strokeWidth="1.8"/><path d="M2 10h20" strokeWidth="1.8" strokeLinecap="round"/><circle cx="7" cy="15" r="1" fill="currentColor"/><circle cx="10" cy="15" r="1" fill="currentColor"/></svg> },
  { id:"other",  label:"Other",             icon:<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><rect x="2" y="5" width="20" height="14" rx="2" strokeWidth="1.8"/><path d="M6 12h4m4 0h4" strokeWidth="1.8" strokeLinecap="round"/></svg> },
];

const SECURITY_ITEMS = [
  { icon:"🔒", text:"256-bit Encryption" },{ icon:"💳", text:"PCI-DSS Level 1" },
  { icon:"🏛️", text:"Tokenization" },      { icon:"💧", text:"AI Fraud Detection" },
];

const inputCls = "px-4 py-3 rounded-xl text-sm outline-none transition-all";
...
export default function Step10_PaySetup({ formData, updateData, next, prev }) {
  ...
  const iStyle = { border:`1px solid ${G.greenBorder}`, backgroundColor:"white", color:"#1e293b" };
  const fo = e => e.target.style.borderColor = G.navyLight;
  const bl = e => e.target.style.borderColor = G.greenBorder;

  const handleCard   = e => { const r=e.target.value.replace(/\D/g,"").slice(0,16); setCardNumber(r.replace(/(.{4})/g,"$1 ").trim()); };
  const handleExpiry = e => { const r=e.target.value.replace(/\D/g,"").slice(0,4); setExpiry(r.length>2?r.slice(0,2)+"/"+r.slice(2):r); };

  const canContinue = billingName&&billingEmail&&street&&city&&zip&&
    (payMethod!=="card"||(cardNumber.replace(/\s/g,"").length===16&&expiry.length===5&&cvv.length>=3));

  const handleNext = () => { updateData({ billing:{ billingName, billingEmail, street, city, state, zip }, payMethod }); next(); };
      ...
      <div className="flex-1 min-w-0">
        <div className="rounded-2xl overflow-hidden" style={{ backgroundColor:"white", border:`1px solid ${G.greenBorder}`, boxShadow:"0 4px 24px rgba(110,192,48,0.08)" }}>
          ...
          <div className="p-8 sm:p-10">
            <h1 className="text-2xl font-extrabold mb-1" style={{ color:G.navyDeep }}>Set Up Your Payment Profile</h1>
            <p className="text-sm mb-6" style={{ color:G.sub }}>No money will be charged yet — this confirms you're ready to pay when hiring</p>

            {/* Badge — Green */}
            <span className="inline-block text-xs font-bold tracking-widest uppercase px-3 py-1 rounded mb-6"
              style={{ backgroundColor:G.greenBg, color:G.greenDeep, border:`1px solid ${G.greenBorder}` }}>Payment Setup</span>

            {/* Notice */}
            <div className="flex items-center gap-3 px-5 py-3.5 rounded-xl mb-7"
              style={{ backgroundColor:G.navyBg, border:`1px solid ${G.navyBorder}` }}>
              <svg className="w-4 h-4 flex-shrink-0" fill="none" stroke={G.navyLight} viewBox="0 0 24 24"><path d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/></svg>
              <span className="text-sm" style={{ color:G.navy }}>Your card will NOT be charged until you approve a milestone. This is just verification.</span>
            </div>

            {/* Billing Info */}
            <h2 className="text-base font-bold mb-4" style={{ color:G.navyDeep }}>Billing Information</h2>
            <div className="flex flex-col gap-4 mb-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {[{ label:"Billing Name *", val:billingName, set:setBillingName, ph:"Full legal name", type:"text" },
                  { label:"Billing Email *", val:billingEmail, set:setBillingEmail, ph:"billing@company.com", type:"email" }].map(f=>(
                  <div key={f.label} className="flex flex-col gap-1.5">
                    <label className="text-sm font-semibold" style={{ color:G.navyDeep }}>{f.label}</label>
                    <input type={f.type} placeholder={f.ph} value={f.val} onChange={e=>f.set(e.target.value)}
                      className={`${inputCls}`} style={iStyle} onFocus={fo} onBlur={bl}/>
                  </div>
                ))}
              </div>
              <div className="flex flex-col gap-1.5">
                <label className="text-sm font-semibold" style={{ color:G.navyDeep }}>Street Address *</label>
                <input type="text" placeholder="123 Main St" value={street} onChange={e=>setStreet(e.target.value)}
                  className={`w-full ${inputCls}`} style={iStyle} onFocus={fo} onBlur={bl}/>
              </div>
              <div className="grid grid-cols-3 gap-4">
                {[{ label:"City *", val:city, set:setCity },{ label:"State", val:state, set:setState },{ label:"Zip *", val:zip, set:setZip }].map(f=>(
                  <div key={f.label} className="flex flex-col gap-1.5">
                    <label className="text-sm font-semibold" style={{ color:G.navyDeep }}>{f.label}</label>
            ...
            {/* Payment Method */}
            <h2 className="text-base font-bold mb-4" style={{ color:G.navyDeep }}>Payment Method</h2>
            <div className="grid grid-cols-4 gap-3 mb-6">
              {PAYMENT_METHODS.map(m=>{
                const sel = payMethod===m.id;
                return (
                  <button key={m.id} onClick={()=>setPayMethod(m.id)}
                    className="flex flex-col items-center gap-2 py-4 rounded-xl transition-all text-sm font-semibold"
                    style={{ border:sel?`2px solid ${G.navyLight}`:`1px solid ${G.greenBorder}`, backgroundColor:sel?G.navyBg:"white", color:sel?G.navy:G.sub }}>
                    {m.icon}
                    <span className="text-xs text-center leading-tight">{m.label}</span>
                  </button>
                );
            ...
            {payMethod==="card" && (
              <div className="rounded-xl p-6 flex flex-col gap-5 mb-6"
                ...
                <div className="flex flex-col gap-1.5">
                  <label className="text-sm font-semibold" style={{ color:G.navyDeep }}>Card Number *</label>
                  <input type="text" placeholder="•••• •••• •••• ••••" value={cardNumber} onChange={handleCard}
                    className={`w-full ${inputCls} tracking-widest`} style={iStyle} onFocus={fo} onBlur={bl}/>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div className="flex flex-col gap-1.5">
                    <label className="text-sm font-semibold" style={{ color:G.navyDeep }}>Expiration *</label>
                    <input type="text" placeholder="MM/YY" value={expiry} onChange={handleExpiry}
                      className={inputCls} style={iStyle} onFocus={fo} onBlur={bl}/>
                  </div>
                  <div className="flex flex-col gap-1.5">
                    <label className="text-sm font-semibold" style={{ color:G.navyDeep }}>CVV *</label>
                    <input type="password" placeholder="•••" maxLength={4} value={cvv}
                      onChange={e=>setCvv(e.target.value.replace(/\D/g,""))}
                      className={inputCls} style={iStyle} onFocus={fo} onBlur={bl}/>
                    <span className="text-xs" style={{ color:G.muted }}>Not stored</span>
                  </div>
                </div>
                {/* Verify button */}
                <button className="w-full flex items-center justify-center gap-2 py-3.5 rounded-full text-sm font-bold"
                  style={{ background:G.gradNavy, color:"white", opacity:0.7, cursor:"default" }}>
                  <svg className="w-4 h-4" fill="none" stroke="white" viewBox="0 0 24 24"><path d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
                  Verify Card ($1 micro-authorization, refunded immediately)
                </button>
              </div>
            )}
            ...
            {/* Security */}
            <div className="rounded-xl px-6 py-5" style={{ border:`1px solid ${G.navyBorder}`, backgroundColor:G.navyBg }}>
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="text-sm font-bold" style={{ color:G.navyDeep }}>How we protect your information</span>
              </div>
              <div className="grid grid-cols-2 gap-2">
                {SECURITY_ITEMS.map(s=>(
                  <div key={s.text} className="flex items-center gap-2 text-sm" style={{ color:G.sub }}><span>{s.icon}</span>{s.text}</div>
                ))}
              </div>
            </div>
          </div>
