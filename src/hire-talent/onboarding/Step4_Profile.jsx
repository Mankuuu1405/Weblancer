...
const TIMEZONES = [
  "(GMT-8) Pacific Time","(GMT-7) Mountain Time","(GMT-6) Central Time",
  "(GMT-5) Eastern Time","(GMT+0) London","(GMT+1) Berlin",
  "(GMT+5:30) Mumbai","(GMT+8) Singapore","(GMT+9) Tokyo",
];
...
export default function Step4_Profile({ formData, updateData, next, prev }) {
  ...
  const strength = useMemo(()=>{
    let pts=0;
    if (timezone) pts+=20; if (languages.length) pts+=20;
    if (website.trim()) pts+=20; if (linkedin.trim()) pts+=20;
    if (bio.trim().length>10) pts+=20;
    return pts;
  },[timezone,languages,website,linkedin,bio]);
  ...
  const toggleLang = (lang) => {
    setLanguages(prev=>prev.includes(lang)?prev.filter(l=>l!==lang):prev.length<3?[...prev,lang]:prev);
  };

  const canContinue = !!timezone && languages.length>0;
  const handleNext  = () => { updateData({ timezone, languages, website, linkedin, bio }); next(); };

  const insight = strength===100 ? "Great job! Your profile is 100% complete."
    : strength>=60 ? "Almost there — a few more fields will boost your trust score."
    : "Complete your profile for better talent matching.";
      ...
      <div className="flex-1 min-w-0">
        <div className="rounded-2xl overflow-hidden" style={{ backgroundColor:"white", border:`1px solid ${G.greenBorder}`, boxShadow:"0 4px 24px rgba(110,192,48,0.08)" }}>
          ...
          <div className="p-8 sm:p-10">
            <h1 className="text-2xl font-extrabold mb-1" style={{ color:G.navyDeep }}>Complete Your Profile</h1>
            <p className="text-sm mb-6" style={{ color:G.sub }}>Better profiles get faster, higher-quality responses</p>

            {/* Profile Strength bar */}
            <div className="rounded-xl px-5 py-4 mb-8" style={{ border:`1px solid ${G.navyBorder}`, backgroundColor:G.navyBg }}>
              <div className="flex items-center justify-between mb-2">
                <span className="text-sm font-semibold" style={{ color:G.navyDeep }}>Profile Strength</span>
                <span className="text-sm font-bold" style={{ color:strength===100?G.green:G.navyLight }}>{strength}%</span>
              </div>
              <div className="w-full h-2 rounded-full mb-3" style={{ backgroundColor:"#e2e8f0" }}>
                <div className="h-full rounded-full transition-all duration-500"
                  style={{ width:`${strength}%`, background:strength===100?G.green:G.gradNavy }}/>
              </div>
              <div className="flex flex-wrap gap-2">
                {STRENGTH_FIELDS.map(f=>(
                  <span key={f.key} className="flex items-center gap-1 text-xs px-2 py-0.5 rounded-full"
                    style={{
                      backgroundColor: completedKeys[f.key]?G.greenBg:G.navyBg,
                      color:           completedKeys[f.key]?G.greenDeep:G.navy,
                      border:`1px solid ${completedKeys[f.key]?G.greenBorder:G.navyBorder}`,
                    }}>
                    <span>{completedKeys[f.key]?"●":"○"}</span>{f.label}
                  </span>
                ))}
              </div>
            </div>

            <h2 className="text-base font-bold mb-5" style={{ color:G.navyDeep }}>Work Preferences</h2>

            {/* Timezone */}
            <div className="flex flex-col gap-1.5 mb-6">
              <label className="text-sm font-semibold" style={{ color:G.navyDeep }}>Preferred Timezone *</label>
              <div className="relative">
                <select value={timezone} onChange={e=>setTimezone(e.target.value)}
                  className="w-full px-4 py-3 rounded-xl text-sm outline-none appearance-none transition-all"
                  style={{ ...inputStyle, color:timezone?"#1e293b":G.muted }}
                  onFocus={e=>e.target.style.borderColor=G.navyLight} onBlur={e=>e.target.style.borderColor=G.greenBorder}>
                  <option value="" disabled>Select timezone</option>
                  {TIMEZONES.map(tz=><option key={tz} value={tz} style={{ color:"#1e293b" }}>{tz}</option>)}
                </select>
            ...
            <div className="flex flex-col gap-2 mb-8">
              <div className="flex items-center justify-between">
                <label className="text-sm font-semibold" style={{ color:G.navyDeep }}>
                  Preferred Languages * <span style={{ color:G.muted, fontWeight:400 }}>(up to 3)</span>
                </label>
                <span className="text-sm" style={{ color:G.muted }}>{languages.length}/3</span>
              </div>
              <div className="flex flex-wrap gap-2">
                {LANGUAGES.map(lang=>{
                  const active = languages.includes(lang);
                  return (
                    <button key={lang} onClick={()=>toggleLang(lang)}
                      className="px-4 py-2 rounded-xl text-sm font-medium transition-all"
                      style={{
                        border:          active?`2px solid ${G.navyLight}`:`1px solid ${G.greenBorder}`,
                        backgroundColor: active?G.navyBg:"white",
            ...
            {/* Optional Details */}
            <div className="flex items-center gap-2 mb-5">
              <h2 className="text-base font-bold" style={{ color:G.navyDeep }}>Optional Details</h2>
              <span className="text-sm" style={{ color:G.muted }}>(Strongly Encouraged)</span>
            </div>

            <div className="flex flex-col gap-5">
              {/* Website */}
              <div className="flex flex-col gap-1.5">
                <label className="text-sm font-semibold flex items-center gap-2" style={{ color:G.navyDeep }}>
                  <svg className="w-4 h-4" fill="none" stroke={G.navyLight} viewBox="0 0 24 24"><path d="M3.055 11H5a2 2 0 012 2v1a2 2 0 002 2 2 2 0 012 2v2.945M8 3.935V5.5A2.5 2.5 0 0010.5 8h.5a2 2 0 012 2 2 2 0 104 0 2 2 0 012-2h1.064M15 20.488V18a2 2 0 012-2h3.064M21 12a9 9 0 11-18 0 9 9 0 0118 0z" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/></svg>
                  Company Website
                  <span className="text-xs font-semibold" style={{ color:G.green }}>+10 trust</span>
                </label>
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-sm font-semibold flex items-center gap-2" style={{ color:G.navyDeep }}>
                  ...
                  LinkedIn Profile
                  <span className="text-xs font-semibold" style={{ color:G.green }}>+15 trust</span>
                </label>
              ...
              {/* Bio */}
              <div className="flex flex-col gap-1.5">
                <label className="text-sm font-semibold" style={{ color:G.navyDeep }}>Professional Bio</label>
                <textarea rows={4} placeholder="Tell us about your business and what you're looking to achieve"
                  value={bio} onChange={e=>{ if(e.target.value.length<=200) setBio(e.target.value); }}
                  className="w-full px-4 py-3 rounded-xl text-sm outline-none resize-none transition-all" style={inputStyle}
                  onFocus={e=>e.target.style.borderColor=G.navyLight} onBlur={e=>e.target.style.borderColor=G.greenBorder}/>
                <div className="text-xs text-right" style={{ color:G.muted }}>{bio.length}/200</div>
              </div>

              {/* Benefits */}
              <div className="rounded-xl px-6 py-5" style={{ border:`1px solid ${G.navyBorder}`, backgroundColor:G.navyBg }}>
                <p className="text-sm font-bold mb-4" style={{ color:G.navyDeep }}>Benefits of completing your profile:</p>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {[
                    { text:"Better talent matching",         icon:<svg className="w-4 h-4 flex-shrink-0" fill="none" stroke={G.navyLight} viewBox="0 0 24 24"><path d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" strokeWidth="1.8"/><path d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" strokeWidth="1.8"/></svg> },
                    { text:"Faster responses",               icon:<svg className="w-4 h-4 flex-shrink-0" fill="none" stroke={G.navyLight} viewBox="0 0 24 24"><path d="M13 10V3L4 14h7v7l9-11h-7z" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/></svg> },
                    { text:"Improved communication",         icon:<svg className="w-4 h-4 flex-shrink-0" fill="none" stroke={G.navyLight} viewBox="0 0 24 24"><path d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/></svg> },
                    { text:"Higher trust from providers",    icon:<svg className="w-4 h-4 flex-shrink-0" fill="none" stroke={G.navyLight} viewBox="0 0 24 24"><path d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/></svg> },
                  ].map(b=>(
                    <div key={b.text} className="flex items-center gap-2 text-sm" style={{ color:G.sub }}>{b.icon}{b.text}</div>
                  ))}
                </div>
              </div>
            </div>
