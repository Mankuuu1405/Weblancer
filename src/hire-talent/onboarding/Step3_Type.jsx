...
const TYPES = [
  {
    id:"individual",
    icon:<svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/></svg>,
    title:"Individual",
    desc:"Personal projects, side hustles, or one-time needs",
    examples:"Portfolio website, mobile app idea, design work",
    badge:"Simplified process",
    badgeStyle:{ bg:G.navyBg, color:G.navy },
    insight:"Individual accounts have a simplified, faster process.",
  },
  {
    id:"startup",
    icon:<svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M13 10V3L4 14h7v7l9-11h-7z" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/></svg>,
    title:"Startup / Small Business",
    desc:"Growing business with ongoing or scalable needs",
    examples:"MVP development, brand identity, marketing platform",
    badge:"Growth-focused",
    badgeStyle:{ bg:G.greenBg, color:G.greenDeep },
    insight:"Startup accounts get access to vetted growth-focused freelancers.",
  },
  {
    id:"enterprise",
    icon:<svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/></svg>,
    title:"Enterprise / Company",
    desc:"Established organization with complex requirements",
    examples:"Enterprise software, internal tools, large-scale projects",
    badge:"Priority support",
    badgeStyle:{ bg:"#fef3c7", color:"#d97706" },
    insight:"Enterprise clients get a dedicated account manager and priority support.",
  },
];

const HOW_HELPS = [
  { text:"Better project complexity matching",   icon:<svg className="w-4 h-4 flex-shrink-0" fill="none" stroke={G.navyLight} viewBox="0 0 24 24"><path d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/></svg> },
  { text:"Accurate budget recommendations",      icon:<svg className="w-4 h-4 flex-shrink-0" fill="none" stroke={G.navyLight} viewBox="0 0 24 24"><path d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/></svg> },
  { text:"Appropriate talent suggestions",       icon:<svg className="w-4 h-4 flex-shrink-0" fill="none" stroke={G.navyLight} viewBox="0 0 24 24"><path d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" strokeWidth="1.8"/><path d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" strokeWidth="1.8"/></svg> },
  { text:"Faster admin response",                icon:<svg className="w-4 h-4 flex-shrink-0" fill="none" stroke={G.navyLight} viewBox="0 0 24 24"><path d="M13 10V3L4 14h7v7l9-11h-7z" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/></svg> },
];

export default function Step3_Type({ formData, updateData, next, prev }) {
  const [selected, setSelected] = useState(formData?.clientType||"");
  const selectedType = TYPES.find(t=>t.id===selected);
  const handleNext = () => { updateData({ clientType:selected }); next(); };
    ...
    <div className="flex gap-6 items-start">
      ...
      <div className="flex-1 min-w-0">
        <div className="rounded-2xl overflow-hidden" style={{ backgroundColor:"white", border:`1px solid ${G.greenBorder}`, boxShadow:"0 4px 24px rgba(110,192,48,0.08)" }}>
          ...
          <div className="p-8 sm:p-10">
            <h1 className="text-2xl font-extrabold mb-1" style={{ color:G.navyDeep }}>Help Us Understand Your Needs</h1>
            <p className="text-sm mb-8" style={{ color:G.sub }}>This helps us recommend the right talent and support level</p>
              ...
              {TYPES.map(t=>{
                ...
                return (
                  <button key={t.id} onClick={()=>setSelected(t.id)}
                    className="relative flex flex-col gap-3 p-6 rounded-2xl text-left transition-all"
                    style={{
                      border: isSel?`2px solid ${G.navyLight}`:`1px solid ${G.greenBorder}`,
                      backgroundColor: isSel?G.navyBg:"white",
                      boxShadow: isSel?`0 0 0 3px rgba(74,111,165,0.12)`:"none",
                    }}>
                    {isSel && (
                      <span className="absolute top-3 right-3" style={{ color:G.navyLight }}>
                    ...
                    <div className="w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0"
                      style={{ background:isSel?G.gradNavy:G.navyBg, color:isSel?"white":G.navyLight }}>
                      {t.icon}
                    </div>
                    <div className="font-extrabold text-base" style={{ color:G.navyDeep }}>{t.title}</div>
                    <div className="text-sm" style={{ color:G.sub }}>{t.desc}</div>
                    <div className="text-xs italic" style={{ color:G.muted }}>{t.examples}</div>
                    <span className="inline-block text-xs font-semibold px-3 py-1 rounded-full w-fit mt-1"
            ...
            <div className="rounded-xl px-6 py-5" style={{ border:`1px solid ${G.navyBorder}`, backgroundColor:G.navyBg }}>
              <p className="text-sm font-bold mb-4" style={{ color:G.navyDeep }}>How this helps you:</p>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {HOW_HELPS.map(h=>(
                  <div key={h.text} className="flex items-center gap-2.5 text-sm" style={{ color:G.sub }}>{h.icon}{h.text}</div>
                ))}
              </div>
            </div>
          </div>
        ...
        <div className="flex items-center justify-between mt-6 pb-10">
          ...
          <button onClick={handleNext} disabled={!selected}
            ...
            style={{ background:selected?G.gradNavy:"#d1d5db", boxShadow:selected?"0 3px 14px rgba(15,26,59,0.30)":"none", cursor:selected?"pointer":"not-allowed" }}
      ...
      <div className="hidden lg:block w-72 flex-shrink-0 sticky top-24">
        <div className="rounded-2xl p-5" style={{ backgroundColor:"white", border:`1px solid ${G.greenBorder}`, boxShadow:"0 2px 12px rgba(110,192,48,0.06)" }}>
          ...
          {!selected
            ? <p className="text-xs" style={{ color:G.muted }}>Select your client type to see tailored recommendations.</p>
            : <div className="flex items-start gap-3 px-4 py-3 rounded-xl text-sm"
