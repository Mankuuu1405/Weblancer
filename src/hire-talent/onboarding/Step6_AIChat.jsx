...
const FLOW = [
  { ai:"Thanks for sharing your idea! Let me ask a few questions. First — is this for web, mobile, or both?", chips:["Web only","Mobile only","Both web and mobile","Not sure yet"] },
  { ai:"Got it! Do you already have a design or wireframe in mind?", chips:["Yes, I have designs","Just rough sketches","No, starting from scratch","Need help with design too"] },
  { ai:"Great. What's your rough timeline for this project?", chips:["ASAP (under 1 month)","1–3 months","3–6 months","Flexible / ongoing"] },
  { ai:"Almost done! Do you have a budget range in mind?", chips:["Under ₹50,000","₹50k – ₹2L","₹2L – ₹10L","₹10L+","Not sure yet"] },
  { ai:"Perfect! I have enough context to generate your project blueprint. Click 'Generate Blueprint' when ready.", chips:[] },
];

export default function Step6_AIChat({ formData, updateData, next, prev }) {
  const [messages,     setMessages]     = useState([{ role:"ai", text:FLOW[0].ai, chips:FLOW[0].chips }]);
  const [input,        setInput]        = useState("");
  const [flowIdx,      setFlowIdx]      = useState(0);
  const [understanding,setUnderstanding]= useState(0);
  const chatRef = useRef(null);
  ...
  const advance = userText => {
    ...
    const newPct = Math.min(Math.round((next_/(FLOW.length-1))*100),100);
    setMessages(prev=>{
      const updated = prev.map((m,i)=>i===prev.length-1&&m.role==="ai"?{...m,chips:[]}:m);
      const withUser = [...updated,{ role:"user", text:userText }];
      if (next_<FLOW.length) return [...withUser,{ role:"ai", text:FLOW[next_].ai, chips:FLOW[next_].chips }];
      return withUser;
    });
    setFlowIdx(next_); setUnderstanding(newPct); setInput("");
  };

  const handleChip = chip => advance(chip);
  const handleSend = () => { if(!input.trim()) return; advance(input.trim()); };
  const handleKeyDown = e => { if(e.key==="Enter"&&!e.shiftKey){ e.preventDefault(); handleSend(); } };

  const canGenerate = flowIdx >= FLOW.length-1;
  const handleNext  = () => { updateData({ aiChat:messages }); next(); };
    ...
    <div className="flex gap-6 items-start">
      <div className="flex-1 min-w-0">
        <div className="rounded-2xl overflow-hidden" style={{ backgroundColor:"white", border:`1px solid ${G.greenBorder}`, boxShadow:"0 4px 24px rgba(110,192,48,0.08)" }}>
          ...
          <div className="px-8 pt-8 pb-4">
            <h1 className="text-2xl font-extrabold mb-1" style={{ color:G.navyDeep }}>Let's Refine Your Idea</h1>
            <p className="text-sm mb-5" style={{ color:G.sub }}>AI will ask a few questions to understand your project better</p>
            <span className="inline-block text-xs font-bold tracking-widest uppercase px-3 py-1 rounded mb-4"
          ...
          {/* Chat area */}
          <div ref={chatRef} className="px-6 pb-4 flex flex-col gap-5 overflow-y-auto"
            style={{ minHeight:"320px", maxHeight:"420px" }}>
            {messages.map((msg,i)=>(
              <div key={i}>
                {msg.role==="ai"
                  ? <div className="flex items-start gap-3">
                      {/* AI avatar — Navy gradient */}
                      <div className="w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0" style={{ background:G.gradNavy }}>
                      ...
                      <div className="flex flex-col gap-3">
                        {/* AI bubble — Navy bg */}
                        <div className="px-4 py-3 rounded-2xl rounded-tl-none text-sm max-w-lg"
                          style={{ backgroundColor:G.navyBg, color:G.navyDeep }}>
                          {msg.text}
                        </div>
                        {/* Chips — Navy outline */}
                        {msg.chips&&msg.chips.length>0 && (
                          <div className="flex flex-wrap gap-2">
                            {msg.chips.map(chip=>(
                              <button key={chip} onClick={()=>handleChip(chip)}
                                ...
                                onMouseOver={e=>e.currentTarget.style.backgroundColor=G.navyBg}
                                onMouseOut={e=>e.currentTarget.style.backgroundColor="white"}>
                                {chip}
                              </button>
                            ))}
                  ...
                  : /* User bubble — Navy gradient */
                    <div className="flex justify-end">
                      <div className="px-4 py-3 rounded-2xl rounded-tr-none text-sm max-w-xs text-white"
          ...
          {/* Input row */}
          <div className="px-6 pb-6 pt-2" style={{ borderTop:`1px solid ${G.greenBorder}` }}>
            <div className="flex items-center gap-2 mt-3">
              <input type="text" placeholder="Type your answer..." value={input}
                onChange={e=>setInput(e.target.value)} onKeyDown={handleKeyDown} disabled={canGenerate}
                className="flex-1 px-4 py-3 rounded-xl text-sm outline-none transition-all"
                style={{ border:`1px solid ${G.greenBorder}`, backgroundColor:canGenerate?"#f9fafb":"white", color:"#1e293b" }}
                onFocus={e=>e.target.style.borderColor=G.navyLight} onBlur={e=>e.target.style.borderColor=G.greenBorder}/>
              <button onClick={handleSend} disabled={!input.trim()||canGenerate}
                className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 transition-all"
                style={{ background:input.trim()&&!canGenerate?G.gradNavy:G.navyBg, cursor:input.trim()&&!canGenerate?"pointer":"default" }}>
                <svg className="w-4 h-4" fill="none" stroke={input.trim()&&!canGenerate?"white":G.muted} viewBox="0 0 24 24">
        ...
        <div className="flex items-center justify-between mt-6 pb-10">
          ...
          <button onClick={handleNext} disabled={!canGenerate}
            className="flex items-center gap-2 font-bold px-7 py-3.5 rounded-full text-white text-sm transition-all"
            style={{ background:canGenerate?G.gradNavy:"#d1d5db", boxShadow:canGenerate?"0 3px 14px rgba(15,26,59,0.30)":"none", cursor:canGenerate?"pointer":"not-allowed" }}
        ...
        <div className="rounded-2xl p-5" style={{ backgroundColor:"white", border:`1px solid ${G.greenBorder}`, boxShadow:"0 2px 12px rgba(110,192,48,0.06)" }}>
          ...
          <div className="w-full h-1.5 rounded-full mb-3" style={{ backgroundColor:"#e2e8f0" }}>
            <div className="h-full rounded-full transition-all duration-700"
              style={{ width:`${understanding}%`, background:G.gradNavy }}/>
          </div>
