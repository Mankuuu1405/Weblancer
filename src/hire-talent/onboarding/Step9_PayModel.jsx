...
const PROJECT_TYPES = [
  { id:"fixed",    icon:<svg className="w-6 h-6" fill="none" stroke="#4A6FA5" viewBox="0 0 24 24"><path d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/></svg>,
    label:"Fixed-Price", desc:"Agreed total price upfront. Scope clearly defined.",
    points:["Well-defined projects","Specific deliverables","Budget certainty"],
    badge:"Best match for your project", badgeStyle:{ bg:G.navyBg, color:G.navy }, recommended:true },
  { id:"hourly",   icon:<svg className="w-6 h-6" fill="none" stroke="#4A6FA5" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" strokeWidth="1.8"/><path d="M12 6v6l4 2" strokeWidth="1.8" strokeLinecap="round"/></svg>,
    label:"Hourly", desc:"Pay based on time worked. More flexible scope.",
    points:["Ongoing development","Consulting work","Maintenance projects"], badge:null },
  { id:"retainer", icon:<svg className="w-6 h-6" fill="none" stroke="#4A6FA5" viewBox="0 0 24 24"><path d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/></svg>,
    label:"Retainer", desc:"Ongoing relationship. Monthly commitment.",
    points:["Continuous development","Maintenance + updates","Team augmentation"],
    badge:"Good for post-launch", badgeStyle:{ bg:G.greenBg, color:G.greenDeep } },
];

const SCHEDULES = [
  { id:"milestone", icon:<svg className="w-5 h-5" fill="none" stroke="#4A6FA5" viewBox="0 0 24 24"><path d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" strokeWidth="1.8"/><path d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" strokeWidth="1.8"/></svg>,
    label:"Milestone-Based", sub:"Break project into stages, pay as each completes", tag:"Recommended", disabled:false },
  { id:"single", icon:<svg className="w-5 h-5" fill="none" stroke="#9ca3af" viewBox="0 0 24 24"><path d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/></svg>,
    label:"Single Payment", sub:"Not available for first project", tag:null, disabled:true },
];
...
export default function Step9_PayModel({ formData, updateData, next, prev }) {
  const [projectType, setProjectType] = useState("fixed");
  const [schedule,    setSchedule]    = useState("milestone");
  const handleNext = () => { updateData({ projectType, schedule }); next(); };
      ...
      <div className="flex-1 min-w-0">
        <div className="rounded-2xl overflow-hidden" style={{ backgroundColor:"white", border:`1px solid ${G.greenBorder}`, boxShadow:"0 4px 24px rgba(110,192,48,0.08)" }}>
          ...
          <div className="p-8 sm:p-10">
            <h1 className="text-2xl font-extrabold mb-1" style={{ color:G.navyDeep }}>How Do You Want to Structure This Project?</h1>
            <p className="text-sm mb-6" style={{ color:G.sub }}>Choose the payment model that matches your needs</p>
            <span className="inline-block text-xs font-bold tracking-widest uppercase px-3 py-1 rounded mb-7"
              style={{ backgroundColor:G.navyBg, color:G.navy, border:`1px solid ${G.navyBorder}` }}>Payment Model</span>

            {/* Project Type */}
            <h2 className="text-base font-bold mb-4" style={{ color:G.navyDeep }}>Project Type</h2>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
              {PROJECT_TYPES.map(t=>{
                const sel = projectType===t.id;
                return (
                  <button key={t.id} onClick={()=>setProjectType(t.id)}
                    className="relative flex flex-col gap-3 p-5 rounded-2xl text-left transition-all"
                    style={{ border:sel?`2px solid ${G.navyLight}`:`1px solid ${G.greenBorder}`, backgroundColor:sel?G.navyBg:"white" }}>
                    {sel && <span className="absolute top-3 right-3" style={{ color:G.navyLight }}>
                    ...
                    <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ backgroundColor:G.navyBg }}>{t.icon}</div>
                    <div className="font-extrabold text-sm" style={{ color:G.navyDeep }}>{t.label}</div>
                    <div className="text-xs" style={{ color:G.sub }}>{t.desc}</div>
                    <div className="flex flex-col gap-1">
                      {t.points.map(p=>(
                        <div key={p} className="flex items-center gap-1.5 text-xs">
                          <svg className="w-3.5 h-3.5 flex-shrink-0" fill="none" stroke={G.green} viewBox="0 0 24 24"><path d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
                          <span style={{ color:G.sub }}>{p}</span>
                        </div>
                      ))}
                    </div>
                    {t.badge && <span className="text-xs font-semibold px-2.5 py-0.5 rounded-full w-fit"
                      style={{ backgroundColor:t.badgeStyle.bg, color:t.badgeStyle.color }}>{t.badge}</span>}
                  </button>
                );
              })}
            </div>
            ...
            {/* Payment Schedule */}
            <h2 className="text-base font-bold mb-4" style={{ color:G.navyDeep }}>Payment Schedule</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
              {SCHEDULES.map(s=>{
                const sel = schedule===s.id&&!s.disabled;
                return (
                  <button key={s.id} onClick={()=>!s.disabled&&setSchedule(s.id)} disabled={s.disabled}
                    className="flex items-start gap-3 p-5 rounded-xl text-left transition-all"
                    style={{ border:sel?`2px solid ${G.navyLight}`:`1px solid ${G.greenBorder}`, backgroundColor:sel?G.navyBg:s.disabled?"#f8fafc":"white", cursor:s.disabled?"not-allowed":"pointer", opacity:s.disabled?0.7:1 }}>
                    <div className="mt-0.5 flex-shrink-0">{s.icon}</div>
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        <span className="text-sm font-bold" style={{ color:s.disabled?G.muted:G.navyDeep }}>{s.label}</span>
                        {s.tag && <span className="text-xs font-semibold px-2 py-0.5 rounded-full"
                          style={{ backgroundColor:G.greenBg, color:G.greenDeep }}>{s.tag}</span>}
                      </div>
                      <div className="flex items-center gap-1.5 text-xs" style={{ color:G.sub }}>
                        {s.disabled && <svg className="w-3.5 h-3.5" fill="none" stroke={G.muted} viewBox="0 0 24 24"><path d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/></svg>}
                        {s.sub}
                      </div>
                    </div>
                  </button>
            ...
            {/* Milestone Breakdown */}
            <h2 className="text-base font-bold mb-4" style={{ color:G.navyDeep }}>Milestone Breakdown</h2>
            <div className="rounded-xl overflow-hidden mb-2" style={{ border:`1px solid ${G.greenBorder}` }}>
              {MILESTONES.map((m,i)=>(
                <div key={m.n} className="flex items-center gap-4 px-5 py-4"
                  style={{ borderBottom:i<MILESTONES.length-1?`1px solid ${G.greenBorder}`:"none", backgroundColor:"white" }}>
                  <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0"
                    style={{ background:G.gradNavy, color:"white" }}>{m.n}</div>
                  <div className="flex-1 min-w-0">
                    <div className="text-sm font-bold" style={{ color:G.navyDeep }}>{m.phase}</div>
                    <div className="text-xs truncate" style={{ color:G.muted }}>{m.desc}</div>
                    <div className="text-xs mt-0.5" style={{ color:G.sub }}>{m.duration} • {m.pct}</div>
                  </div>
                  <div className="text-sm font-extrabold flex-shrink-0" style={{ color:G.navyDeep }}>{m.amount}</div>
                </div>
              ))}
              <div className="flex justify-end px-5 py-3" style={{ backgroundColor:G.navyBg, borderTop:`1px solid ${G.navyBorder}` }}>
                <span className="text-sm" style={{ color:G.sub }}>Total: <span className="font-extrabold text-base" style={{ color:G.navyDeep }}>$45,000</span></span>
              </div>
            </div>

            {/* Escrow */}
            <div className="rounded-xl px-6 py-5 mt-6" style={{ border:`1px solid ${G.navyBorder}`, backgroundColor:G.navyBg }}>
              <div className="flex items-center gap-2 mb-4">
                <svg className="w-4 h-4" fill="none" stroke={G.navyLight} viewBox="0 0 24 24"><path d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/></svg>
                <span className="font-bold text-sm" style={{ color:G.navyDeep }}>How Escrow Works</span>
              </div>
              <ol className="flex flex-col gap-2 mb-4">
                {["Your money goes into secure escrow (not to the provider)","Work begins on each milestone","You review and approve deliverables","Payment released from escrow upon approval"].map((s,i)=>(
                  <li key={i} className="flex items-start gap-2 text-sm" style={{ color:G.sub }}>
                    <span className="font-semibold flex-shrink-0" style={{ color:G.navyLight }}>{i+1}.</span>{s}
                  </li>
                ))}
              </ol>
              <div className="flex items-center gap-2 flex-wrap">
                {["🔒 PCI DSS Compliant","🔒 256-bit Encryption"].map(b=>(
                  <span key={b} className="text-xs font-semibold px-3 py-1 rounded-full"
                    style={{ backgroundColor:"#fefce8", color:"#854d0e", border:"1px solid #fde68a" }}>{b}</span>
                ))}
              </div>
            </div>
          </div>
