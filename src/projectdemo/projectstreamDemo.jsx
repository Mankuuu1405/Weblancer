  ...
  return (
    <>
      ...
      <div className="px-4 py-3 border-b border-gray-100">
        ...
        <p className="text-xs font-bold text-green-600 text-right mt-1">40%</p>
      </div>
      <div className="px-4 py-3 border-b border-gray-100">
        <Label icon={<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/></svg>} text="Participants" />
        <div className="space-y-2">
          {participants.map((p) => {
            const badgeCls =
              p.role === "CLIENT"     ? "bg-green-500 text-white" :
              p.role === "FREELANCER" ? "bg-blue-500 text-white"  :
              p.role === "ADMIN"      ? "bg-yellow-400 text-white" :
              "bg-gray-100 text-gray-600";
            return (
              <div key={p.name} className="flex items-center gap-2">
                <span className={"w-2 h-2 rounded-full flex-shrink-0 "+(p.online?"bg-green-500":"bg-gray-300")}/>
                <span className="text-xs text-gray-700 font-medium flex-1">{p.name}</span>
                <span className={"text-xs font-bold px-2 py-0.5 rounded-md "+badgeCls}>{p.role}</span>
              </div>
            );
          })}
        </div>
      ...
      <div className="px-4 py-3 border-b border-gray-100">
        ...
        <div className="space-y-1">
          {quickActions.map((qa) => (
            <button key={qa.label} className="w-full flex items-center gap-2 text-xs text-gray-600 hover:text-green-700 font-medium py-1.5 transition-colors text-left">
              <span className="text-gray-400">{QAIcons[qa.label]}</span>{qa.label}
            </button>
          ))}
        </div>
...
function SidebarItem({ icon, label, active, badge, onClick }) {
  return (
    <button onClick={onClick} className={"w-full flex items-center gap-2.5 px-4 py-2 text-sm font-medium transition-all relative "+(active?"bg-green-50 text-green-700 font-semibold":"text-gray-600 hover:bg-gray-50 hover:text-gray-800")}>
      {active && <span className="absolute left-0 top-2 bottom-2 w-0.5 bg-green-500 rounded-r"/>}
      <span className={active?"text-green-500":"text-gray-400"}>{icon}</span>
      {label}
      {badge && <span className="ml-auto bg-green-500 text-white text-xs font-bold px-1.5 py-0.5 rounded-full leading-none">{badge}</span>}
    </button>
  );
}
  ...
  return (
    <nav className="flex md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 z-40 safe-area-pb">
      {tabs.map(({ label, view, icon }) => {
        ...
        return (
          <button key={view} onClick={() => setActiveView(view)}
            className={"relative flex-1 flex flex-col items-center justify-center py-2 gap-0.5 " + (active ? "text-green-600" : "text-gray-400")}>
            {active && <span className="absolute top-0 left-1/2 -translate-x-1/2 w-8 h-0.5 bg-green-500 rounded-b-full"/>}
            <span>{icon}</span>
            <span className={"text-xs " + (active ? "font-bold" : "font-medium")}>{label}</span>
          </button>
        );
  ...
  return (
    <div className="h-[100dvh] flex flex-col bg-gray-100 text-sm overflow-hidden">
      ...
      <nav className="bg-white border-b border-gray-200 flex items-center px-3 md:px-4 py-2.5 gap-2 flex-shrink-0 z-30">
        ...
        <button className="hidden md:flex items-center gap-1.5 text-sm text-gray-600 hover:text-green-700">
          <IconBack /> Back
        </button>
        <span className="font-bold text-gray-900 text-sm md:text-base truncate max-w-[140px] md:max-w-none">Food Delivery App</span>
        <div className="flex-1"/>
