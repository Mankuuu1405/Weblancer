...
const badgeSty = { display:"inline-flex", alignItems:"center", gap:4, fontSize:10, fontWeight:700, padding:"3px 9px", borderRadius:99 };
const statusBadge = {
...
function pillBtn(primary = false) {
  return {
    display:"inline-flex", alignItems:"center", gap:6, fontSize:12, fontWeight:700, fontFamily:FONT, padding:"7px 14px", borderRadius:100, cursor:"pointer",
...
export default function FilesView({ project }) {
  ...
  const getFilesForMilestone = (label) => files.filter(f => f.milestone === label);

  const handleUpload = (e) => {
    const file = e.target.files[0];
    if (!file) return;
    setFiles(prev => [{
      id:`f${Date.now()}`, name:file.name, type:file.name.split(".").pop(),
      milestone:"Milestone 2: Backend API", milestoneStatus:"in_progress",
      uploadedBy:"Raj Kumar", uploadedByRole:"agency_admin",
      uploadedAt: new Date().toLocaleString("en-IN", { day:"numeric", month:"short", year:"numeric", hour:"2-digit", minute:"2-digit" }),
      version:"1 of 1", replaces:null, note:null, status:"pending",
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", flex:1, overflow:"hidden", background:G.bg, fontFamily:FONT }}>
      ...
      {/* Header */}
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"12px 16px", background:G.white, borderBottom:`1px solid ${G.greenBorder}`, flexShrink:0, flexWrap:"wrap", gap:10, boxShadow:"0 2px 8px rgba(110,192,48,0.06)" }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:8 }}>
          <button onClick={() => setShowRequestDeliverable(true)} style={pillBtn()}>Request Deliverable</button>
      ...
      <div style={{ flex:1, overflowY:"auto", padding:"16px", display:"flex", flexDirection:"column", gap:20 }}>
          ...
          return (
            <div key={mg.id}>
              <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:10 }}>
                <p style={{ fontSize:10, fontWeight:800, color:G.greenDeep, textTransform:"uppercase", letterSpacing:"0.07em", margin:0 }}>{mg.label}</p>
                {mg.status === "in_progress" && <span style={{ fontSize:10, fontWeight:700, color:G.greenDeep }}>(In Progress)</span>}
              </div>
              {mgFiles.length === 0 ? (
                <div style={{ background:G.white, border:`1.5px dashed ${G.greenBorder}`, borderRadius:12, padding:"12px 16px", display:"flex", alignItems:"center", gap:10 }}>
                  <span style={{ fontSize:16 }}>📦</span>
                  <p style={{ fontSize:12, color:G.muted, margin:0 }}>No files submitted yet for this milestone.</p>
                </div>
              ) : (
                <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
                  {mgFiles.map(file => <FileCard key={file.id} file={file} onHistory={() => setShowHistory(file)} />)}
                </div>
              )}
            </div>
          );
        ...
        <div>
          <p style={{ fontSize:10, fontWeight:800, color:G.greenDeep, textTransform:"uppercase", letterSpacing:"0.07em", margin:"0 0 10px" }}>Project Documents</p>
          <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
            {projectDocs.map(file => <FileCard key={file.id} file={file} onHistory={() => setShowHistory(file)} isDoc />)}
          </div>
        </div>
        <div style={{ height:12 }} />
      </div>
...
function FileCard({ file, onHistory }) {
  const isReplaced = file.status === "replaced";
  return (
    <div style={{ background:G.white, border:`1px solid ${isReplaced ? G.border : G.greenBorder}`, borderRadius:14, padding:"14px 16px", opacity:isReplaced?0.7:1, boxShadow:isReplaced?"none":"0 2px 8px rgba(110,192,48,0.04)" }}>
      <div style={{ display:"flex", alignItems:"flex-start", gap:12 }}>
        <div style={{ flexShrink:0, marginTop:2, fontSize:20 }}>
          {file.type==="pdf" ? "📄" : file.type==="fig" ? "🎨" : "📁"}
        </div>
        <div style={{ flex:1, minWidth:0 }}>
          <div style={{ display:"flex", alignItems:"center", gap:8, flexWrap:"wrap", marginBottom:4 }}>
            <p style={{ fontSize:13, fontWeight:700, color:G.text, margin:0 }}>{file.name}</p>
            {statusBadge[file.status]}
          </div>
          <p style={{ fontSize:11, color:G.muted, margin:"0 0 2px" }}>Uploaded by {file.uploadedBy} · {file.uploadedAt}</p>
          <p style={{ fontSize:11, color:G.muted, margin:0 }}>Version {file.version}{file.replaces ? ` · Replaces: ${file.replaces}` : ""} · {file.size}</p>
          {file.note && (
          ...
          {isReplaced && <p style={{ fontSize:11, color:"#92400e", fontWeight:700, margin:"6px 0 0" }}>⚠ Older version</p>}
          <div style={{ display:"flex", alignItems:"center", gap:8, marginTop:10, flexWrap:"wrap" }}>
            ...
            <button onClick={onHistory} style={actionBtnSty}>
              <svg width="13" height="13" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
              History
            </button>
            {file.status === "locked" && <span style={{ fontSize:11, color:"#dc2626", fontWeight:600 }}>🔒 Locked</span>}
          </div>
        </div>
      </div>
    </div>
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
        {history.map((h, i) => (
          <div key={i} style={{ display:"flex", gap:12, alignItems:"flex-start" }}>
            <div style={{ width:28, height:28, borderRadius:"50%", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0, fontSize:11, fontWeight:700, background:h.status==="Current"?G.greenBg:G.bg, color:h.status==="Current"?G.greenDeep:G.sub, border:`1px solid ${h.status==="Current"?G.greenBorder:G.border}` }}>
              {h.version}
            </div>
            <div style={{ flex:1 }}>
              <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:3 }}>
                <p style={{ fontSize:13, fontWeight:700, color:G.text, margin:0 }}>{h.action}</p>
                <span style={{ fontSize:10, fontWeight:700, padding:"2px 7px", borderRadius:99, background:h.status==="Current"?G.greenBg:G.bg, color:h.status==="Current"?G.greenDeep:G.sub, border:`1px solid ${h.status==="Current"?G.greenBorder:G.border}` }}>{h.status}</span>
              </div>
              <p style={{ fontSize:11, color:G.muted, margin:0 }}>{h.date} · by {h.by}</p>
            </div>
          </div>
        ))}
      </div>
...
function RequestDeliverableModal({ onClose }) {
  ...
  return (
    <ModalShell title="Request Deliverable" onClose={onClose}>
      <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
        <div>
          <label style={labelSty}>Milestone</label>
          <select value={milestone} onChange={e => setMilestone(e.target.value)} style={inputSty}>
            <option value="">Select milestone...</option>
            {milestoneGroups.map(m => <option key={m.id} value={m.label}>{m.label}</option>)}
          </select>
        </div>
        <div>
          <label style={labelSty}>What do you need?</label>
          <textarea value={description} onChange={e => setDescription(e.target.value)} placeholder="Describe the deliverable..." rows={3} style={{ ...inputSty, resize:"none" }} />
        </div>
        <div>
          <label style={labelSty}>Expected by</label>
      ...
      <div style={{ display:"flex", justifyContent:"flex-end", gap:8, marginTop:20 }}>
        ...
        <button onClick={onClose} disabled={!milestone||!description} style={{ ...pillBtn(true), opacity:(!milestone||!description)?0.4:1, cursor:(!milestone||!description)?"not-allowed":"pointer" }}>
          Send Request
        </button>
      </div>
    </ModalShell>
