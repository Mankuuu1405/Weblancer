...
function Avatar({ name, color }) {
  const bg = color === "red" ? "bg-red-500" : color === "green" ? "bg-green-500" : "bg-blue-500";
  return (
    <div className={"w-8 h-8 md:w-9 md:h-9 rounded-full flex items-center justify-center text-white font-bold text-xs md:text-sm flex-shrink-0 " + bg}>
      {name[0]}
    </div>
  );
}
...
function RoleBadge({ role }) {
  const m = {
    ...
    FREELANCER: "bg-green-500 text-white",
    SUPPORT:    "bg-red-500 text-white",
  };
  return (
    <span className={"text-xs font-bold px-2 py-0.5 rounded-md " + (m[role] || "bg-gray-100 text-gray-600")}>
      {role}
    </span>
  );
}
...
function TypeBadge({ type }) {
  const m = {
    DECISION: "bg-orange-100 text-orange-600 border border-orange-200",
    APPROVAL: "bg-green-100 text-green-600 border border-green-200",
    DELIVERY: "bg-orange-100 text-orange-600 border border-orange-200",
    WARNING:  "bg-yellow-100 text-yellow-700 border border-yellow-200",
    UPDATE:   "bg-gray-100 text-gray-600 border border-gray-200",
  };
  const ic = { DECISION: "🔒", APPROVAL: "✅", DELIVERY: "📦", WARNING: "⚠️", UPDATE: "📋" };
  return (
    <span className={"text-xs font-bold px-2 py-0.5 rounded-md flex items-center gap-1 " + (m[type] || "bg-gray-100")}>
      {ic[type]} {type}
    </span>
  );
}
...
function SystemMsg({ icon, text, date, green }) {
  return (
    <div className="flex justify-center my-3">
      <div className={"max-w-xs md:max-w-lg text-center px-4 md:px-5 py-3 rounded-2xl " + (green ? "bg-green-50 border border-green-100" : "bg-gray-100")}>
        <p className="text-sm text-gray-600">{icon} {text}</p>
        {date && <p className="text-xs text-gray-400 mt-1">{date}</p>}
      </div>
    </div>
...
// ── Chat Message ───────────────────────────────────────
function ChatMessage({ msg }) {
  // Screenshot: ADMIN=red bg, CLIENT=green bg, FREELANCER=green bg
  const borderMap = { ADMIN: "border-l-red-400",   CLIENT: "border-l-green-400", FREELANCER: "border-l-green-400" };
  const bgMap     = { ADMIN: "bg-red-50",           CLIENT: "bg-green-50",        FREELANCER: "bg-green-50"        };
  const avatarColor = msg.role === "ADMIN" ? "red" : "green";

  return (
    <div className={"rounded-xl border-l-4 px-3 md:px-4 py-2.5 md:py-3 mb-3 border border-gray-100 "
      + (borderMap[msg.role] || "border-l-gray-300") + " "
      + (bgMap[msg.role] || "bg-white")}>

      {/* Header row */}
      <div className="flex items-center justify-between gap-2 mb-2 flex-wrap">
        <div className="flex items-center gap-2 flex-wrap">
          <Avatar name={msg.author} color={avatarColor} />
          <span className="font-bold text-gray-900 text-sm">{msg.author}</span>
          <RoleBadge role={msg.role} />
          {msg.msgType && <TypeBadge type={msg.msgType} />}
        </div>
      ...
      {/* Locked decision banner */}
      {msg.lockedDecision && (
        <p className="text-xs text-yellow-600 font-semibold mb-2 ml-8 md:ml-11 flex items-center gap-1">
      ...
      <div className="ml-8 md:ml-11">
          ...
          <div className="mt-3 space-y-2">
            {msg.attachments.map((a, i) => (
              <div key={i} className="flex items-center gap-3 bg-white border border-gray-200 rounded-lg px-3 py-2.5">
                <IconFile />
                <span className="text-sm text-gray-700 flex-1 truncate min-w-0">{a.name}</span>
                <span className="text-xs text-gray-400 flex-shrink-0">{a.size}</span>
                <button className="inline-flex items-center gap-1 text-xs font-semibold text-gray-700 hover:text-blue-600 border border-gray-200 rounded-md px-2.5 py-1 flex-shrink-0">
                  <IconDl /> Download
                </button>
              </div>
            ))}
        ...
        {/* Acknowledge block */}
        {msg.acknowledge && (
          <div className="mt-3 border border-orange-200 rounded-xl p-3 bg-white">
            <p className="text-xs text-orange-500 font-semibold mb-2 flex items-center gap-1">
              ⚠️ Certain actions are blocked until acknowledged.
            </p>
            <button className="bg-yellow-500 hover:bg-yellow-600 text-white text-sm font-bold px-5 py-2.5 rounded-xl flex items-center gap-2 transition-colors w-full justify-center">
              <IconCheck /> I have read and understood this message
            </button>
          </div>
        )}
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl px-3 md:px-5 py-3 md:py-4 mb-3">
      ...
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-sm font-bold text-gray-900">Support Request Opened</span>
      </div>
      <div className="space-y-1 text-sm text-gray-700 mb-3">
        <p><span className="text-gray-500">Ticket:</span> <span className="font-semibold">#SUP-20260318-1042</span></p>
        <p><span className="text-gray-500">Category:</span> 📋 Contract or scope question</p>
      ...
      {/* Platform Support reply — red left border */}
      <div className="border-l-4 border-red-400 bg-white rounded-r-xl px-4 py-3">
        <div className="flex items-center gap-2 mb-1 flex-wrap">
          <span className="text-blue-600 font-bold text-xs flex items-center gap-1">🛡️ Platform Support</span>
          <RoleBadge role="SUPPORT" />
          <span className="text-xs text-gray-400 ml-auto">Feb 12, 2:30 PM</span>
        </div>
        <p className="text-xs text-gray-500 mb-1.5">Re: Ticket #SUP-20260212-0891 — Payment or escrow issue</p>
        <p className="text-sm text-gray-700 leading-relaxed">
          Hi John, we have reviewed your case. The payment release for Milestone 1 is currently processing.
          It will appear in the agency account within 24 business hours. No action needed on your end.
          If not resolved by Feb 14, please reply here.
        </p>
        <p className="text-xs text-gray-400 mt-2">Ticket remains open until confirmed resolved.</p>
        <p className="text-xs text-gray-400">Seen by: Client ✓ · Agency Admin ✓</p>
      </div>
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl px-3 md:px-5 py-3 md:py-4 mb-3">
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-xs font-mono font-bold text-gray-400">[SYSTEM]</span>
        <span className="text-sm font-bold text-gray-900">Meeting Requested</span>
      </div>
      <div className="space-y-1 text-sm text-gray-700 mb-3">
        <p><span className="text-gray-500">Type:</span> 🔍 Requirement Clarification</p>
        <p><span className="text-gray-500">Requested by:</span> Sara M. (freelancer)</p>
      ...
      <div className="space-y-1.5 mb-3">
        {[
          { name: "Sara M.",  role: "FREELANCER", st: "Confirmed", ic: "✅" },
          { name: "John D.",  role: "CLIENT",     st: "Pending",   ic: "⏳" },
        ].map(p => (
          <div key={p.name} className="flex items-center gap-2">
            <span>{p.ic}</span>
            <span className="text-sm font-medium text-gray-800">{p.name}</span>
            <RoleBadge role={p.role} />
            <span className="ml-auto text-xs text-gray-400">{p.st}</span>
          </div>
        ))}
      </div>
      ...
      <div className="mb-3">
        <span className="text-xs font-bold text-yellow-500 border border-yellow-300 bg-yellow-50 px-3 py-1 rounded-full">
          ⏳ PENDING — Awaiting confirmation
        </span>
      </div>

      <div className="flex flex-wrap gap-2">
        <button className="bg-green-500 hover:bg-green-600 text-white text-sm font-semibold px-5 py-2 rounded-xl transition-colors">Accept ✅</button>
        <button className="bg-red-500 hover:bg-red-600 text-white text-sm font-semibold px-5 py-2 rounded-xl transition-colors">Decline ✕</button>
        <button className="border border-gray-200 text-gray-700 text-sm font-semibold px-4 py-2 rounded-xl hover:bg-gray-50 transition-colors">Propose New Time 🕐</button>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl px-3 md:px-5 py-3 md:py-4 mb-3">
      <div className="flex items-center gap-2 mb-3">
        <span className="text-base">🗓️</span>
        <span className="text-xs font-mono font-bold text-gray-400">[SYSTEM]</span>
        <span className="text-sm font-bold text-gray-900">Meeting Confirmed ✅</span>
      </div>
      <div className="space-y-1 text-sm text-gray-700 mb-3">
        <p><span className="text-gray-500">Type:</span> 💬 Project Discussion</p>
        <p><span className="text-gray-500">Requested by:</span> John D. (client)</p>
      ...
      <div className="space-y-1.5 mb-3">
        {[
          { name: "John D.",        role: "CLIENT"     },
          { name: "Sara M.",        role: "FREELANCER" },
          ...
          <div key={p.name} className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium text-gray-800">{p.name}</span>
            <RoleBadge role={p.role} />
            <span className="ml-auto text-xs text-gray-400">Confirmed</span>
          </div>
        ))}
      </div>
      ...
      <div className="mb-3">
        <span className="text-xs font-bold text-white bg-green-500 px-3 py-1 rounded-full">✅ CONFIRMED</span>
      </div>

      <div className="flex flex-wrap gap-2">
        {["Add to Google Calendar", "Add to Outlook", "Add to Apple Calendar"].map(l => (
          <button key={l} className="border border-gray-200 text-gray-700 text-xs font-medium px-3 py-1.5 rounded-lg hover:bg-gray-50 transition-colors">
            {l}
          </button>
        ))}
      </div>
...
// ── Meeting Starting Soon ──────────────────────────────
// Screenshot: green background, green full-width "Join Meeting Now" button
function MeetingStartingSoonCard() {
  return (
    <div className="bg-green-50 border border-green-100 rounded-xl px-3 md:px-5 py-3 md:py-4 mb-3">
      <p className="text-sm font-bold text-gray-900 mb-2">🗓️ Meeting Starting Soon!</p>
      <p className="text-sm text-gray-700 font-medium mb-0.5">💬 Project Discussion — Mar 20, 2:00 PM IST</p>
      <p className="text-xs text-gray-500 mb-4">Starts in: 15554 minutes</p>
      ...
      <button className="w-full bg-green-500 hover:bg-green-600 text-white text-sm font-bold py-3 rounded-xl mb-4 transition-colors">
        Join Meeting Now →
      </button>
...
function MeetingNotesCard() {
  return (
    <div className="bg-white border border-gray-200 rounded-xl mb-3 overflow-hidden">
      {/* Admin header with red left border */}
      <div className="border-l-4 border-red-400 px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <div className="w-9 h-9 rounded-full bg-red-500 flex items-center justify-center text-white font-bold text-sm">P</div>
          <span className="font-bold text-gray-900 text-sm">Platform Admin</span>
      ...
      <div className="px-3 md:px-5 py-3 md:py-4">
        ...
        <div className="border border-gray-100 rounded-xl p-4 mb-3">
          <div className="flex items-center gap-2 mb-1">
            <span className="text-base">📋</span>
            <p className="text-sm font-bold text-gray-900">Meeting Summary — Delivery Review</p>
          </div>
          <p className="text-xs text-gray-500">Feb 18, 2026 · 3:00 PM · Duration: 43 min</p>
          <p className="text-xs text-gray-500">Attendees: John D. · Sara M.</p>

          <div className="border-t border-gray-100 mt-3 pt-3">
            <p className="text-xs text-gray-500 font-medium mb-1">What was discussed:</p>
            <p className="text-sm text-gray-700 mb-3 leading-relaxed">
              Reviewed all 12 wireframe screens for the food delivery app. Client provided feedback on navigation flow and visual design preferences.
            </p>
        ...
        <p className="text-xs text-gray-400">Seen by: John D. ✓ · Sara M. ✓</p>
      </div>
    </div>
  );
}
...
// ── Project Completed ──────────────────────────────────
// Screenshot: green "Leave Review" button
function ProjectCompletedCard() {
  return (
    <div className="bg-white border border-gray-200 rounded-xl px-3 md:px-5 py-3 md:py-4 mb-3">
      <div className="flex items-center gap-2 mb-3">
        <span className="text-base">🎉</span>
        <span className="text-xs font-mono font-bold text-gray-400">[SYSTEM]</span>
        <span className="text-sm font-bold text-gray-900">Project Completed!</span>
      </div>
      <div className="text-sm text-gray-700 space-y-1 mb-3">
        <p>All 4 milestones have been approved.</p>
        <p>Total paid: <span className="text-green-600 font-bold">$42,000</span> (released from escrow)</p>
      ...
      <button className="inline-flex items-center gap-2 bg-green-500 hover:bg-green-600 text-white text-sm font-semibold px-5 py-2.5 rounded-xl transition-colors">
        <IconStar /> Leave Review →
      </button>
    </div>
  );
...
function PinnedBanner({ collapsed, onToggle }) {
  return (
    <div
      className="bg-white border-b border-gray-200 px-3 md:px-4 py-2 md:py-2.5 flex items-center gap-2 flex-shrink-0 cursor-pointer hover:bg-gray-50 transition-colors"
      onClick={onToggle}
    >
      <span className="text-red-400"><IconPin /></span>
      <span className="text-sm font-semibold text-gray-700">📌 Pinned (5)</span>
